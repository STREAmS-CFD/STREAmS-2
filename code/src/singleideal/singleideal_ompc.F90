module streams_equation_singleideal_ompc_object

  use streams_base_ompc_object
  use streams_field_object
  use streams_grid_object
  use streams_kernels_ompc
  ...
  implicit none
  private
  public :: equation_singleideal_ompc_object
  ...
  type :: equation_singleideal_ompc_object
    type(base_ompc_object) :: base_ompc
    ...
    integer(ikind) :: error
    real(rkind) :: time0
    real(rkind), allocatable, dimension(:,:) :: coeff_deriv1_ompc
    real(rkind), allocatable, dimension(:,:) :: coeff_deriv2_ompc
    real(rkind), allocatable, dimension(:) :: coeff_filter_ompc
    ...
    real(rkind), pointer, contiguous, dimension(:,:,:,:) :: w_aux_ompc
    integer, pointer, contiguous, dimension(:,:,:) :: fluid_mask_ompc
    integer, pointer, contiguous, dimension(:,:,:,:) :: ep_ord_change_ompc

    real(rkind), allocatable, dimension(:) :: winf_ompc, winf_past_shock_ompc
    real(rkind), allocatable, dimension(:,:,:,:) :: w_aux_debug
    real(rkind), allocatable, dimension(:,:,:,:) :: fl_debug
    real(rkind), allocatable, dimension(:,:,:,:) :: fhat_ompc
    real(rkind), allocatable, dimension(:,:,:,:) :: w_aux_trans_ompc, fl_trans_ompc, fhat_trans_ompc
    real(rkind), allocatable, dimension(:,:,:,:) :: fl_ompc, fln_ompc
    real(rkind), allocatable, dimension(:,:) :: dcoe_ompc

    real(rkind), allocatable, dimension(:,:,:,:) :: wrecyc_ompc
    real(rkind), allocatable, dimension(:,:,:) :: wrecycav_ompc
    real(rkind), dimension(:,:,:), allocatable :: wmean_ompc

    real(rkind), dimension(:,:,:), allocatable :: inflow_random_plane_ompc
    real(rkind), dimension(:), allocatable :: weta_inflow_ompc
    real(rkind), dimension(:), allocatable :: yplus_inflow_ompc, eta_inflow_ompc
    real(rkind), dimension(:), allocatable :: yplus_recyc_ompc, eta_recyc_ompc, eta_recyc_blend_ompc
    integer, dimension(:), allocatable :: map_j_inn_ompc, map_j_out_ompc, map_j_out_blend_ompc

    real(rkind), dimension(:), allocatable :: cv_coeff_ompc, cp_coeff_ompc

    real(rkind), dimension(:,:), allocatable :: w_aux_probe_ompc
    real(rkind), dimension(:,:,:,:), allocatable :: probe_coeff_ompc
    integer, dimension(:,:), allocatable :: ijk_probe_ompc
    real(rkind), allocatable, dimension(:,:,:) :: wallprop_ompc

    integer, dimension(:), allocatable :: lmax_tag_ompc
    integer, dimension(:), allocatable :: vis_tag_ompc

    real(rkind), dimension(:,:,:,:,:), allocatable :: w_tspec_ompc
    real(rkind), dimension(:,:), allocatable :: w_psd_tspec_ompc
    real(rkind), dimension(:,:), allocatable :: wfar_ompc
    real(rkind), dimension(:), allocatable :: f_sponge_ompc


    !insitu_var_start
    real(rkind), dimension(:,:,:,:), allocatable :: psi_ompc
    integer, dimension(:), allocatable :: aux_list_ompc, add_list_ompc
    !insitu_var_end
  ...
  contains
    ...
    procedure, pass(self) :: point_to_equation_base
    ...
    !insitu_proc_start
    procedure, pass(self) :: insitu_alloc_ompc
    procedure, pass(self) :: insitu_coprocess
    procedure, pass(self) :: insitu_compute_psi
    procedure, pass(self) :: insitu_do_catalyst_execute
    !insitu_proc_end

  endtype equation_singleideal_ompc_object

contains


  subroutine point_to_equation_base(self, equation_base)
    class(equation_singleideal_ompc_object), intent(inout) :: self
    class(equation_singleideal_object), target :: equation_base 
    self%w_aux_ompc          => equation_base%w_aux
    self%ep_ord_change_ompc => equation_base%ep_ord_change
    self%fluid_mask_ompc    => equation_base%fluid_mask
  endsubroutine point_to_equation_base
  ...
  subroutine insitu_alloc_ompc(self)
    class(equation_singleideal_ompc_object), intent(inout) :: self
    ...
    &x_list, n_add_list => self%equation_base%n_add_list )
      if (npsi > 0) allocate(self%psi_ompc(1-ng:nx+ng,1-ng:ny+ng,1-ng:nz+ng,npsi))
      if (n_aux_list > 0) then
        allocate(self%aux_list_ompc(1:n_aux_list))
        self%aux_list_ompc = self%equation_base%aux_list
      endif
      if (n_add_list > 0) then
        allocate(self%add_list_ompc(1:n_add_list))
        self%add_list_ompc = self%equation_base%add_list
      endif
    endassociate
  endsubroutine insitu_alloc_ompc
  ...
  subroutine visflx(self, mode)
    ...
    &exp, coeff_deriv1_ompc => self%coeff_deriv1_ompc, coeff_deriv2_ompc => self%coeff_deriv2_ompc,&
    & fhat_trans_ompc => self%fhat_trans_ompc, fl_trans_ompc => self%fl_trans_ompc, fl_ompc => self%fl_om&
    &pc, w_aux_ompc => self%w_aux_ompc, w_aux_trans_ompc => self%w_aux_trans_ompc, dcsidx_ompc => self%ba&
    &se_ompc%dcsidx_ompc, detady_ompc => self%base_ompc%detady_ompc, dzitdz_ompc => self%base_ompc%dzitdz&
    &_ompc, dcsidxs_ompc => self%base_ompc%dcsidxs_ompc, detadys_ompc => self%base_ompc%detadys_ompc,&
    & dzitdzs_ompc => self%base_ompc%dzitdzs_ompc, dcsidx2_ompc => self%base_ompc%dcsidx2_ompc,&
    & detady2_ompc => self%base_ompc%detady2_ompc, dzitdz2_ompc => self%base_ompc%dzitdz2_ompc,&
    & x_ompc => self%base_ompc%x_ompc, y_ompc => self%base_ompc%y_ompc, z_ompc => self%base_ompc%z_ompc,&
    ...
    & cv_coeff_ompc => self%cv_coeff_ompc, cp_coeff_ompc => self%cp_coeff_ompc, indx_cp_l => self%equatio&
    &n_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_ba&
    &se%calorically_perfect, t0 => self%equation_base%t0, channel_case => self%equation_base%channel_case&
    &, grid_dim => self%grid%grid_dim, dcsidxc2_ompc => self%base_ompc%dcsidxc2_ompc, dcsidyc2_ompc => se&
    &lf%base_ompc%dcsidyc2_ompc, detadxc2_ompc => self%base_ompc%detadxc2_ompc, detadyc2_ompc => self%bas&
    &e_ompc%detadyc2_ompc, g1_ompc => self%base_ompc%g1_ompc, g2_ompc => self%base_ompc%g2_ompc,&
    & g12_ompc => self%base_ompc%g12_ompc, jac_ompc => self%base_ompc%jac_ompc, vis_tag_ompc => self%vis_&
    &tag_ompc, wall_tag_ompc => self%base_ompc%wall_tag_ompc, iblock => self%field%ncoords(1),&
    & ite_rank_x => self%field%ite_rank_x, itu_rank_x => self%field%itu_rank_x, ite_l => self%field%ite_l&
    &, itu_l => self%field%itu_l, teshk => self%equation_base%teshk, ortho => self%equation_base%ortho,&
    & jweno => self%equation_base%jweno, theta_ij_ompc => self%base_ompc%theta_ij_ompc,&
    & theta_threshold => self%equation_base%theta_threshold)

      if (mode == 0) then
        if (grid_dim == 1) then
          call visflx_subroutine(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r,&
          & cp_coeff_ompc, calorically_perfect, u0, l0, self%base_ompc%w_ompc, self%w_aux_ompc, self%fl_ompc,&
          & coeff_deriv1_ompc, coeff_deriv2_ompc, dcsidx_ompc, detady_ompc, dzitdz_ompc, dcsidxs_ompc,&
          & detadys_ompc, dzitdzs_ompc, dcsidx2_ompc, detady2_ompc, dzitdz2_ompc, self%wallprop_ompc)
        elseif (grid_dim == 2) then
          call visflx_c2_subroutine(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r,&
          & cp_coeff_ompc, calorically_perfect, u0, l0, self%base_ompc%w_ompc, self%w_aux_ompc, self%fl_ompc,&
          & coeff_deriv1_ompc, coeff_deriv2_ompc, dcsidxc2_ompc, detadyc2_ompc, detadxc2_ompc, dcsidyc2_ompc,&
          & dzitdz_ompc, dzitdzs_ompc, dzitdz2_ompc, g1_ompc, g2_ompc, g12_ompc, jac_ompc, self%wallprop_ompc,&
          & vis_tag_ompc, wall_tag_ompc, iblock, ite_rank_x, itu_rank_x, ite_l, itu_l, teshk )
        endif
      elseif (mode == 1) then
        if (grid_dim == 1) then
          call visflx_div_subroutine(nx, ny, nz, ng, visc_order, self%w_aux_ompc, self%fl_ompc,&
          & coeff_deriv1_ompc, dcsidx_ompc, detady_ompc, dzitdz_ompc, stream1)
        elseif (grid_dim == 2) then
          call visflx_div_c2_subroutine(nx, ny, nz, ng, visc_order, self%w_aux_ompc, self%fl_ompc,&
          & coeff_deriv1_ompc, dcsidxc2_ompc, dcsidyc2_ompc, detadxc2_ompc, detadyc2_ompc, dzitdz_ompc,&
          & vis_tag_ompc, wall_tag_ompc, stream1)
        endif
      elseif (mode == 2) then
        call visflx_reduced_ord2_subroutine(nx, ny, nz, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_ompc, calorically_perfect, u0, l0, self%base_ompc%w_ompc, self%w_aux_ompc, self%fl_ompc,&
        & x_ompc, y_ompc, z_ompc, self%wallprop_ompc, 1)
      elseif (mode == 3) then
        if (grid_dim == 1) then
          call sensor_subroutine(nx, ny, nz, ng, u0, l0, self%w_aux_ompc)
        elseif (grid_dim == 2) then
          call sensor_c2_subroutine(nx, ny, nz, ng, u0, l0_ducros, self%w_aux_ompc, iblock,&
          & ite_rank_x, itu_rank_x, ite_l, itu_l, teshk, theta_ij_ompc, theta_threshold, jweno)
        endif
      elseif (mode == 4) then
        if (grid_dim == 1) then
          call visflx_nosensor_subroutine(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l,&
          & indx_cp_r, cp_coeff_ompc, calorically_perfect, u0, l0, self%base_ompc%w_ompc, self%w_aux_ompc,&
          & self%fl_ompc, coeff_deriv1_ompc, coeff_deriv2_ompc, dcsidx_ompc, detady_ompc, dzitdz_ompc,&
          & dcsidxs_ompc, detadys_ompc, dzitdzs_ompc, dcsidx2_ompc, detady2_ompc, dzitdz2_ompc,&
          & self%wallprop_ompc)
        elseif (grid_dim == 2) then
          call visflx_nosensor_c2_subroutine(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l,&
          & indx_cp_r, cp_coeff_ompc, calorically_perfect, iblock, ite_rank_x, itu_rank_x, ite_l, itu_l, u0,&
          & l0, self%base_ompc%w_ompc, self%w_aux_ompc, self%fl_ompc, coeff_deriv1_ompc, coeff_deriv2_ompc,&
          & dcsidxc2_ompc, detadyc2_ompc, detadxc2_ompc, dcsidyc2_ompc, dzitdz_ompc, dzitdzs_ompc,&
          & dzitdz2_ompc, g1_ompc, g2_ompc, g12_ompc, jac_ompc, self%wallprop_ompc, vis_tag_ompc,&
          & wall_tag_ompc, ortho)
        endif
      elseif (mode == 5) then
        call visflx_x_subroutine(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_ompc, calorically_perfect, self%base_ompc%x_ompc, w_aux_ompc, self%fl_ompc,&
        & self%fhat_ompc)
        call visflx_y_subroutine(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_ompc, calorically_perfect, self%base_ompc%y_ompc, self%w_aux_ompc, self%fl_ompc)
        call visflx_z_subroutine(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_ompc, calorically_perfect, self%base_ompc%z_ompc, self%w_aux_ompc, self%fl_ompc)
      elseif (mode == 6) then
        call visflx_reduced_ord2_subroutine(nx, ny, nz, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_ompc, calorically_perfect, u0, l0, self%base_ompc%w_ompc, self%w_aux_ompc, self%fl_ompc,&
        & x_ompc, y_ompc, z_ompc, self%wallprop_ompc, 0)
      elseif (mode == 7) then
        call visflx_div_ord2_subroutine(nx, ny, nz, ng, self%w_aux_ompc, self%fl_ompc, x_ompc, y_ompc, z_ompc, stream1)
      endif
    endassociate
  ...
  subroutine compute_aux(self, central, ghost)
    ...
    &%sutherland_s, powerlaw_vtexp => self%powerlaw_vtexp, cv_coeff_ompc => self%cv_coeff_ompc,&
    & cp_coeff_ompc => self%cp_coeff_ompc, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%e&
    &quation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_perfect,&
    & rgas0 => self%equation_base%rgas0)
      if(central_ == 1 .and. ghost_ == 1) then
        ...
        & self%base_ompc%w_ompc, self%w_aux_ompc, visc_model, mu0, t0, sutherland_s, t_ref_dim,&
        & powerlaw_vtexp, visc_power, visc_sutherland, visc_no, cv_coeff_ompc, indx_cp_l, indx_cp_r, rgas0,&
        & calorically_perfect, tol_iter_nr,stream1)
      elseif(central_ == 1 .and. ghost_ == 0) then
        call eval_aux_subroutine(nx, ny, nz, ng, 1, nx, 1, ny, 1, nz, self%base_ompc%w_ompc,&
        & self%w_aux_ompc, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power,&
        & visc_sutherland, visc_no, cv_coeff_ompc, indx_cp_l, indx_cp_r, rgas0, calorically_perfect,&
        & tol_iter_nr,stream1)
  ...
  subroutine rk_sync(self)
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        call init_flux_subroutine(nx, ny, nz, nv, self%fl_ompc, self%fln_ompc, rhodt)
        call self%base_ompc%bcswap()
        ...
        if(flow_init == 5) then
          call self%base_ompc%bcswap_wake_var(self%w_aux_ompc(:,:,:,10:10))
          call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,10:10))
          call self%bcextr_var(self%w_aux_ompc(:,:,:,10:10), mode=2)
          call self%base_ompc%bcte_var(self%w_aux_ompc(:,:,:,10:10))
        else
          call self%bcextr_var(self%w_aux_ompc(:,:,:,10:10), mode=1)
          call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,10:10))
        endif
        ...
        if (istep == 3) then
          ...
          call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,8:8))
          if(flow_init == 5) then
            call self%base_ompc%bcswap_wake_var(self%w_aux_ompc(:,:,:,8:8))
          endif
        endif
        if (conservative_viscous==1) then
        ...
        call update_flux_subroutine(nx, ny, nz, nv, self%fl_ompc, self%fln_ompc, gamdt)
        if (channel_case) call self%force_rhs()
        if (grid_dim == 2 .and. enable_sponge > 0) call self%sponge()
        call update_field_subroutine(nx, ny, nz, ng, nv, self%base_ompc%w_ompc, self%fln_ompc, self%fluid_mask_ompc)
  ...
  subroutine bcextr_var(self, w_swap, mode)
    ...
    & irightz => self%field%irightz)
      if(mode == 1) then
        call bcextr_var_subroutine(nx, ny, nz, ng, w_swap)
      elseif(mode == 2) then
        call bcextr_airfoil_var_subroutine(nx, ny, nz, ng, ndim, self%base_ompc%wall_tag_ompc,&
        & w_swap, ileftx, irightx, ileftz, irightz)
      endif
    endassociate
  ...
  subroutine rk_async(self)
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        call self%base_ompc%bcswap(steps=[.true.,.false.,.false.])
        call self%euler_x(lmax+1,nx-lmax,lmax,nx-lmax,do_update=.false.)
        call self%base_ompc%bcswap(steps=[.false.,.true.,.true.])
        ...
        if(flow_init == 5) then
          call self%base_ompc%bcswap_wake_var(self%w_aux_ompc(:,:,:,10:10))
          call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,10:10), steps=[.true.,.false.,.false.])
          call self%euler_y(eul_jmin,eul_jmax)
          call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,10:10), steps=[.false.,.true.,.true.])
          call self%bcextr_var(self%w_aux_ompc(:,:,:,10:10), mode=2)
          call self%base_ompc%bcte_var(self%w_aux_ompc(:,:,:,10:10))
        else
          call self%bcextr_var(self%w_aux_ompc(:,:,:,10:10), mode=1)
        ...
        if (istep == 3) then
          ...
          if(flow_init == 5) then
            call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,8:8))
            call self%base_ompc%bcswap_wake_var(self%w_aux_ompc(:,:,:,8:8), steps=[.true.,.false.,.false.])
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            else
              call self%visflx(mode=1)
            endif
            call self%base_ompc%bcswap_wake_var(self%w_aux_ompc(:,:,:,8:8), steps=[.false.,.true.,.true.])
          else
            call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,8:8), steps=[.true.,.false.,.false.])
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            else
              call self%visflx(mode=1)
            endif
            call self%base_ompc%bcswap_var(self%w_aux_ompc(:,:,:,8:8), steps=[.false.,.true.,.true.])
          endif
        else
          if (conservative_viscous==1) then
            call self%visflx(mode=7)
  ...
  subroutine sponge(self)
    class(equation_singleideal_ompc_object), intent(inout) :: self
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_ompc=>self%base_ompc%w_o&
    &mpc, wfar_ompc=>self%wfar_ompc,fln_ompc=>self%fln_ompc,f_sponge_ompc=>self%f_sponge_ompc,&
    & j_sponge=>self%equation_base%j_sponge)

      call sponge_subroutine(nx,ny,nz,ng,nv,w_ompc,wfar_ompc,fln_ompc,f_sponge_ompc,j_sponge)
    endassociate
  ...
  subroutine limiter(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,w_ompc=>self%base_ompc%w_ompc,&
    &w_aux_ompc=>self%w_aux_ompc, ncoords=>self%field%ncoords, indx_cp_l=>self%equation_base%indx_cp_l,&
    & indx_cp_r=>self%equation_base%indx_cp_r, cv_coeff_ompc => self%cv_coeff_ompc, calorically_perfect =&
    &> self%equation_base%calorically_perfect, t0 => self%equation_base%t0, flow_init => self%equation_ba&
    &se%flow_init, rho_lim => self%equation_base%rho_lim, tem_lim => self%equation_base%tem_lim,&
    & rho_lim_rescale => self%equation_base%rho_lim_rescale, tem_lim_rescale => self%equation_base%tem_li&
    &m_rescale)
      iblock = ncoords(1)
      kblock = ncoords(3)

      call limiter_subroutine(nx,ny,nz,ng,w_ompc,w_aux_ompc,iblock,kblock,indx_cp_l,indx_cp_r,&
      &cv_coeff_ompc,calorically_perfect,t0, tol_iter_nr, rho_lim, tem_lim, rho_lim_rescale,&
      & tem_lim_rescale)
    endassociate
  ...
  subroutine filter(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,w_ompc=>self%base_ompc%w_ompc,&
    &w_aux_ompc=>self%w_aux_ompc,indx_cp_l=>self%equation_base%indx_cp_l,indx_cp_r=>self%equation_base%in&
    &dx_cp_r,cv_coeff_ompc => self%cv_coeff_ompc,calorically_perfect => self%equation_base%calorically_pe&
    &rfect,t0 => self%equation_base%t0, coeff_filter_ompc => self%coeff_filter_ompc,jfilter => self%equat&
    &ion_base%jfilter, wall_tag_ompc => self%base_ompc%wall_tag_ompc)


      call filter_subroutine(nx,ny,nz,ng,w_ompc,w_aux_ompc,indx_cp_l,indx_cp_r,cv_coeff_ompc,&
  ...
  subroutine tripping(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_ompc=>self%base_ompc%w_o&
    &mpc,fl_ompc=>self%fl_ompc, xc2_ompc => self%base_ompc%xc2_ompc, yc2_ompc => self%base_ompc%yc2_ompc,&
    & z_ompc => self%base_ompc%z_ompc, ncoords=>self%field%ncoords, rlz => self%grid%domain_size(3),&
    & wall_tag_ompc => self%base_ompc%wall_tag_ompc, dxdetanc2_ompc => self%base_ompc%dxdetanc2_ompc,&
    & dydetanc2_ompc => self%base_ompc%dydetanc2_ompc, a_tr => self%equation_base%a_tr,&
    & u0 => self%equation_base%u0, time => self%equation_base%time, del0_tr => self%equation_base%del0_tr&
    &, masterproc => self%masterproc, itr1 => self%equation_base%itr1, itr2 => self%equation_base%itr2,&
    & its1 => self%equation_base%its1, its2 => self%equation_base%its2, x0tr => self%equation_base%x0tr,&
    & y0tr => self%equation_base%y0tr, x0ts => self%equation_base%x0ts, y0ts => self%equation_base%y0ts,&
    & lamz => self%equation_base%lamz , lamz1 => self%equation_base%lamz1 , lams => self%equation_base%la&
    &ms , lams1 => self%equation_base%lams1 , phiz => self%equation_base%phiz , phiz1 => self%equation_ba&
    &se%phiz1 , phis => self%equation_base%phis , phis1 => self%equation_base%phis1, lamz_old => self%equ&
    &ation_base%lamz_old , lamz1_old => self%equation_base%lamz1_old , lams_old => self%equation_base%lam&
    &s_old , lams1_old => self%equation_base%lams1_old , phiz_old => self%equation_base%phiz_old ,&
    ...
    &e%is_old)
      ...
      call tripping_pressure_subroutine(nx,ny,nz,ng,nv,i_rank_start,pi,itr1,itr2,x0tr,y0tr,x0ts,&
      ...
      call tripping_suction_subroutine(nx,ny,nz,ng,nv,i_rank_start,pi,its1,its2,x0tr,y0tr,x0ts,y0ts,&
      &lamx,lamy,lams,lams1,phis,phis1,asl,bt,xc2_ompc,yc2_ompc,z_ompc,w_ompc,fl_ompc,wall_tag_ompc,&
      & dxdetanc2_ompc, dydetanc2_ompc)
  ...
  subroutine euler_x(self, istart, iend, istart_face, iend_face, do_update)
    ...
    &se%force_zero_flux, coeff_deriv1_ompc => self%coeff_deriv1_ompc, dcsidx_ompc => self%base_ompc%dcsid&
    &x_ompc, fhat_ompc => self%fhat_ompc, w_ompc => self%base_ompc%w_ompc, w_aux_ompc => self%w_aux_ompc,&
    & fl_ompc => self%fl_ompc, sensor_threshold => self%equation_base%sensor_threshold,&
    & weno_scheme => self%equation_base%weno_scheme, weno_version => self%equation_base%weno_version,&
    & cp_coeff_ompc => self%cp_coeff_ompc, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%e&
    &quation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_perfect,&
    & ep_ord_change_ompc => self%ep_ord_change_ompc, nkeep => self%equation_base%nkeep,&
    & flux_splitting => self%equation_base%flux_splitting, dcsidxc2_ompc => self%base_ompc%dcsidxc2_ompc,&
    & dcsidxnc2_ompc => self%base_ompc%dcsidxnc2_ompc, dcsidyc2_ompc => self%base_ompc%dcsidyc2_ompc,&
    & dcsidync2_ompc => self%base_ompc%dcsidync2_ompc, jac_ompc => self%base_ompc%jac_ompc,&
    & mcsijac1_ompc => self%base_ompc%mcsijac1_ompc, lmax_tag_ompc => self%lmax_tag_ompc,&
    & eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_base%eul_imax,&
    & rgas0 => self%equation_base%rgas0)
      ...
      if(iend - istart >= 0) then
        ...
        if (self%grid%grid_dim == 1) then
          if (flux_splitting==1) then
            ...
            & iend_face_, lmax, nkeep, rgas0, w_aux_ompc, coeff_deriv1_ompc, dcsidx_ompc, fhat_ompc,&
            & force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size,&
            & cp_coeff_ompc, indx_cp_l, indx_cp_r, ep_ord_change_ompc, calorically_perfect, tol_iter_nr,&
            ...
            & lmax, nkeep, rgas0, w_aux_ompc, coeff_deriv1_ompc, dcsidx_ompc, fhat_ompc, force_zero_flux_min,&
            & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_ompc,&
            & indx_cp_l, indx_cp_r, ep_ord_change_ompc, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
            &self%equation_base%u0, self%equation_base%t0)
          endif

          if(do_update_) then
            call euler_x_update_subroutine(nx, ny, nz, ng, nv, eul_imin, eul_imax, fhat_ompc, fl_ompc, dcsidx_ompc, stream1)
          endif

        elseif (self%grid%grid_dim == 2) then
          call euler_x_fluxes_hybrid_c2_kernel(nv, nv_aux, nx, ny, nz, ng, istart, iend, lmax,&
          & nkeep, rgas0, w_aux_ompc, coeff_deriv1_ompc, fhat_ompc, dcsidxc2_ompc, dcsidxnc2_ompc,&
          & dcsidyc2_ompc, dcsidync2_ompc, jac_ompc, mcsijac1_ompc, lmax_tag_ompc, force_zero_flux_min,&
          & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_ompc,&
          & indx_cp_l, indx_cp_r, ep_ord_change_ompc, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
          & self%equation_base%u0, self%equation_base%t0)
          call euler_x_update_c2_subroutine(nx, ny, nz, ng, nv, istart, iend, fhat_ompc, fl_ompc, jac_ompc, stream1)
        endif
  ...
  subroutine euler_y(self, eul_jmin, eul_jmax)
    ...
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & nv_aux => self%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_ba&
    &se%force_zero_flux, coeff_deriv1_ompc => self%coeff_deriv1_ompc, detady_ompc => self%base_ompc%detad&
    &y_ompc, fhat_ompc => self%fhat_ompc, w_aux_ompc => self%w_aux_ompc, fl_ompc => self%fl_ompc,&
    ...
    &rfect, cp_coeff_ompc => self%cp_coeff_ompc, ep_ord_change_ompc => self%ep_ord_change_ompc,&
    & nkeep => self%equation_base%nkeep, wall_tag_ompc => self%base_ompc%wall_tag_ompc,&
    & flux_splitting => self%equation_base%flux_splitting, detadxc2_ompc => self%base_ompc%detadxc2_ompc,&
    & detadxnc2_ompc => self%base_ompc%detadxnc2_ompc, detadyc2_ompc => self%base_ompc%detadyc2_ompc,&
    & detadync2_ompc => self%base_ompc%detadync2_ompc, jac_ompc => self%base_ompc%jac_ompc,&
    & metajac1_ompc => self%base_ompc%metajac1_ompc, rgas0 => self%equation_base%rgas0)
      weno_size = 2*weno_scheme
      lmax = ep_order/2
      ...
      if (self%grid%grid_dim == 1) then
        if (flux_splitting==1) then
          ...
          & nkeep, rgas0, w_aux_ompc, fl_ompc, coeff_deriv1_ompc, detady_ompc, fhat_ompc, force_zero_flux_min,&
          & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_ompc,&
          & indx_cp_l, indx_cp_r, ep_ord_change_ompc, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
          &self%equation_base%u0, self%equation_base%t0)
        else
        ...
        call euler_y_update_subroutine(nx, ny, nz, ng, nv, eul_jmin, eul_jmax, fhat_ompc, fl_ompc, detady_ompc, stream1)
      elseif (self%grid%grid_dim == 2) then
        call euler_y_hybrid_c2_kernel(nv, nv_aux, nx, ny, nz, ng, eul_jmin, eul_jmax, lmax, nkeep,&
        & rgas0, w_aux_ompc, fl_ompc, coeff_deriv1_ompc, fhat_ompc, detadxc2_ompc, detadxnc2_ompc,&
        & detadyc2_ompc, detadync2_ompc, jac_ompc, metajac1_ompc, wall_tag_ompc, force_zero_flux_min,&
        & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_ompc,&
        & indx_cp_l, indx_cp_r, ep_ord_change_ompc, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0, self%equation_base%t0)
        call euler_y_update_c2_subroutine(nx, ny, nz, ng, nv, eul_jmin, eul_jmax, fhat_ompc,&
        & fl_ompc, jac_ompc, wall_tag_ompc, stream1)
      endif
    endassociate
  ...
  subroutine euler_z(self, eul_kmin, eul_kmax)
    ...
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & nv_aux => self%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_ba&
    &se%force_zero_flux, coeff_deriv1_ompc => self%coeff_deriv1_ompc, dzitdz_ompc => self%base_ompc%dzitd&
    &z_ompc, fhat_ompc => self%fhat_ompc, w_aux_ompc => self%w_aux_ompc, fl_ompc => self%fl_ompc,&
    & sensor_threshold => self%equation_base%sensor_threshold, weno_scheme => self%equation_base%weno_sch&
    &eme, weno_version => self%equation_base%weno_version, cp_coeff_ompc => self%cp_coeff_ompc,&
    & indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r,&
    & calorically_perfect => self%equation_base%calorically_perfect, ep_ord_change_ompc => self%ep_ord_ch&
    &ange_ompc, nkeep => self%equation_base%nkeep, flux_splitting => self%equation_base%flux_splitting,&
    & rgas0 => self%equation_base%rgas0, inflow_random_plane => self%equation_base%inflow_random_plane,&
    & inflow_random_plane_ompc => self%inflow_random_plane_ompc)
      weno_size = 2*weno_scheme
      lmax = ep_order/2
      ...
      if (flux_splitting==1) then
        ...
        & nkeep, rgas0, w_aux_ompc, fl_ompc, coeff_deriv1_ompc, dzitdz_ompc, fhat_ompc, force_zero_flux_min,&
        & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_ompc,&
        & indx_cp_l, indx_cp_r, ep_ord_change_ompc, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0, self%equation_base%t0)
      else
      ...
      call euler_z_update_subroutine(nx, ny, nz, ng, nv, eul_kmin, eul_kmax, fhat_ompc, fl_ompc, dzitdz_ompc, 0)
  ...
  subroutine force_rhs(self)
    ...
    & yn_ompc => self%base_ompc%yn_ompc, w_ompc => self%base_ompc%w_ompc, w_aux_ompc => self%w_aux_ompc,&
    & fln_ompc => self%fln_ompc, volchan => self%equation_base%volchan, fluid_mask_ompc => self%fluid_mas&
    &k_ompc, r_curv => self%grid%r_curv, volume => self%field%volume, dxdcsinc2_ompc => self%base_ompc%dx&
    &dcsinc2_ompc, dydcsinc2_ompc => self%base_ompc%dydcsinc2_ompc, dcsidxnc2_ompc => self%base_ompc%dcsi&
    &dxnc2_ompc, dcsidync2_ompc => self%base_ompc%dcsidync2_ompc, jac_ompc => self%base_ompc%jac_ompc)

      if(self%grid%grid_dim == 1) then
        call force_rhs_1_subroutine(nx, ny, nz, ng, yn_ompc, fln_ompc, w_ompc, w_aux_ompc, bulk5, fluid_mask_ompc)
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_1_c2_subroutine(nx, ny, nz, ng, yn_ompc, fln_ompc, w_ompc, w_aux_ompc, bulk5,&
      ...
      if(self%grid%grid_dim == 1) then
        call force_rhs_2_subroutine(nx, ny, nz, ng, fln_ompc, w_aux_ompc, bulk5g, fluid_mask_ompc)
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_2_c2_subroutine(nx, ny, nz, ng, fln_ompc, w_aux_ompc, bulk5g,&
        & fluid_mask_ompc, r_curv, yn_ompc, dxdcsinc2_ompc, dydcsinc2_ompc)
      endif
    endassociate
  ...
  subroutine force_var(self)
    ...
    & yn_ompc => self%base_ompc%yn_ompc, w_ompc => self%base_ompc%w_ompc, w_aux_ompc => self%w_aux_ompc,&
    & fln_ompc => self%fln_ompc, volchan => self%equation_base%volchan, fluid_mask_ompc => self%fluid_mas&
    &k_ompc, cv_coeff_ompc => self%cv_coeff_ompc, volume => self%field%volume, t0 => self%equation_base%t&
    &0, time => self%equation_base%time, calorically_perfect => self%equation_base%calorically_perfect,&
    & jac_ompc => self%base_ompc%jac_ompc, dcsidxnc2_ompc => self%base_ompc%dcsidxnc2_ompc,&
    & dcsidync2_ompc => self%base_ompc%dcsidync2_ompc, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r)

      if (self%equation_base%theta_wall>=-1._rkind) then
        if(self%grid%grid_dim == 1) then
          call force_var_1_subroutine(nx, ny, nz, ng, yn_ompc, fln_ompc, w_ompc, w_aux_ompc, bulkt,&
          ...
          call force_var_2_subroutine(nx, ny, nz, ng, w_ompc, w_aux_ompc, tbdiff, fluid_mask_ompc,&
          & cv_coeff_ompc, indx_cp_l, indx_cp_r, t0, calorically_perfect)
        elseif(self%grid%grid_dim == 2) then
          call force_var_1_c2_subroutine(nx, ny, nz, ng, yn_ompc, fln_ompc, w_ompc, w_aux_ompc,&
          & bulkt, fluid_mask_ompc, cv_coeff_ompc, indx_cp_l, indx_cp_r, t0, calorically_perfect, tol_iter_nr,&
          & jac_ompc, dcsidxnc2_ompc, dcsidync2_ompc)
          call mpi_allreduce(bulkt,bulktg,1,mpi_prec,mpi_sum,self%equation_base%field%mp_cart,self%mpi_err)
          ...
          call force_var_2_c2_subroutine(nx, ny, nz, ng, w_ompc, w_aux_ompc, tbdiff,&
          & fluid_mask_ompc, cv_coeff_ompc, indx_cp_l, indx_cp_r, t0, calorically_perfect, jac_ompc)
        endif
      endif
    endassociate
  ...
  subroutine recyc_exchange(self)
    ...
    & nblocks => self%equation_base%field%nblocks, iermpi => self%mpi_err, w_ompc => self%base_ompc%w_omp&
    &c, nv => self%equation_base%nv, wbuf1s_ompc => self%base_ompc%wbuf1s_ompc, wbuf2r_ompc => self%base_&
    &ompc%wbuf2r_ompc, wbuf1r_ompc => self%base_ompc%wbuf1r_ompc, wrecyc_ompc => self%wrecyc_ompc,&
    & ibrecyc => self%equation_base%ib_recyc, irecyc => self%equation_base%i_recyc)
      ...
      if (ncoords(1)==ibrecyc) then
        ...
        call recyc_exchange_subroutine_1(irecyc, w_ompc, wbuf1s_ompc, nx, ny, nz, ng, nv)
        indx = nv*ng*ny*nz
        call mpi_isend(wbuf1s_ompc,indx,mpi_prec,sendto1,2000,mp_cart,req(1),iermpi)
        call mpi_isend(wbuf1s_ompc,indx,mpi_prec,sendto2,3000,mp_cart,req(2),iermpi)
      endif
      if (ncoords(1)==0) then
        ...
        indx = nv*ng*ny*nz
        call mpi_irecv(wbuf1r_ompc,indx,mpi_prec,recvfrom1,2000,mp_cart,req(3),iermpi)
        call mpi_irecv(wbuf2r_ompc,indx,mpi_prec,recvfrom2,3000,mp_cart,req(4),iermpi)
      endif
      call mpi_waitall(4,req,mpi_statuses_ignore,iermpi)
      if (ncoords(1)==0) then
        call recyc_exchange_subroutine_2(n1_start_recv, n1_start_send, n1_end_recv, wrecyc_ompc, wbuf1r_ompc, nx, ny, nz, ng, nv)
        call recyc_exchange_subroutine_3(n2_start_recv, n2_start_send, n2_end_recv, wrecyc_ompc, wbuf2r_ompc, nx, ny, nz, ng, nv)
      endif
    endassociate
  ...
  subroutine bc_nr(self)
    ...
    & nz => self%nz, nv => self%nv, ng => self%ng, w_aux_ompc => self%w_aux_ompc, w_ompc => self%base_omp&
    &c%w_ompc, fl_ompc => self%fl_ompc, dcsidx_ompc => self%base_ompc%dcsidx_ompc, detady_ompc => self%ba&
    &se_ompc%detady_ompc, jac_ompc => self%base_ompc%jac_ompc , dcsidxnc2_ompc => self%base_ompc%dcsidxnc&
    &2_ompc , dcsidync2_ompc => self%base_ompc%dcsidync2_ompc , dcsidxc2_ompc => self%base_ompc%dcsidxc2_&
    &ompc , dcsidyc2_ompc => self%base_ompc%dcsidyc2_ompc , detadxnc2_ompc => self%base_ompc%detadxnc2_om&
    &pc , detadync2_ompc => self%base_ompc%detadync2_ompc , detadxc2_ompc => self%base_ompc%detadxc2_ompc&
    & , detadyc2_ompc => self%base_ompc%detadyc2_ompc , wall_tag_ompc => self%base_ompc%wall_tag_ompc ,&
    & dzitdz_ompc => self%base_ompc%dzitdz_ompc, wfar_ompc => self%wfar_ompc, mcsi_ompc => self%base_ompc&
    &%mcsi_ompc, meta_ompc => self%base_ompc%meta_ompc, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_pe&
    &rfect, rgas0 => self%equation_base%rgas0, t0 => self%equation_base%t0, cp_coeff_ompc => self%cp_coef&
    &f_ompc, winf_ompc => self%winf_ompc)
      ...
      do ilat=1,2*self%grid%ndim
        if(bctags_nr(ilat) > 0) then
          ...
          if(dir == 1) then
            ...
            if(self%grid%grid_dim == 1) then
              call bc_nr_lat_x_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_ompc,&
              & w_ompc, fl_ompc, dcsidx_ompc, indx_cp_l, indx_cp_r, cp_coeff_ompc, winf_ompc, calorically_perfect,&
              &rgas0,t0)
            elseif(self%grid%grid_dim == 2) then
              call bc_nr_lat_x_c2_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv,&
              & w_aux_ompc, w_ompc, fl_ompc, dcsidx_ompc, indx_cp_l, indx_cp_r, cp_coeff_ompc, winf_ompc, jac_ompc,&
              & mcsi_ompc, dcsidxnc2_ompc, dcsidync2_ompc, dcsidxc2_ompc, dcsidyc2_ompc, calorically_perfect,rgas0,&
              &t0)
            endif
          endif
          if(dir == 2) then
            ...
            if(self%grid%grid_dim == 1) then
              call bc_nr_lat_y_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_ompc,&
              & w_ompc, fl_ompc, detady_ompc, indx_cp_l, indx_cp_r, cp_coeff_ompc, winf_ompc, calorically_perfect,&
              &rgas0,t0)
            elseif(self%grid%grid_dim == 2) then
              call bc_nr_lat_y_c2_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv,&
              & w_aux_ompc, w_ompc, fl_ompc, detady_ompc, indx_cp_l, indx_cp_r, cp_coeff_ompc, wfar_ompc, jac_ompc,&
              & meta_ompc, detadxnc2_ompc, detadync2_ompc, detadxc2_ompc, detadyc2_ompc, wall_tag_ompc,&
          ...
          if(dir == 3) then
            ...
            call bc_nr_lat_z_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_ompc,&
            & w_ompc, fl_ompc, dzitdz_ompc, indx_cp_l, indx_cp_r, cp_coeff_ompc, winf_ompc, calorically_perfect,&
            &rgas0,t0)
          endif
        endif
      enddo
  ...
  subroutine update_ghost(self, do_swap)
    ...
    do ilat=1,2*self%grid%ndim
      ...
      case(0)
      case(1)
        call bcfree_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%winf_ompc, self%base_ompc%w_ompc)
      case(3)
        call bcfree_sub_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%base_ompc%w_ompc,&
        & self%w_aux_ompc, self%equation_base%aoa, self%equation_base%t0, self%equation_base%ptot0,&
        & self%equation_base%ttot0, self%equation_base%rgas0, self%equation_base%indx_cp_l,&
        & self%equation_base%indx_cp_r, self%cv_coeff_ompc, self%cp_coeff_ompc, self%equation_base%caloricall&
        &y_perfect, tol_iter_nr)
      case(2)
        call bcextr_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_ompc%w_ompc)
      case(4)
        call bcextr_sub_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%p0,&
        & self%equation_base%rgas0, self%base_ompc%w_ompc, self%equation_base%indx_cp_l, self%equation_base%i&
        &ndx_cp_r, self%cv_coeff_ompc, self%equation_base%t0, self%equation_base%calorically_perfect)
      case(5)
        if(self%grid%grid_dim == 1) then
          call bcsym_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%base_ompc%w_ompc)
        elseif(self%grid%grid_dim == 2) then
          call bcsym_c2_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%base_ompc%w_ompc,&
          & self%base_ompc%dxdcsic2_ompc, self%base_ompc%dydcsic2_ompc)
        endif
      case(6)
        if (self%grid%is_y_staggered) then
          call bcwall_staggered_subroutine(ilat, self%nx, self%ny, self%nz, self%ng,&
          & self%equation_base%t_wall, self%base_ompc%w_ompc, self%w_aux_ompc, self%equation_base%indx_cp_l,&
          & self%equation_base%indx_cp_r, self%cv_coeff_ompc, self%equation_base%t0, self%equation_base%rgas0,&
          ...
          &, self%base_ompc%w_ompc, self%w_aux_ompc, self%equation_base%indx_cp_l, self%equation_base%indx_cp_r&
          &, self%cv_coeff_ompc, self%equation_base%t0, self%equation_base%rgas0, self%equation_base%caloricall&
          &y_perfect,tol_iter_nr)
        endif
      case(7)
        call bcshock_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%nv,&
        & self%base_ompc%w_ompc, self%winf_ompc, self%winf_past_shock_ompc, self%equation_base%xshock_imp,&
        & self%equation_base%shock_angle, self%base_ompc%x_ompc, self%base_ompc%y_ompc, self%equation_base%ta&
        &nhfacs)
      case(8)
      case(9)
        call bclam_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_ompc%w_om&
        &pc, self%wmean_ompc, self%equation_base%p0, self%equation_base%rgas0, self%equation_base%indx_cp_l,&
        & self%equation_base%indx_cp_r, self%cv_coeff_ompc, self%equation_base%t0, self%equation_base%caloric&
        &ally_perfect)
      case(10)
        call self%bcrecyc(ilat)
      case(11)
        call self%base_ompc%bcswap_wake()
        call bcwall_airfoil_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%&
        &t_wall, self%base_ompc%w_ompc, self%w_aux_ompc, self%base_ompc%wall_tag_ompc, self%equation_base%ind&
        &x_cp_l, self%equation_base%indx_cp_r, self%cv_coeff_ompc, self%equation_base%t0, self%equation_base%&
        &rgas0, self%equation_base%calorically_perfect,tol_iter_nr, self%base_ompc%xc2_ompc,&
        & self%base_ompc%yc2_ompc, self%equation_base%a_tw, self%equation_base%v_bs, self%equation_base%thic,&
        & self%equation_base%kx_tw, self%equation_base%om_tw, self%equation_base%xtw1, self%equation_base%xtw&
        &2, self%equation_base%time, self%base_ompc%dxdetanc2_ompc, self%base_ompc%dydetanc2_ompc,&
        & self%equation_base%u0)
        call self%base_ompc%bcte()
      endselect
    enddo

    if (do_swap_ == 1) call self%base_ompc%bcswap()
  ...
  subroutine bcrecyc(self, ilat)
    ...
    if (ilat == 1) then
      ...
      & ng => self%ng, nv => self%nv, wrecycav_ompc => self%wrecycav_ompc, wrecyc_ompc => self%wrecyc_ompc,&
      & mp_cartz => self%field%mp_cartz, iermpi => self%mpi_err, p0 => self%equation_base%p0,&
      & w_ompc => self%base_ompc%w_ompc, wmean_ompc => self%wmean_ompc, weta_inflow_ompc => self%weta_inflo&
      &w_ompc, map_j_inn_ompc => self%map_j_inn_ompc, map_j_out_ompc => self%map_j_out_ompc,&
      & map_j_out_blend_ompc => self%map_j_out_blend_ompc, eta_recyc_blend_ompc => self%eta_recyc_blend_omp&
      &c, yplus_inflow_ompc => self%yplus_inflow_ompc, eta_inflow_ompc => self%eta_inflow_ompc,&
      & yplus_recyc_ompc => self%yplus_recyc_ompc, eta_recyc_ompc => self%eta_recyc_ompc,&
      & betarecyc => self%equation_base%betarecyc, i_recyc => self%equation_base%i_recyc,&
      & glund1 => self%equation_base%glund1, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%e&
      &quation_base%indx_cp_r, rgas0 => self%equation_base%rgas0, calorically_perfect => self%equation_base&
      &%calorically_perfect, rand_type => self%equation_base%rand_type, t0 => self%equation_base%t0,&
      & u0 => self%equation_base%u0, l0 => self%equation_base%l0, cv_coeff_ompc => self%cv_coeff_ompc,&
      & cp_coeff_ompc => self%cp_coeff_ompc, inflow_random_plane => self%equation_base%inflow_random_plane,&
      & inflow_random_plane_ompc => self%inflow_random_plane_ompc)
        call bcrecyc_subroutine_1(nx, ny, nz, ng, nv, wrecycav_ompc, wrecyc_ompc)

        inflow_random_plane_ompc = inflow_random_plane

        ntot = ng*ny*nv
        call mpi_allreduce(mpi_in_place,wrecycav_ompc,ntot,mpi_prec,mpi_sum,mp_cartz,iermpi)

        call bcrecyc_subroutine_2(nx, ny, nz, nzmax, ng, wrecycav_ompc, wrecyc_ompc)

        call bcrecyc_subroutine_3(nx, ny, nz, ng, p0, u0, rgas0, w_ompc, wmean_ompc, wrecyc_ompc,&
        ...
        & inflow_random_plane_ompc, indx_cp_l, indx_cp_r, cv_coeff_ompc, cp_coeff_ompc, t0,&
        & calorically_perfect,rand_type)
      endassociate
    endif
  ...
  subroutine initialize(self, filename)
    ...
    call self%base_ompc%initialize(self%equation_base%field, self%equation_base%gpu_bind)

    if(self%equation_base%debug_memory>0) then
      call self%field%check_cpu_mem(description="--initialize-start--")
    endif

    call self%base_ompc%copy_from_field()
    ...
    self%w_aux_ompc = self%equation_base%w_aux
    self%fluid_mask_ompc = self%equation_base%fluid_mask
    self%ep_ord_change_ompc = self%equation_base%ep_ord_change
    self%winf_ompc = self%equation_base%winf
    self%winf_past_shock_ompc = self%equation_base%winf_past_shock
    if (self%grid%grid_dim == 2) self%wfar_ompc = self%equation_base%wfar

    self%lmax_tag_ompc = self%equation_base%lmax_tag
    self%vis_tag_ompc = self%equation_base%vis_tag

    self%cp_coeff_ompc = self%equation_base%cp_coeff
    self%cv_coeff_ompc = self%equation_base%cv_coeff

    if (self%equation_base%num_probe>0) then
      self%probe_coeff_ompc = self%equation_base%probe_coeff
      self%ijk_probe_ompc = self%equation_base%ijk_probe
    endif

    if(self%equation_base%flow_init /= 5) then
      self%wmean_ompc = self%equation_base%wmean
    endif

    if(self%equation_base%enable_tspec > 0) then
      self%w_tspec_ompc = self%equation_base%w_tspec
    endif

    allocate(self%coeff_deriv1_ompc(1:4,4))
    allocate(self%coeff_deriv2_ompc(0:4,4))
    self%coeff_deriv1_ompc = self%equation_base%coeff_deriv1
    self%coeff_deriv2_ompc = self%equation_base%coeff_deriv2

    if (self%equation_base%ifilter > 0) then
      self%coeff_filter_ompc = self%equation_base%coeff_filter
    endif

    if (self%equation_base%enable_sponge > 0) then
      self%f_sponge_ompc = self%equation_base%f_sponge
    endif

    if (self%equation_base%recyc) then
      self%yplus_inflow_ompc = self%equation_base%yplus_inflow
      self%eta_inflow_ompc = self%equation_base%eta_inflow
      self%yplus_recyc_ompc = self%equation_base%yplus_recyc
      self%eta_recyc_ompc = self%equation_base%eta_recyc
      self%eta_recyc_blend_ompc = self%equation_base%eta_recyc_blend
      self%map_j_inn_ompc = self%equation_base%map_j_inn
      self%map_j_out_ompc = self%equation_base%map_j_out
      self%map_j_out_blend_ompc = self%equation_base%map_j_out_blend
      self%weta_inflow_ompc = self%equation_base%weta_inflow
    endif


    if (self%equation_base%enable_insitu > 0) then
      call self%insitu_alloc_ompc()
    endif
    if (self%equation_base%debug_memory>0) then
  ...
  subroutine alloc(self)
    ...
    &on_base%indx_cp_r, ndft => self%equation_base%ndft_tspec, grid_dim => self%grid%grid_dim)
      ...
      call self%point_to_equation_base(self%equation_base)

      self%w_aux_ompc = self%equation_base%w_aux
      allocate(self%winf_ompc(nv))
      allocate(self%winf_past_shock_ompc(nv))
      if (grid_dim == 2) allocate (self%wfar_ompc(nx,nv))


      allocate(self%fl_ompc(1:nx, 1:ny, 1:nz, nv))
      allocate(self%fln_ompc(1:nx, 1:ny, 1:nz, nv))
      allocate(self%wallprop_ompc(1-ng:nx+ng, 1-ng:nz+ng, 2:4))
      allocate(self%fhat_ompc(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng, nv))

      allocate(self%wrecyc_ompc(ng,ny,nz,nv))
      allocate(self%wrecycav_ompc(ng,ny,nv))
      allocate(self%wmean_ompc(1-self%grid%ng:self%field%nx+self%grid%ng+1, 1:self%field%ny,4))

      allocate(self%yplus_inflow_ompc(1-ng:ny+ng))
      allocate(self%eta_inflow_ompc(1-ng:ny+ng))
      allocate(self%yplus_recyc_ompc(1-ng:ny+ng))
      allocate(self%eta_recyc_ompc(1-ng:ny+ng))
      allocate(self%eta_recyc_blend_ompc(1-ng:ny+ng))
      allocate(self%map_j_inn_ompc(1:ny))
      allocate(self%map_j_out_ompc(1:ny))
      allocate(self%map_j_out_blend_ompc(1:ny))
      allocate(self%weta_inflow_ompc(1:ny))
      allocate(self%inflow_random_plane_ompc(1:ny,1:nz,3))

      allocate(self%cv_coeff_ompc(indx_cp_l:indx_cp_r+1))
      allocate(self%cp_coeff_ompc(indx_cp_l:indx_cp_r+1))

      if (self%equation_base%num_probe>0) then
        allocate(self%w_aux_probe_ompc(6,self%equation_base%num_probe))
        allocate(self%ijk_probe_ompc(3,self%equation_base%num_probe))
        allocate(self%probe_coeff_ompc(2,2,2,self%equation_base%num_probe))
      endif

      allocate(self%lmax_tag_ompc(1-ng:nx+ng))
      allocate(self%vis_tag_ompc(1-ng:nx+ng))

      if (self%equation_base%enable_tspec > 0) then
        allocate(self%w_psd_tspec_ompc(nx, ndft))
        allocate(self%w_tspec_ompc(nx, nz, ndft, 2, 2))
      endif

      if (self%equation_base%ifilter > 0) allocate(self%coeff_filter_ompc(0:4))
      if (self%equation_base%enable_sponge > 0) allocate(self%f_sponge_ompc(ny))
  ...
  subroutine compute_residual(self)
    ...
    real(rkind) :: restemp
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & dt => self%equation_base%dt, fln_ompc => self%fln_ompc, residual_rhou => self%equation_base%residua&
    &l_rhou, fluid_mask_ompc => self%fluid_mask_ompc, vmax => self%equation_base%vmax,&
    & w_aux_ompc => self%w_aux_ompc, rgas0 => self%equation_base%rgas0, rhomin => self%equation_base%rhom&
    &in, rhomax => self%equation_base%rhomax, pmin => self%equation_base%pmin, pmax => self%equation_base&
    &%pmax, tmin => self%equation_base%tmin, tmax => self%equation_base%tmax)

      call compute_residual_subroutine(nx, ny, nz, ng, nv, fln_ompc, dt, residual_rhou, fluid_mask_ompc)
      ...
      call compute_rho_t_p_minmax_subroutine(nx, ny, nz, ng, rgas0, w_aux_ompc,rhomin,rhomax, tmin,&
      & tmax, pmin, pmax, fluid_mask_ompc)
  ...
  subroutine count_weno(self)
    ...
    &ion_base%weno_scheme, sensor_threshold => self%equation_base%sensor_threshold, w_aux_ompc => self%w_&
    &aux_ompc, ep_ord_change_ompc => self%ep_ord_change_ompc, lmax_tag_ompc => self%lmax_tag_ompc,&
    & wall_tag_ompc => self%base_ompc%wall_tag_ompc, count_weno_x => self%equation_base%count_weno_x,&
    & count_weno_y => self%equation_base%count_weno_y, count_weno_z => self%equation_base%count_weno_z)

      if (grid_dim==1) then
        call count_weno_subroutine(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin,&
        & eul_jmax, eul_kmin, eul_kmax, weno_scheme, sensor_threshold, w_aux_ompc, ep_ord_change_ompc,&
        & count_weno_x, count_weno_y, count_weno_z)
      else
        lmax = ep_order/2
        call count_weno_c2_subroutine(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin,&
        & eul_jmax, eul_kmin, eul_kmax, lmax, weno_scheme, sensor_threshold, w_aux_ompc, ep_ord_change_ompc ,&
        & lmax_tag_ompc, wall_tag_ompc, count_weno_x, count_weno_y, count_weno_z)
      endif
  ...
  subroutine compute_airfoil_forces_runtime(self)
    ...
    &0, u0 => self%equation_base%u0, rgas0 => self%equation_base%rgas0, w_aux_ompc => self%w_aux_ompc,&
    & wall_tag_ompc => self%base_ompc%wall_tag_ompc, meta_ompc => self%base_ompc%meta_ompc,&
    & csimod_ompc => self%base_ompc%csimod_ompc, dxdcsic2_ompc => self%base_ompc%dxdcsic2_ompc,&
    & dydcsic2_ompc => self%base_ompc%dydcsic2_ompc, lift_af => self%equation_base%lift_af,&
    & drag_af => self%equation_base%drag_af, pres_af => self%equation_base%pres_af, fric_af => self%equat&
    &ion_base%fric_af)

      call compute_airfoil_forces_runtime_subroutine(nx, ny, nz, ng, nv, p0, u0, rgas0, w_aux_ompc,&
  ...
  subroutine compute_dt(self)
    ...
    & sutherland_s => self%sutherland_s, w_ompc => self%base_ompc%w_ompc, w_aux_ompc => self%w_aux_ompc,&
    & dcsidx_ompc => self%base_ompc%dcsidx_ompc, detady_ompc => self%base_ompc%detady_ompc,&
    & dzitdz_ompc => self%base_ompc%dzitdz_ompc, dcsidxs_ompc => self%base_ompc%dcsidxs_ompc,&
    & detadys_ompc => self%base_ompc%detadys_ompc, dzitdzs_ompc => self%base_ompc%dzitdzs_ompc,&
    & dcsidxnc2_ompc => self%base_ompc%dcsidxnc2_ompc, dcsidync2_ompc => self%base_ompc%dcsidync2_ompc,&
    & detadxnc2_ompc => self%base_ompc%detadxnc2_ompc, detadync2_ompc => self%base_ompc%detadync2_ompc,&
    & mcsi_ompc => self%base_ompc%mcsi_ompc, meta_ompc => self%base_ompc%meta_ompc, calorically_perfect =&
    &> self%equation_base%calorically_perfect, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, cp_coeff_ompc => self%cp_coeff_ompc, fluid_mask_ompc => &
    &self%fluid_mask_ompc, rgas0 => self%equation_base%rgas0, t0 => self%equation_base%t0,&
    & ndim => self%grid%ndim)
      if (cfl < 0) then
        dt = -cfl
      else
        if(self%grid%grid_dim == 1) then
          call compute_dt_subroutine(nx, ny, nz, ng, rgas0, prandtl, dcsidx_ompc, detady_ompc,&
          & dzitdz_ompc, dcsidxs_ompc, detadys_ompc, dzitdzs_ompc, w_ompc, w_aux_ompc, dtxi_max, dtyi_max,&
          & dtzi_max, dtxv_max, dtyv_max, dtzv_max, dtxk_max, dtyk_max, dtzk_max, indx_cp_l, indx_cp_r,&
          & cp_coeff_ompc,fluid_mask_ompc,calorically_perfect,t0)
        elseif(self%grid%grid_dim == 2) then
          call compute_dt_c2_subroutine(nx, ny, nz, ng, rgas0, prandtl, dcsidxnc2_ompc,&
          & dcsidync2_ompc, detadxnc2_ompc, detadync2_ompc, mcsi_ompc, meta_ompc, dzitdz_ompc, dzitdzs_ompc,&
          & w_ompc, w_aux_ompc, dtxi_max, dtyi_max, dtzi_max, dtxv_max, dtyv_max, dtzv_max, dtxk_max, dtyk_max,&
          & dtzk_max, indx_cp_l, indx_cp_r, cp_coeff_ompc,fluid_mask_ompc,calorically_perfect,t0)
        endif
        if(ndim == 2) then
  ...
  subroutine run(self, filename)
    ...
    & powerlaw_vtexp => self%powerlaw_vtexp, cv_coeff_ompc => self%cv_coeff_ompc, calorically_perfect => &
    &self%equation_base%calorically_perfect, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, mode_async => self%equation_base%mode_async,&
    & flow_init => self%equation_base%flow_init, enable_forces_runtime => self%equation_base%enable_force&
    &s_runtime, count_weno_control => self%equation_base%count_weno_control, time_from_last_rst => self%e&
    &quation_base%time_from_last_rst, time_from_last_write => self%equation_base%time_from_last_write,&
    ...
    & time_from_last_slice_p3d => self%equation_base%time_from_last_slice_p3d, time_from_last_insitu => s&
    &elf%equation_base%time_from_last_insitu, time_from_last_tspec => self%equation_base%time_from_last_t&
    &spec, time_is_freezed => self%equation_base%time_is_freezed)
      ...
      call zero_flux_subroutine(self%nx,self%ny,self%nz,self%nv,self%fl_ompc)
      ...
      if (self%equation_base%restart_type==0) then
        self%equation_base%w_aux = self%w_aux_ompc
        if (self%equation_base%enable_plot3d>0) then
        ...
        if ( time_is_freezed ) then
          ...
          if (mod(icyc-icyc0, self%equation_base%print_control)==0) then
            ...
            if (ieee_is_nan(self%equation_base%residual_rhou)) then
              ...
              call self%base_ompc%copy_to_field()
              if (self%equation_base%enable_plot3d>0) then
      ...
      if (self%equation_base%io_type_w==1) then
        ...
        if (self%equation_base%enable_tspec>0) then
          self%equation_base%w_tspec = self%w_tspec_ompc
  ...
  subroutine insitu_coprocess(self)
    ...
    integer :: l,i,j,k,ll
    call self%update_ghost()
    call self%base_ompc%bcswap_edges_corners()
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, visc_model => self%visc_mo&
    &del, mu0 => self%mu0, t_ref_dim => self%t_ref_dim, sutherland_s => self%sutherland_s,&
    & powerlaw_vtexp => self%powerlaw_vtexp, w_aux_ompc => self%w_aux_ompc, cv_coeff_ompc => self%cv_coef&
    &f_ompc, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r,&
    ...
    if (self%equation_base%npsi > 0) then
      ...
      do l=1,self%equation_base%npsi
        call self%base_ompc%bcswap_var(self%psi_ompc(:,:,:,l:l ))
      enddo
      do l=1,self%equation_base%npsi
        call self%base_ompc%bcswap_edges_corners_var(self%psi_ompc(:,:,:,l:l ))
      enddo
    endif
    associate( nxsl_ins => self%equation_base%nxsl_ins, nxel_ins => self%equation_base%nxel_ins,&
    & nysl_ins => self%equation_base%nysl_ins, nyel_ins => self%equation_base%nyel_ins,&
    ...
    &e%nv_aux, n_aux_list => self%equation_base%n_aux_list, psi_ompc => self%psi_ompc,&
    & n_add_list => self%equation_base%n_add_list, w_aux_ompc => self%w_aux_ompc, aux_list_ompc => self%a&
    &ux_list_ompc, add_list_ompc => self%add_list_ompc, nx => self%field%nx, ny => self%field%ny,&
    & nz => self%field%nz, icyc => self%equation_base%icyc, time => self%equation_base%time,&
    & i_insitu => self%equation_base%i_insitu, flag => self%equation_base%insitu_flag,&
    & nrank => self%myrank, ng => self%grid%ng, aux_list_name => self%equation_base%aux_list_name,&
    & add_list_name => self%equation_base%add_list_name, time_insitu => self%equation_base%time_insitu)

      self%equation_base%w_aux = w_aux_ompc
      self%equation_base%psi = psi_ompc
  ...
  subroutine insitu_compute_psi(self)
    ...
    & sutherland_s => self%sutherland_s, powerlaw_vtexp => self%powerlaw_vtexp, coeff_deriv1_ompc => self&
    &%coeff_deriv1_ompc, coeff_deriv2_ompc => self%coeff_deriv2_ompc, fhat_trans_ompc => self%fhat_trans_&
    &ompc, fl_trans_ompc => self%fl_trans_ompc, fl_ompc => self%fl_ompc, w_aux_ompc => self%w_aux_ompc,&
    & w_aux_trans_ompc => self%w_aux_trans_ompc, dcsidx_ompc => self%base_ompc%dcsidx_ompc,&
    & detady_ompc => self%base_ompc%detady_ompc, dzitdz_ompc => self%base_ompc%dzitdz_ompc,&
    & dcsidxs_ompc => self%base_ompc%dcsidxs_ompc, detadys_ompc => self%base_ompc%detadys_ompc,&
    & dzitdzs_ompc => self%base_ompc%dzitdzs_ompc, dcsidx2_ompc => self%base_ompc%dcsidx2_ompc,&
    & detady2_ompc => self%base_ompc%detady2_ompc, dzitdz2_ompc => self%base_ompc%dzitdz2_ompc,&
    & dcsidxc2_ompc => self%base_ompc%dcsidxc2_ompc, detadyc2_ompc => self%base_ompc%detadyc2_ompc,&
    & detadxc2_ompc => self%base_ompc%detadxc2_ompc, dcsidyc2_ompc => self%base_ompc%dcsidyc2_ompc,&
    & vis_tag_ompc => self%vis_tag_ompc, wall_tag_ompc => self%base_ompc%wall_tag_ompc,&
    & eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_base%eul_imax,&
    & eul_jmin => self%equation_base%eul_jmin, eul_jmax => self%equation_base%eul_jmax,&
    & eul_kmin => self%equation_base%eul_kmin, eul_kmax => self%equation_base%eul_kmax,&
    & cv_coeff_ompc => self%cv_coeff_ompc, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%e&
    &quation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_perfect,&
    & npsi => self%equation_base%npsi, psi_ompc => self%psi_ompc, add_list => self%equation_base%add_list&
    &, x_ompc => self%base_ompc%x_ompc )
      do l=1,npsi
        if (add_list(l) == 1) then
          call insitu_div_subroutine(nx, ny, nz, ng, visc_order, npsi,l, w_aux_ompc,&
          & coeff_deriv1_ompc, dcsidx_ompc, detady_ompc, dzitdz_ompc, psi_ompc )
        endif
        if (add_list(l) == 2) then
          call insitu_omega_subroutine(nx, ny, nz, ng, visc_order, npsi,l, w_aux_ompc,&
          & coeff_deriv1_ompc, dcsidx_ompc, detady_ompc, dzitdz_ompc, psi_ompc )
        endif
        if (add_list(l) == 3) then
          call insitu_ducros_subroutine(nx, ny, nz, ng, visc_order, npsi,l, u0,l0,w_aux_ompc,&
          & coeff_deriv1_ompc, dcsidx_ompc, detady_ompc, dzitdz_ompc, psi_ompc )
        endif
        if (add_list(l) == 4) then
          ...
          if(self%grid%grid_dim == 1) then
            call insitu_swirling_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dcsidx_ompc,&
            & detady_ompc, dzitdz_ompc, w_aux_ompc, coeff_deriv1_ompc, psi_ompc, u0, x_ompc )
          elseif(self%grid%grid_dim == 2) then
            call insitu_swirling_c2_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dzitdz_ompc,&
            & w_aux_ompc, coeff_deriv1_ompc, dcsidxc2_ompc, detadyc2_ompc, detadxc2_ompc, dcsidyc2_ompc,&
            & psi_ompc, u0, x_ompc, vis_tag_ompc, wall_tag_ompc)
          endif
        endif
        if (add_list(l) == 5) then
          ...
          if(self%grid%grid_dim == 1) then
            call insitu_schlieren_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dcsidx_ompc,&
            & detady_ompc, dzitdz_ompc, w_aux_ompc, coeff_deriv1_ompc, psi_ompc, u0, x_ompc )
          elseif(self%grid%grid_dim == 2) then
            call insitu_schlieren_c2_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dzitdz_ompc,&
            & w_aux_ompc, coeff_deriv1_ompc, dcsidxc2_ompc, detadyc2_ompc, detadxc2_ompc, dcsidyc2_ompc,&
            & psi_ompc, u0, x_ompc, vis_tag_ompc, wall_tag_ompc)
          endif
        endif
  ...
  subroutine manage_output(self)
    ...
    &icyc, time => self%equation_base%time, w_aux_ompc => self%w_aux_ompc, grid_dim => self%grid%grid_dim&
    &, ijk_probe_ompc => self%ijk_probe_ompc, w_aux_probe_ompc => self%w_aux_probe_ompc,&
    & probe_coeff_ompc => self%probe_coeff_ompc, w_aux_probe => self%equation_base%w_aux_probe,&
    & wallprop => self%equation_base%wallprop, wallprop_ompc => self%wallprop_ompc, nx => self%nx,&
    & ny => self%ny, nz => self%nz, ng => self%ng, nv_aux => self%nv_aux, nzmax => self%grid%nzmax,&
    & mp_cartz => self%field%mp_cartz, mpi_err => self%mpi_err, time_insitu => self%equation_base%time_in&
    &situ, ndft => self%equation_base%ndft_tspec, jg_tspec => self%equation_base%jg_tspec,&
    & dt_tspec => self%equation_base%dt_tspec, it_win1_tspec => self%equation_base%it_win1_tspec,&
    & it_win2_tspec => self%equation_base%it_win2_tspec, it_nwin_tspec => self%equation_base%it_nwin_tspe&
    &c, w_tspec_ompc => self%w_tspec_ompc, w_psd_tspec_ompc => self%w_psd_tspec_ompc, w_psd_tspec => self&
    &%equation_base%w_psd_tspec, w_psd_avg_tspec => self%equation_base%w_psd_avg_tspec,&
    & wss => self%equation_base%wss_tspec, wall_tag_ompc => self%base_ompc%wall_tag_ompc,&
    & meta_ompc => self%base_ompc%meta_ompc, csimod_ompc => self%base_ompc%csimod_ompc,&
    & dxdcsic2_ompc => self%base_ompc%dxdcsic2_ompc, dydcsic2_ompc => self%base_ompc%dydcsic2_ompc )
      ...
      if (time_from_last_stat >= self%equation_base%dtstat) then
        ...
        self%equation_base%w_aux(:,:,:,6) = self%w_aux_ompc(:,:,:,6)
        self%equation_base%w_aux(:,:,:,7) = self%w_aux_ompc(:,:,:,7)
      ...
      if (time_from_last_slice >= self%equation_base%dtslice) then
        ...
        if (allocated(self%equation_base%islice)) then
          ...
          do i=1,size(self%equation_base%islice)
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              sliceyz_aux(i,:,:,m) = self%w_aux_ompc(ii,:,:,mm)
            enddo
          enddo
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              if(jj == 1 .and. mm >= 2 .and. mm <= 4) then
                if (grid_dim == 2 .and. mm == 2) then
                  call compute_wallprop_c2_subroutine(nx,ny,nz,ng,w_aux_ompc,wallprop_ompc,&
                  &dxdcsic2_ompc,dydcsic2_ompc,csimod_ompc,meta_ompc,wall_tag_ompc)
                endif
                slicexz_aux(:,j,:,m) = wallprop_ompc(:,:,mm)
              else
                slicexz_aux(:,j,:,m) = self%w_aux_ompc(:,jj,:,mm)
              endif
            enddo
          enddo
        ...
        if (allocated(self%equation_base%kslice)) then
          ...
          do k=1,size(self%equation_base%kslice)
            ...
            do m=1,self%equation_base%num_aux_slice
              mm = self%equation_base%list_aux_slice(m)
              slicexy_aux(:,:,k,m) = self%w_aux_ompc(:,:,kk,mm)
            enddo
          enddo
      ...
      if (time_from_last_probe >= self%equation_base%dtprobe) then
        ...
        if (self%equation_base%num_probe>0) then
          ...
          &ijk_probe_ompc,w_aux_probe_ompc,w_aux_ompc,probe_coeff_ompc)
          w_aux_probe = w_aux_probe_ompc
        ...
        if(self%equation_base%enable_slice_plot3d > 0) then
          ...
          if (allocated(self%equation_base%islice_p3d)) then
            do i=1,size(self%equation_base%islice_p3d)
              ...
              do m=1,self%equation_base%num_aux_slice
                mm = self%equation_base%list_aux_slice(m)
                self%equation_base%w_aux(ii,:,:,mm) = self%w_aux_ompc(ii,:,:,mm)
              enddo
            enddo
          endif
          if (allocated(self%equation_base%jslice_p3d)) then
            do j=1,size(self%equation_base%jslice_p3d)
              ...
              do m=1,self%equation_base%num_aux_slice
                mm = self%equation_base%list_aux_slice(m)
                self%equation_base%w_aux(:,jj,:,mm) = self%w_aux_ompc(:,jj,:,mm)
              enddo
              if(jj == 1) then
                if (grid_dim == 2) then
                  call compute_wallprop_c2_subroutine(nx,ny,nz,ng,w_aux_ompc,wallprop_ompc,&
          ...
          if (allocated(self%equation_base%kslice_p3d)) then
            do k=1,size(self%equation_base%kslice_p3d)
              ...
              do m=1,self%equation_base%num_aux_slice
                mm = self%equation_base%list_aux_slice(m)
                self%equation_base%w_aux(:,:,kk,mm) = self%w_aux_ompc(:,:,kk,mm)
              enddo
            enddo
          endif
          call self%equation_base%write_slice_p3d(plot3dgrid=.false.,plot3dfield=.true.)
      ...
      if (time_from_last_tspec >= self%equation_base%dt_tspec) then
        if(self%equation_base%enable_tspec > 0) then
          ...
          call compute_tspec_subroutine(nx, ny, nz, ng, ndft, jg_tspec, 1, it_win1_tspec, w_aux_ompc, w_tspec_ompc)
          if(self%equation_base%it_win2_tspec > 0) then
            call compute_tspec_subroutine(nx, ny, nz, ng, ndft, jg_tspec, 2, it_win2_tspec, w_aux_ompc, w_tspec_ompc)
          endif
          if(it_win1_tspec == ndft .or. it_win2_tspec == ndft) then
            if(it_win1_tspec == ndft) then
              call compute_psd_tspec_subroutine(nx, ny, nz, ndft, 1, dt_tspec, w_tspec_ompc, w_psd_tspec_ompc)
            endif
            if(it_win2_tspec == ndft) then
              call compute_psd_tspec_subroutine(nx, ny, nz, ndft, 2, dt_tspec, w_tspec_ompc, w_psd_tspec_ompc)
            endif

            w_psd_tspec = w_psd_tspec_ompc
...
endmodule streams_equation_singleideal_ompc_object
