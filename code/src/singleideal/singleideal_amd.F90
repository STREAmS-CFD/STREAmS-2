module streams_equation_singleideal_amd_object

  use streams_base_amd_object
  use streams_field_object
  use streams_grid_object
  use streams_kernels_amd
  ...
  implicit none
  private
  public :: equation_singleideal_amd_object
  ...
  type :: equation_singleideal_amd_object
    type(base_amd_object) :: base_amd
    ...
    integer(ikind) :: error
    real(rkind) :: time0
    real(rkind), dimension(:,:), pointer :: coeff_deriv1_gpu
    real(rkind), dimension(:,:), pointer :: coeff_deriv2_gpu
    real(rkind), dimension(:) , pointer :: coeff_filter_gpu
    ...
    real(rkind), dimension(:,:,:,:), pointer :: w_aux_gpu
    integer, dimension(:,:,:), pointer :: fluid_mask_gpu
    integer, dimension(:,:,:,:), pointer :: ep_ord_change_gpu

    real(rkind), dimension(:), pointer :: winf_gpu, winf_past_shock_gpu
    real(rkind), allocatable, dimension(:,:,:,:) :: w_aux_debug
    real(rkind), allocatable, dimension(:,:,:,:) :: fl_debug
    real(rkind), dimension(:,:,:,:), pointer :: fhat_gpu
    real(rkind), dimension(:,:,:,:), pointer :: w_aux_trans_gpu, fl_trans_gpu, fhat_trans_gpu
    real(rkind), dimension(:,:,:,:), pointer :: fl_gpu, fln_gpu
    real(rkind), dimension(:,:), pointer :: dcoe_gpu

    real(rkind), dimension(:,:,:,:), pointer :: wrecyc_gpu
    real(rkind), dimension(:,:,:), pointer :: wrecycav_gpu
    real(rkind), dimension(:,:,:), pointer :: wmean_gpu
    real(rkind), dimension(:,:,:), pointer :: redn_3d_gpu
    real(rkind), dimension(:,:,:), pointer :: inflow_random_plane_gpu
    real(rkind), dimension(:), pointer :: weta_inflow_gpu
    real(rkind), dimension(:), pointer :: yplus_inflow_gpu, eta_inflow_gpu
    real(rkind), dimension(:), pointer :: yplus_recyc_gpu, eta_recyc_gpu, eta_recyc_blend_gpu
    integer, dimension(:), pointer :: map_j_inn_gpu, map_j_out_gpu, map_j_out_blend_gpu

    real(rkind), dimension(:), pointer :: cv_coeff_gpu, cp_coeff_gpu

    real(rkind), dimension(:,:), pointer :: w_aux_probe_gpu
    real(rkind), dimension(:,:,:,:), pointer :: probe_coeff_gpu
    integer, dimension(:,:), pointer :: ijk_probe_gpu
    real(rkind), dimension(:,:,:), pointer :: wallprop_gpu

    integer, dimension(:), pointer :: lmax_tag_gpu
    integer, dimension(:), pointer :: vis_tag_gpu

    real(rkind), dimension(:,:,:,:,:), pointer :: w_tspec_gpu
    real(rkind), dimension(:,:), pointer :: w_psd_tspec_gpu
    real(rkind), dimension(:,:), pointer :: wfar_gpu
    real(rkind), dimension(:), pointer :: f_sponge_gpu


    !insitu_var_start
    real(rkind), dimension(:,:,:,:), pointer :: psi_gpu
    integer, dimension(:), pointer :: aux_list_gpu, add_list_gpu
    !insitu_var_end
  ...
  endtype equation_singleideal_amd_object

contains


  subroutine point_to_field(self, field)
    class(equation_singleideal_amd_object), intent(inout) :: self
  ...
  subroutine insitu_alloc_gpu(self)
    ...
    &x_list, n_add_list => self%equation_base%n_add_list )
      if (npsi > 0) call hipCheck(hipMalloc(self%psi_gpu,(nx+ng)-(1-ng)+1,(ny+ng)-(1-ng)+1,(nz+ng)-(1-ng)+1,(npsi)-(1)+1))
      if (n_aux_list > 0) then
        call hipCheck(hipMalloc(self%aux_list_gpu,(n_aux_list)-(1)+1))
        call hipCheck(hipMemcpy(self%aux_list_gpu , self%equation_base%aux_list,hipMemcpyHostToDevice))
      endif
      if (n_add_list > 0) then
        call hipCheck(hipMalloc(self%add_list_gpu,(n_add_list)-(1)+1))
        call hipCheck(hipMemcpy(self%add_list_gpu , self%equation_base%add_list,hipMemcpyHostToDevice))
      endif
    endassociate
  ...
  subroutine visflx(self, mode)
    ...
    & w_aux_gpu => self%w_aux_gpu, w_aux_trans_gpu => self%w_aux_trans_gpu, dcsidx_gpu => self%base_amd%d&
    &csidx_gpu, detady_gpu => self%base_amd%detady_gpu, dzitdz_gpu => self%base_amd%dzitdz_gpu,&
    & dcsidxs_gpu => self%base_amd%dcsidxs_gpu, detadys_gpu => self%base_amd%detadys_gpu,&
    & dzitdzs_gpu => self%base_amd%dzitdzs_gpu, dcsidx2_gpu => self%base_amd%dcsidx2_gpu,&
    & detady2_gpu => self%base_amd%detady2_gpu, dzitdz2_gpu => self%base_amd%dzitdz2_gpu,&
    & x_gpu => self%base_amd%x_gpu, y_gpu => self%base_amd%y_gpu, z_gpu => self%base_amd%z_gpu,&
    ...
    & grid_dim => self%grid%grid_dim, dcsidxc2_gpu => self%base_amd%dcsidxc2_gpu, dcsidyc2_gpu => self%ba&
    &se_amd%dcsidyc2_gpu, detadxc2_gpu => self%base_amd%detadxc2_gpu, detadyc2_gpu => self%base_amd%detad&
    &yc2_gpu, g1_gpu => self%base_amd%g1_gpu, g2_gpu => self%base_amd%g2_gpu, g12_gpu => self%base_amd%g1&
    &2_gpu, jac_gpu => self%base_amd%jac_gpu, vis_tag_gpu => self%vis_tag_gpu, wall_tag_gpu => self%base_&
    &amd%wall_tag_gpu, iblock => self%field%ncoords(1), ite_rank_x => self%field%ite_rank_x,&
    ...
    &weno, theta_ij_gpu => self%base_amd%theta_ij_gpu, theta_threshold => self%equation_base%theta_thresh&
    &old)

      if (mode == 0) then
        if (grid_dim == 1) then
          call visflx_kernel(nx,ny,nz,ng,visc_order,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
          &calorically_perfect,u0,l0,self%base_amd%w_gpu,self%w_aux_gpu,self%fl_gpu,coeff_deriv1_gpu,&
          &coeff_deriv2_gpu,dcsidx_gpu,detady_gpu,dzitdz_gpu,dcsidxs_gpu,detadys_gpu,dzitdzs_gpu,dcsidx2_gpu,&
          &detady2_gpu,dzitdz2_gpu,self%wallprop_gpu)
        elseif (grid_dim == 2) then
          call visflx_c2_kernel(nx,ny,nz,ng,visc_order,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
          &calorically_perfect,u0,l0,self%base_amd%w_gpu,self%w_aux_gpu,self%fl_gpu,coeff_deriv1_gpu,&
          &coeff_deriv2_gpu,dcsidxc2_gpu,detadyc2_gpu,detadxc2_gpu,dcsidyc2_gpu,dzitdz_gpu,dzitdzs_gpu,&
          &dzitdz2_gpu,g1_gpu,g2_gpu,g12_gpu,jac_gpu,self%wallprop_gpu,vis_tag_gpu,wall_tag_gpu,iblock,&
          &ite_rank_x,itu_rank_x,ite_l,itu_l,teshk)
        endif
      elseif (mode == 1) then
        if (grid_dim == 1) then
          call visflx_div_kernel(nx,ny,nz,ng,visc_order,self%w_aux_gpu,self%fl_gpu,coeff_deriv1_gpu,&
          &dcsidx_gpu,detady_gpu,dzitdz_gpu,stream1)
        elseif (grid_dim == 2) then
          call visflx_div_c2_kernel(nx,ny,nz,ng,visc_order,self%w_aux_gpu,self%fl_gpu,&
          &coeff_deriv1_gpu,dcsidxc2_gpu,dcsidyc2_gpu,detadxc2_gpu,detadyc2_gpu,dzitdz_gpu,vis_tag_gpu,&
          &wall_tag_gpu,stream1)
        endif
      elseif (mode == 2) then
        call visflx_reduced_ord2_kernel(nx,ny,nz,ng,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
        &calorically_perfect,u0,l0,self%base_amd%w_gpu,self%w_aux_gpu,self%fl_gpu,x_gpu,y_gpu,z_gpu,&
        &self%wallprop_gpu,1)
      elseif (mode == 3) then
        if (grid_dim == 1) then
          call sensor_kernel(nx,ny,nz,ng,u0,l0,self%w_aux_gpu)
        elseif (grid_dim == 2) then
          call sensor_c2_kernel(nx,ny,nz,ng,u0,l0_ducros,self%w_aux_gpu,iblock,ite_rank_x,&
          &itu_rank_x,ite_l,itu_l,teshk,theta_ij_gpu,theta_threshold,jweno)
        endif
      elseif (mode == 4) then
        if (grid_dim == 1) then
          call visflx_nosensor_kernel(nx,ny,nz,ng,visc_order,prandtl,t0,indx_cp_l,indx_cp_r,&
          &cp_coeff_gpu,calorically_perfect,u0,l0,self%base_amd%w_gpu,self%w_aux_gpu,self%fl_gpu,&
        ...
        elseif (grid_dim == 2) then
          call visflx_nosensor_c2_kernel(nx,ny,nz,ng,visc_order,prandtl,t0,indx_cp_l,indx_cp_r,&
          &cp_coeff_gpu,calorically_perfect,iblock,ite_rank_x,itu_rank_x,ite_l,itu_l,u0,l0,self%base_amd%w_gpu,&
          &self%w_aux_gpu,self%fl_gpu,coeff_deriv1_gpu,coeff_deriv2_gpu,dcsidxc2_gpu,detadyc2_gpu,detadxc2_gpu,&
          &dcsidyc2_gpu,dzitdz_gpu,dzitdzs_gpu,dzitdz2_gpu,g1_gpu,g2_gpu,g12_gpu,jac_gpu,self%wallprop_gpu,&
          &vis_tag_gpu,wall_tag_gpu,ortho)
        endif
      elseif (mode == 5) then
        call visflx_x_kernel(nx,ny,nz,nv,ng,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
        &calorically_perfect,self%base_amd%x_gpu,w_aux_gpu,self%fl_gpu,self%fhat_gpu)
        call visflx_y_kernel(nx,ny,nz,nv,ng,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
        &calorically_perfect,self%base_amd%y_gpu,self%w_aux_gpu,self%fl_gpu)
        call visflx_z_kernel(nx,ny,nz,nv,ng,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
        &calorically_perfect,self%base_amd%z_gpu,self%w_aux_gpu,self%fl_gpu)
      elseif (mode == 6) then
        call visflx_reduced_ord2_kernel(nx,ny,nz,ng,prandtl,t0,indx_cp_l,indx_cp_r,cp_coeff_gpu,&
        &calorically_perfect,u0,l0,self%base_amd%w_gpu,self%w_aux_gpu,self%fl_gpu,x_gpu,y_gpu,z_gpu,&
        &self%wallprop_gpu,0)
      elseif (mode == 7) then
        call visflx_div_ord2_kernel(nx,ny,nz,ng,self%w_aux_gpu,self%fl_gpu,x_gpu,y_gpu,z_gpu,stream1)
      endif
    endassociate
  ...
  subroutine compute_aux(self, central, ghost)
    ...
    & rgas0 => self%equation_base%rgas0)
      if(central_ == 1 .and. ghost_ == 1) then
        call eval_aux_kernel(nx,ny,nz,ng,1-ng,nx+ng,1-ng,ny+ng,1-ng,nz+ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
      elseif(central_ == 1 .and. ghost_ == 0) then
        call eval_aux_kernel(nx,ny,nz,ng,1,nx,1,ny,1,nz,self%base_amd%w_gpu,self%w_aux_gpu,&
      ...
      elseif(central_ == 0 .and. ghost_ == 1) then
        call eval_aux_kernel(nx,ny,nz,ng,1-ng,0,1-ng,ny+ng,1-ng,nz+ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
        call eval_aux_kernel(nx,ny,nz,ng,nx+1,nx+ng,1-ng,ny+ng,1-ng,nz+ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
        call eval_aux_kernel(nx,ny,nz,ng,1-ng,nx+ng,1-ng,0,1-ng,nz+ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
        call eval_aux_kernel(nx,ny,nz,ng,1-ng,nx+ng,ny+1,ny+ng,1-ng,nz+ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
        call eval_aux_kernel(nx,ny,nz,ng,1-ng,nx+ng,1-ng,ny+ng,1-ng,0,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
        call eval_aux_kernel(nx,ny,nz,ng,1-ng,nx+ng,1-ng,ny+ng,nz+1,nz+ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,visc_model,mu0,t0,sutherland_s,t_ref_dim,powerlaw_vtexp,visc_power,visc_sutherland,&
        &visc_no,cv_coeff_gpu,indx_cp_l,indx_cp_r,rgas0,calorically_perfect,tol_iter_nr,stream1)
      endif
    endassociate
  ...
  subroutine rk_sync(self)
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        if(flow_init == 5) then
          call self%base_amd%bcswap_wake_var(self%w_aux_gpu(:,:,:,10:10))
          call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,10:10))
          call self%bcextr_var(self%w_aux_gpu(:,:,:,10:10), mode=2)
          call self%base_amd%bcte_var(self%w_aux_gpu(:,:,:,10:10))
        else
        ...
        if (istep == 3) then
          ...
          call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,8:8))
          if(flow_init == 5) then
            call self%base_amd%bcswap_wake_var(self%w_aux_gpu(:,:,:,8:8))
          endif
        endif
        if (conservative_viscous==1) then
        ...
        call update_field_kernel(nx,ny,nz,ng,nv,self%base_amd%w_gpu,self%fln_gpu,self%fluid_mask_gpu)
  ...
  subroutine bcextr_var(self, w_swap, mode)
    ...
    & irightz => self%field%irightz)
      if(mode == 1) then
        call bcextr_var_kernel(nx,ny,nz,ng,w_swap)
      elseif(mode == 2) then
        call bcextr_airfoil_var_kernel(nx,ny,nz,ng,ndim,self%base_amd%wall_tag_gpu,w_swap,ileftx,irightx,ileftz,irightz)
      endif
    endassociate
  ...
  subroutine rk_async(self)
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        call hipCheck(hipDeviceSynchronize())
        call self%base_amd%bcswap(steps=[.true.,.false.,.false.])
        call self%euler_x(lmax+1,nx-lmax,lmax,nx-lmax,do_update=.false.)
        call self%base_amd%bcswap(steps=[.false.,.true.,.true.])
        ...
        if(flow_init == 5) then
          call self%base_amd%bcswap_wake_var(self%w_aux_gpu(:,:,:,10:10))
          call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,10:10), steps=[.true.,.false.,.false.])
          call self%euler_y(eul_jmin,eul_jmax)
          call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,10:10), steps=[.false.,.true.,.true.])
          call self%bcextr_var(self%w_aux_gpu(:,:,:,10:10), mode=2)
          call self%base_amd%bcte_var(self%w_aux_gpu(:,:,:,10:10))
        else
          call self%bcextr_var(self%w_aux_gpu(:,:,:,10:10), mode=1)
        ...
        if (istep == 3) then
          ...
          if(flow_init == 5) then
            call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,8:8))
            call self%base_amd%bcswap_wake_var(self%w_aux_gpu(:,:,:,8:8), steps=[.true.,.false.,.false.])
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            else
              call self%visflx(mode=1)
            endif
            call self%base_amd%bcswap_wake_var(self%w_aux_gpu(:,:,:,8:8), steps=[.false.,.true.,.true.])
          else
            call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,8:8), steps=[.true.,.false.,.false.])
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            else
              call self%visflx(mode=1)
            endif
            call self%base_amd%bcswap_var(self%w_aux_gpu(:,:,:,8:8), steps=[.false.,.true.,.true.])
          endif
        else
          if (conservative_viscous==1) then
            call self%visflx(mode=7)
  ...
  subroutine sponge(self)
    class(equation_singleideal_amd_object), intent(inout) :: self
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_gpu=>self%base_amd%w_gpu&
    &, wfar_gpu=>self%wfar_gpu,fln_gpu=>self%fln_gpu,f_sponge_gpu=>self%f_sponge_gpu, j_sponge=>self%equa&
    &tion_base%j_sponge)

      call sponge_kernel(nx,ny,nz,ng,nv,w_gpu,wfar_gpu,fln_gpu,f_sponge_gpu,j_sponge)
    endassociate
  ...
  subroutine limiter(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,w_gpu=>self%base_amd%w_gpu,&
    ...
    &m_rescale)
      ...
      call limiter_kernel(nx,ny,nz,ng,w_gpu,w_aux_gpu,iblock,kblock,indx_cp_l,indx_cp_r,&
  ...
  subroutine filter(self)
    ...
    &se%jfilter, wall_tag_gpu => self%base_amd%wall_tag_gpu)


      call filter_kernel(nx,ny,nz,ng,w_gpu,w_aux_gpu,indx_cp_l,indx_cp_r,cv_coeff_gpu,&
  ...
  subroutine tripping(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_gpu=>self%base_amd%w_gpu&
    &,fl_gpu=>self%fl_gpu, xc2_gpu => self%base_amd%xc2_gpu, yc2_gpu => self%base_amd%yc2_gpu,&
    & z_gpu => self%base_amd%z_gpu, ncoords=>self%field%ncoords, rlz => self%grid%domain_size(3),&
    & wall_tag_gpu => self%base_amd%wall_tag_gpu, dxdetanc2_gpu => self%base_amd%dxdetanc2_gpu,&
    & dydetanc2_gpu => self%base_amd%dydetanc2_gpu, a_tr => self%equation_base%a_tr, u0 => self%equation_&
    ...
    &e%is_old)
      ...
      call tripping_pressure_kernel(nx,ny,nz,ng,nv,i_rank_start,pi,itr1,itr2,x0tr,y0tr,x0ts,y0ts,&
      &lamx,lamy,lamz,lamz1,phiz,phiz1,asl,bt,xc2_gpu,yc2_gpu,z_gpu,w_gpu,fl_gpu,wall_tag_gpu,&
      &dxdetanc2_gpu,dydetanc2_gpu)
      call tripping_suction_kernel(nx,ny,nz,ng,nv,i_rank_start,pi,its1,its2,x0tr,y0tr,x0ts,y0ts,&
  ...
  subroutine euler_x(self, istart, iend, istart_face, iend_face, do_update)
    ...
    &se%force_zero_flux, coeff_deriv1_gpu => self%coeff_deriv1_gpu, dcsidx_gpu => self%base_amd%dcsidx_gp&
    &u, fhat_gpu => self%fhat_gpu, w_gpu => self%base_amd%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    ...
    & dcsidxc2_gpu => self%base_amd%dcsidxc2_gpu, dcsidxnc2_gpu => self%base_amd%dcsidxnc2_gpu,&
    & dcsidyc2_gpu => self%base_amd%dcsidyc2_gpu, dcsidync2_gpu => self%base_amd%dcsidync2_gpu,&
    & jac_gpu => self%base_amd%jac_gpu, mcsijac1_gpu => self%base_amd%mcsijac1_gpu, lmax_tag_gpu => self%&
      ...
      if(iend - istart >= 0) then
        ...
        if (self%grid%grid_dim == 1) then
          if (flux_splitting==1) then
            ...
            call euler_x_fluxes_hybrid_rusanov_kernel_wrapper(stream1,nv,nx,ny,nz,ng,nv_aux,&
            &istart_face_,iend_face_,lmax,nkeep,indx_cp_l,indx_cp_r,calorically_perfect,weno_scheme,weno_size,&
            &weno_version,force_zero_flux_min,force_zero_flux_max,sensor_threshold,rgas0,tol_iter_nr,&
            &self%equation_base%rho0,self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),&
            &c_loc(w_aux_gpu),c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(coeff_deriv1_gpu),c_loc(dcsidx_gpu))
          else

            call euler_x_fluxes_hybrid_kernel_wrapper(stream1,nv,nx,ny,nz,ng,nv_aux,istart_face_,&
            &iend_face_,lmax,nkeep,indx_cp_l,indx_cp_r,calorically_perfect,weno_scheme,weno_size,weno_version,&
            &force_zero_flux_min,force_zero_flux_max,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
            &self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),c_loc(w_aux_gpu),&
            &c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(coeff_deriv1_gpu),c_loc(dcsidx_gpu))
          endif

          if(do_update_) then
            call euler_x_update_kernel(nx,ny,nz,ng,nv,eul_imin,eul_imax,fhat_gpu,fl_gpu,dcsidx_gpu,stream1)
          endif

        elseif (self%grid%grid_dim == 2) then
          call euler_x_fluxes_hybrid_c2_kernel_wrapper(stream1,nv,nx,ny,nz,ng,nv_aux,istart,iend,&
          &lmax,nkeep,indx_cp_l,indx_cp_r,calorically_perfect,weno_scheme,weno_size,weno_version,&
          &force_zero_flux_min,force_zero_flux_max,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
          &self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),c_loc(lmax_tag_gpu),&
          &c_loc(w_aux_gpu),c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(coeff_deriv1_gpu),c_loc(dcsidxc2_gpu),&
          &c_loc(dcsidxnc2_gpu),c_loc(dcsidyc2_gpu),c_loc(dcsidync2_gpu),c_loc(jac_gpu),c_loc(mcsijac1_gpu))
          call euler_x_update_c2_kernel(nx,ny,nz,ng,nv,istart,iend,fhat_gpu,fl_gpu,jac_gpu,stream1)
        endif
  ...
  subroutine euler_y(self, eul_jmin, eul_jmax)
    ...
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & nv_aux => self%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_ba&
    &se%force_zero_flux, coeff_deriv1_gpu => self%coeff_deriv1_gpu, detady_gpu => self%base_amd%detady_gp&
    ...
    & nkeep => self%equation_base%nkeep, wall_tag_gpu => self%base_amd%wall_tag_gpu, flux_splitting => se&
    &lf%equation_base%flux_splitting, detadxc2_gpu => self%base_amd%detadxc2_gpu, detadxnc2_gpu => self%b&
    &ase_amd%detadxnc2_gpu, detadyc2_gpu => self%base_amd%detadyc2_gpu, detadync2_gpu => self%base_amd%de&
    &tadync2_gpu, jac_gpu => self%base_amd%jac_gpu, metajac1_gpu => self%base_amd%metajac1_gpu,&
    & rgas0 => self%equation_base%rgas0)
      weno_size = 2*weno_scheme
      lmax = ep_order/2
      ...
      if (self%grid%grid_dim == 1) then
        if (flux_splitting==1) then
          call euler_y_hybrid_rusanov_kernel_wrapper(stream1,nv,nx,ny,nz,ng,nv_aux,eul_jmin,&
          &eul_jmax,lmax,nkeep,indx_cp_l,indx_cp_r,calorically_perfect,force_zero_flux_min,force_zero_flux_max,&
          &weno_scheme,weno_size,weno_version,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
          &self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),c_loc(w_aux_gpu),&
          &c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(fl_gpu),c_loc(coeff_deriv1_gpu),c_loc(detady_gpu))
        else
          call euler_y_hybrid_kernel_wrapper(stream1,nv,nx,ny,nz,ng,nv_aux,eul_jmin,eul_jmax,lmax,&
          &nkeep,indx_cp_l,indx_cp_r,calorically_perfect,force_zero_flux_min,force_zero_flux_max,weno_scheme,&
          &weno_size,weno_version,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
          &self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),c_loc(w_aux_gpu),&
          &c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(fl_gpu),c_loc(coeff_deriv1_gpu),c_loc(detady_gpu))
        endif
        call euler_y_update_kernel(nx,ny,nz,ng,nv,eul_jmin,eul_jmax,fhat_gpu,fl_gpu,detady_gpu,stream1)
      elseif (self%grid%grid_dim == 2) then
        call euler_y_hybrid_c2_kernel_wrapper(stream1,nv,nx,ny,nz,ng,nv_aux,eul_jmin,eul_jmax,lmax,&
        &nkeep,indx_cp_l,indx_cp_r,calorically_perfect,force_zero_flux_min,force_zero_flux_max,weno_scheme,&
        &weno_size,weno_version,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0,self%equation_base%t0,c_loc(wall_tag_gpu),c_loc(ep_ord_change_gpu),&
        &c_loc(w_aux_gpu),c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(fl_gpu),c_loc(coeff_deriv1_gpu),&
        &c_loc(detadxc2_gpu),c_loc(detadxnc2_gpu),c_loc(detadyc2_gpu),c_loc(detadync2_gpu),c_loc(jac_gpu),&
        &c_loc(metajac1_gpu))
        call euler_y_update_c2_kernel(nx,ny,nz,ng,nv,eul_jmin,eul_jmax,fhat_gpu,fl_gpu,jac_gpu,wall_tag_gpu,stream1)
      endif
    endassociate
  ...
  subroutine euler_z(self, eul_kmin, eul_kmax)
    ...
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & nv_aux => self%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_ba&
    &se%force_zero_flux, coeff_deriv1_gpu => self%coeff_deriv1_gpu, dzitdz_gpu => self%base_amd%dzitdz_gp&
    &u, fhat_gpu => self%fhat_gpu, w_aux_gpu => self%w_aux_gpu, fl_gpu => self%fl_gpu,&
    & sensor_threshold => self%equation_base%sensor_threshold, weno_scheme => self%equation_base%weno_sch&
    ...
    & inflow_random_plane_gpu => self%inflow_random_plane_gpu)
      ...
      if (flux_splitting==1) then
        call euler_z_hybrid_rusanov_kernel_wrapper(c_null_ptr,nv,nx,ny,nz,ng,nv_aux,eul_kmin,&
        &eul_kmax,lmax,nkeep,indx_cp_l,indx_cp_r,calorically_perfect,force_zero_flux_min,force_zero_flux_max,&
        &weno_scheme,weno_size,weno_version,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),c_loc(w_aux_gpu),&
        &c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(fl_gpu),c_loc(coeff_deriv1_gpu),c_loc(dzitdz_gpu))
        call hipCheck(hipDeviceSynchronize())
      else
        call euler_z_hybrid_kernel_wrapper(c_null_ptr,nv,nx,ny,nz,ng,nv_aux,eul_kmin,eul_kmax,lmax,&
        &nkeep,indx_cp_l,indx_cp_r,calorically_perfect,force_zero_flux_min,force_zero_flux_max,weno_scheme,&
        &weno_size,weno_version,sensor_threshold,rgas0,tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0,self%equation_base%t0,c_loc(ep_ord_change_gpu),c_loc(w_aux_gpu),&
        &c_loc(fhat_gpu),c_loc(cp_coeff_gpu),c_loc(fl_gpu),c_loc(coeff_deriv1_gpu),c_loc(dzitdz_gpu))
      ...
      call euler_z_update_kernel(nx,ny,nz,ng,nv,eul_kmin,eul_kmax,fhat_gpu,fl_gpu,dzitdz_gpu,c_null_ptr)
  ...
  subroutine force_rhs(self)
    ...
    & yn_gpu => self%base_amd%yn_gpu, w_gpu => self%base_amd%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    & fln_gpu => self%fln_gpu, volchan => self%equation_base%volchan, fluid_mask_gpu => self%fluid_mask_g&
    &pu, r_curv => self%grid%r_curv, volume => self%field%volume, dxdcsinc2_gpu => self%base_amd%dxdcsinc&
    &2_gpu, dydcsinc2_gpu => self%base_amd%dydcsinc2_gpu, dcsidxnc2_gpu => self%base_amd%dcsidxnc2_gpu,&
    & dcsidync2_gpu => self%base_amd%dcsidync2_gpu, jac_gpu => self%base_amd%jac_gpu)

      if(self%grid%grid_dim == 1) then
        call force_rhs_1_kernel(nx,ny,nz,ng,yn_gpu,fln_gpu,w_gpu,w_aux_gpu,bulk5,fluid_mask_gpu,self%redn_3d_gpu)
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_1_c2_kernel(nx,ny,nz,ng,yn_gpu,fln_gpu,w_gpu,w_aux_gpu,bulk5,fluid_mask_gpu,&
        &dcsidxnc2_gpu,dcsidync2_gpu,jac_gpu,self%redn_3d_gpu)
      endif
      ...
      if(self%grid%grid_dim == 1) then
        call force_rhs_2_kernel(nx,ny,nz,ng,fln_gpu,w_aux_gpu,bulk5g,fluid_mask_gpu)
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_2_c2_kernel(nx,ny,nz,ng,fln_gpu,w_aux_gpu,bulk5g,fluid_mask_gpu,r_curv,yn_gpu,dxdcsinc2_gpu,dydcsinc2_gpu)
      endif
    endassociate
  ...
  subroutine force_var(self)
    ...
    & jac_gpu => self%base_amd%jac_gpu, dcsidxnc2_gpu => self%base_amd%dcsidxnc2_gpu, dcsidync2_gpu => se&
    &lf%base_amd%dcsidync2_gpu, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_bas&
    &e%indx_cp_r)

      if (self%equation_base%theta_wall>=-1._rkind) then
        if(self%grid%grid_dim == 1) then
          call force_var_1_kernel(nx,ny,nz,ng,yn_gpu,fln_gpu,w_gpu,w_aux_gpu,bulkt,fluid_mask_gpu,&
          &cv_coeff_gpu,indx_cp_l,indx_cp_r,t0,calorically_perfect,tol_iter_nr,self%redn_3d_gpu)
          ...
          call force_var_2_kernel(nx,ny,nz,ng,w_gpu,w_aux_gpu,tbdiff,fluid_mask_gpu,cv_coeff_gpu,&
          &indx_cp_l,indx_cp_r,t0,calorically_perfect)
        elseif(self%grid%grid_dim == 2) then
          call force_var_1_c2_kernel(nx,ny,nz,ng,yn_gpu,fln_gpu,w_gpu,w_aux_gpu,bulkt,&
          &fluid_mask_gpu,cv_coeff_gpu,indx_cp_l,indx_cp_r,t0,calorically_perfect,tol_iter_nr,jac_gpu,&
          &dcsidxnc2_gpu,dcsidync2_gpu,self%redn_3d_gpu)
          call mpi_allreduce(bulkt,bulktg,1,mpi_prec,mpi_sum,self%equation_base%field%mp_cart,self%mpi_err)
          ...
          call force_var_2_c2_kernel(nx,ny,nz,ng,w_gpu,w_aux_gpu,tbdiff,fluid_mask_gpu,cv_coeff_gpu,&
  ...
  subroutine recyc_exchange(self)
    ...
    & nblocks => self%equation_base%field%nblocks, iermpi => self%mpi_err, w_gpu => self%base_amd%w_gpu,&
    & nv => self%equation_base%nv, wbuf1s_gpu => self%base_amd%wbuf1s_gpu, wbuf2r_gpu => self%base_amd%wb&
    &uf2r_gpu, wbuf1r_gpu => self%base_amd%wbuf1r_gpu, wrecyc_gpu => self%wrecyc_gpu, ibrecyc => self%equ&
    &ation_base%ib_recyc, irecyc => self%equation_base%i_recyc)
      ...
      if (ncoords(1)==ibrecyc) then
        ...
        call recyc_exchange_kernel_1(irecyc,w_gpu,wbuf1s_gpu,nx,ny,nz,ng,nv)
        indx = nv*ng*ny*nz
      ...
      if (ncoords(1)==0) then
        call recyc_exchange_kernel_2(n1_start_recv,n1_start_send,n1_end_recv,wrecyc_gpu,wbuf1r_gpu,nx,ny,nz,ng,nv)
        call recyc_exchange_kernel_3(n2_start_recv,n2_start_send,n2_end_recv,wrecyc_gpu,wbuf2r_gpu,nx,ny,nz,ng,nv)
      endif
    endassociate
  ...
  subroutine bc_nr(self)
    ...
    & nz => self%nz, nv => self%nv, ng => self%ng, w_aux_gpu => self%w_aux_gpu, w_gpu => self%base_amd%w_&
    &gpu, fl_gpu => self%fl_gpu, dcsidx_gpu => self%base_amd%dcsidx_gpu, detady_gpu => self%base_amd%deta&
    &dy_gpu, jac_gpu => self%base_amd%jac_gpu , dcsidxnc2_gpu => self%base_amd%dcsidxnc2_gpu ,&
    & dcsidync2_gpu => self%base_amd%dcsidync2_gpu , dcsidxc2_gpu => self%base_amd%dcsidxc2_gpu ,&
    & dcsidyc2_gpu => self%base_amd%dcsidyc2_gpu , detadxnc2_gpu => self%base_amd%detadxnc2_gpu ,&
    & detadync2_gpu => self%base_amd%detadync2_gpu , detadxc2_gpu => self%base_amd%detadxc2_gpu ,&
    & detadyc2_gpu => self%base_amd%detadyc2_gpu , wall_tag_gpu => self%base_amd%wall_tag_gpu ,&
    & dzitdz_gpu => self%base_amd%dzitdz_gpu, wfar_gpu => self%wfar_gpu, mcsi_gpu => self%base_amd%mcsi_g&
    &pu, meta_gpu => self%base_amd%meta_gpu, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_pe&
      ...
      do ilat=1,2*self%grid%ndim
        if(bctags_nr(ilat) > 0) then
          ...
          if(dir == 1) then
            ...
            if(self%grid%grid_dim == 1) then
              call bc_nr_lat_x_kernel_wrapper(c_null_ptr,start_or_end,bctags_nr(ilat),nx,ny,nz,ng,&
              &nv,indx_cp_l,indx_cp_r,calorically_perfect,rgas0,t0,c_loc(cp_coeff_gpu),c_loc(fl_gpu),&
              &c_loc(w_aux_gpu),c_loc(w_gpu),c_loc(dcsidx_gpu),c_loc(winf_gpu))
              call hipCheck(hipDeviceSynchronize())
            elseif(self%grid%grid_dim == 2) then
              call bc_nr_lat_x_c2_kernel_wrapper(c_null_ptr,start_or_end,bctags_nr(ilat),nx,ny,nz,&
              &ng,nv,indx_cp_l,indx_cp_r,calorically_perfect,rgas0,t0,c_loc(cp_coeff_gpu),c_loc(fl_gpu),&
              &c_loc(w_aux_gpu),c_loc(w_gpu),c_loc(dcsidx_gpu),c_loc(winf_gpu),c_loc(jac_gpu),c_loc(mcsi_gpu),&
              &c_loc(dcsidxnc2_gpu),c_loc(dcsidync2_gpu),c_loc(dcsidxc2_gpu),c_loc(dcsidyc2_gpu))
              call hipCheck(hipDeviceSynchronize())
            endif
          endif
          if(dir == 2) then
            ...
            grid = dim3(ceiling(real(nx)/tblock%x),ceiling(real(nz)/tblock%y),1)
            if(self%grid%grid_dim == 1) then
              call bc_nr_lat_y_kernel_wrapper(c_null_ptr,start_or_end,bctags_nr(ilat),nx,ny,nz,ng,&
              &nv,indx_cp_l,indx_cp_r,calorically_perfect,rgas0,t0,c_loc(cp_coeff_gpu),c_loc(fl_gpu),&
              &c_loc(w_aux_gpu),c_loc(w_gpu),c_loc(detady_gpu),c_loc(winf_gpu))
              call hipCheck(hipDeviceSynchronize())
            elseif(self%grid%grid_dim == 2) then
              call bc_nr_lat_y_c2_kernel_wrapper(c_null_ptr,start_or_end,bctags_nr(ilat),nx,ny,nz,&
              &ng,nv,indx_cp_l,indx_cp_r,calorically_perfect,rgas0,t0,c_loc(wall_tag_gpu),c_loc(cp_coeff_gpu),&
              &c_loc(fl_gpu),c_loc(w_aux_gpu),c_loc(w_gpu),c_loc(detady_gpu),c_loc(wfar_gpu),c_loc(jac_gpu),&
              &c_loc(meta_gpu),c_loc(detadxnc2_gpu),c_loc(detadync2_gpu),c_loc(detadxc2_gpu),c_loc(detadyc2_gpu))
              call hipCheck(hipDeviceSynchronize())
            endif
          endif
          if(dir == 3) then
            tblock = dim3(eulerweno_threads_x,eulerweno_threads_y,1)
            grid = dim3(ceiling(real(nx)/tblock%x),ceiling(real(ny)/tblock%y),1)
            call bc_nr_lat_z_kernel_wrapper(c_null_ptr,start_or_end,bctags_nr(ilat),nx,ny,nz,ng,nv,&
            &indx_cp_l,indx_cp_r,calorically_perfect,rgas0,t0,c_loc(cp_coeff_gpu),c_loc(fl_gpu),c_loc(w_aux_gpu),&
            &c_loc(w_gpu),c_loc(dzitdz_gpu),c_loc(winf_gpu))
            call hipCheck(hipDeviceSynchronize())
          endif
        endif
      enddo
  ...
  subroutine update_ghost(self, do_swap)
    ...
    do ilat=1,2*self%grid%ndim
      ...
      case(0)
      case(1)
        call bcfree_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%nv,self%winf_gpu,self%base_amd%w_gpu)
      case(3)
        call bcfree_sub_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%base_amd%w_gpu,&
        &self%w_aux_gpu,self%equation_base%aoa,self%equation_base%t0,self%equation_base%ptot0,&
        &self%equation_base%ttot0,self%equation_base%rgas0,self%equation_base%indx_cp_l,self%equation_base%in&
        &dx_cp_r,self%cv_coeff_gpu,self%cp_coeff_gpu,self%equation_base%calorically_perfect,tol_iter_nr)
      case(2)
        call bcextr_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%nv,self%base_amd%w_gpu)
      case(4)
        call bcextr_sub_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%equation_base%p0,&
        &self%equation_base%rgas0,self%base_amd%w_gpu,self%equation_base%indx_cp_l,self%equation_base%indx_cp&
        &_r,self%cv_coeff_gpu,self%equation_base%t0,self%equation_base%calorically_perfect)
      case(5)
        if(self%grid%grid_dim == 1) then
          call bcsym_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%base_amd%w_gpu)
        elseif(self%grid%grid_dim == 2) then
          call bcsym_c2_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%base_amd%w_gpu,&
          &self%base_amd%dxdcsic2_gpu,self%base_amd%dydcsic2_gpu)
        endif
      case(6)
        if (self%grid%is_y_staggered) then
          call bcwall_staggered_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%equation_base%t_wal&
          &l,self%base_amd%w_gpu,self%w_aux_gpu,self%equation_base%indx_cp_l,self%equation_base%indx_cp_r,&
          &self%cv_coeff_gpu,self%equation_base%t0,self%equation_base%rgas0,self%equation_base%calorically_perf&
          &ect,tol_iter_nr)
        else
          call bcwall_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%equation_base%t_wall,&
          &self%base_amd%w_gpu,self%w_aux_gpu,self%equation_base%indx_cp_l,self%equation_base%indx_cp_r,&
          &self%cv_coeff_gpu,self%equation_base%t0,self%equation_base%rgas0,self%equation_base%calorically_perf&
          &ect,tol_iter_nr)
        endif
      case(7)
        call bcshock_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%nv,self%base_amd%w_gpu,&
        &self%winf_gpu,self%winf_past_shock_gpu,self%equation_base%xshock_imp,self%equation_base%shock_angle,&
        &self%base_amd%x_gpu,self%base_amd%y_gpu,self%equation_base%tanhfacs)
      case(8)
      case(9)
        call bclam_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%nv,self%base_amd%w_gpu,&
        &self%wmean_gpu,self%equation_base%p0,self%equation_base%rgas0,self%equation_base%indx_cp_l,&
        &self%equation_base%indx_cp_r,self%cv_coeff_gpu,self%equation_base%t0,self%equation_base%calorically_&
        &perfect)
      case(10)
        call self%bcrecyc(ilat)
      case(11)
        call self%base_amd%bcswap_wake()
        call bcwall_airfoil_kernel(ilat,self%nx,self%ny,self%nz,self%ng,self%equation_base%t_wall,&
        &self%base_amd%w_gpu,self%w_aux_gpu,self%base_amd%wall_tag_gpu,self%equation_base%indx_cp_l,&
        &self%equation_base%indx_cp_r,self%cv_coeff_gpu,self%equation_base%t0,self%equation_base%rgas0,&
        &self%equation_base%calorically_perfect,tol_iter_nr,self%base_amd%xc2_gpu,self%base_amd%yc2_gpu,&
        &self%equation_base%a_tw,self%equation_base%v_bs,self%equation_base%thic,self%equation_base%kx_tw,&
        &self%equation_base%om_tw,self%equation_base%xtw1,self%equation_base%xtw2,self%equation_base%time,&
        &self%base_amd%dxdetanc2_gpu,self%base_amd%dydetanc2_gpu,self%equation_base%u0)
        call self%base_amd%bcte()
      endselect
    enddo

    if (do_swap_ == 1) call self%base_amd%bcswap()
  ...
  subroutine bcrecyc(self, ilat)
    ...
    if (ilat == 1) then
      ...
      & w_gpu => self%base_amd%w_gpu, wmean_gpu => self%wmean_gpu, weta_inflow_gpu => self%weta_inflow_gpu,&
      ...
      &quation_base%inflow_random_plane, inflow_random_plane_gpu => self%inflow_random_plane_gpu)
        call bcrecyc_kernel_1(nx,ny,nz,ng,nv,wrecycav_gpu,wrecyc_gpu)
        call hipCheck(hipMemcpy(inflow_random_plane_gpu , inflow_random_plane,hipMemcpyHostToDevice))

        ntot = ng*ny*nv
        call mpi_allreduce(mpi_in_place,wrecycav_gpu,ntot,mpi_prec,mpi_sum,mp_cartz,iermpi)

        call bcrecyc_kernel_2(nx,ny,nz,nzmax,ng,wrecycav_gpu,wrecyc_gpu)
        call bcrecyc_kernel_3(nx,ny,nz,ng,p0,u0,rgas0,w_gpu,wmean_gpu,wrecyc_gpu,weta_inflow_gpu,&
  ...
  subroutine initialize(self, filename)
    ...
    call self%base_amd%initialize(self%equation_base%field, self%equation_base%gpu_bind)

    if(self%equation_base%debug_memory>0) then
      call self%field%check_cpu_mem(description="--initialize-start--")
      call self%base_amd%check_gpu_mem(description="--initialize-start--")
    endif

    call self%base_amd%copy_cpu_gpu()

    if(self%equation_base%debug_memory>0) then
      call self%field%check_cpu_mem(description="--initialize-first-gpu-usage--")
      call self%base_amd%check_gpu_mem(description="--initialize-first-gpu-usage--")
    endif

    call self%alloc()

    call hipCheck(hipMemcpy(self%w_aux_gpu , self%equation_base%w_aux,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%fluid_mask_gpu , self%equation_base%fluid_mask,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%ep_ord_change_gpu , self%equation_base%ep_ord_change,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%winf_gpu , self%equation_base%winf,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%winf_past_shock_gpu , self%equation_base%winf_past_shock,hipMemcpyHostToDevice))
    if (self%grid%grid_dim == 2) call hipCheck(hipMemcpy(self%wfar_gpu , self%equation_base%wfar,hipMemcpyHostToDevice))

    call hipCheck(hipMemcpy(self%lmax_tag_gpu , self%equation_base%lmax_tag,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%vis_tag_gpu , self%equation_base%vis_tag,hipMemcpyHostToDevice))

    call hipCheck(hipMemcpy(self%cp_coeff_gpu , self%equation_base%cp_coeff,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%cv_coeff_gpu , self%equation_base%cv_coeff,hipMemcpyHostToDevice))

    if (self%equation_base%num_probe>0) then
      call hipCheck(hipMemcpy(self%probe_coeff_gpu , self%equation_base%probe_coeff,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%ijk_probe_gpu , self%equation_base%ijk_probe,hipMemcpyHostToDevice))
    endif

    if(self%equation_base%flow_init /= 5) then
      call hipCheck(hipMemcpy(self%wmean_gpu , self%equation_base%wmean,hipMemcpyHostToDevice))
    endif

    if(self%equation_base%enable_tspec > 0) then
      call hipCheck(hipMemcpy(self%w_tspec_gpu , self%equation_base%w_tspec,hipMemcpyHostToDevice))
    endif

    call hipCheck(hipMalloc(self%coeff_deriv1_gpu,(4)-(1)+1,(4)-(1)+1))
    call hipCheck(hipMalloc(self%coeff_deriv2_gpu,(4)-(0)+1,(4)-(1)+1))
    call hipCheck(hipMemcpy(self%coeff_deriv1_gpu , self%equation_base%coeff_deriv1,hipMemcpyHostToDevice))
    call hipCheck(hipMemcpy(self%coeff_deriv2_gpu , self%equation_base%coeff_deriv2,hipMemcpyHostToDevice))

    if (self%equation_base%ifilter > 0) then
      call hipCheck(hipMemcpy(self%coeff_filter_gpu , self%equation_base%coeff_filter,hipMemcpyHostToDevice))
    endif

    if (self%equation_base%enable_sponge > 0) then
      call hipCheck(hipMemcpy(self%f_sponge_gpu , self%equation_base%f_sponge,hipMemcpyHostToDevice))
    endif

    if (self%equation_base%recyc) then
      call hipCheck(hipMemcpy(self%yplus_inflow_gpu , self%equation_base%yplus_inflow,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%eta_inflow_gpu , self%equation_base%eta_inflow,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%yplus_recyc_gpu , self%equation_base%yplus_recyc,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%eta_recyc_gpu , self%equation_base%eta_recyc,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%eta_recyc_blend_gpu , self%equation_base%eta_recyc_blend,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%map_j_inn_gpu , self%equation_base%map_j_inn,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%map_j_out_gpu , self%equation_base%map_j_out,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%map_j_out_blend_gpu , self%equation_base%map_j_out_blend,hipMemcpyHostToDevice))
      call hipCheck(hipMemcpy(self%weta_inflow_gpu , self%equation_base%weta_inflow,hipMemcpyHostToDevice))
    endif
    ...
    if (self%equation_base%debug_memory>0) then
      ...
      call self%base_amd%check_gpu_mem(description="initialize-completed")
    endif
  ...
  subroutine alloc(self)
    ...
    &on_base%indx_cp_r, ndft => self%equation_base%ndft_tspec, grid_dim => self%grid%grid_dim)
      ...
      call hipCheck(hipMalloc(self%w_aux_gpu,(nx+ng)-(1-ng)+1,(ny+ng)-(1-ng)+1,(nz+ng)-(1-ng)+1,(nv_aux)-(1)+1))
      call hipCheck(hipMalloc(self%fluid_mask_gpu,(nx+ng)-(1-ng)+1,(ny+ng)-(1-ng)+1,(nz+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%ep_ord_change_gpu,(nx)-(0)+1,(ny)-(0)+1,(nz)-(0)+1,(3)-(1)+1))

      call hipCheck(hipMemcpy(self%w_aux_gpu , self%equation_base%w_aux,hipMemcpyHostToDevice))
      call hipCheck(hipMalloc(self%winf_gpu,(nv)-(1)+1))
      call hipCheck(hipMalloc(self%winf_past_shock_gpu,(nv)-(1)+1))
      if (grid_dim == 2) call hipCheck(hipMalloc(self%wfar_gpu,(nx)-(1)+1,(nv)-(1)+1))


      call hipCheck(hipMalloc(self%fl_gpu,(nx)-(1)+1,(ny)-(1)+1,(nz)-(1)+1,(nv)-(1)+1))
      call hipCheck(hipMalloc(self%fln_gpu,(nx)-(1)+1,(ny)-(1)+1,(nz)-(1)+1,(nv)-(1)+1))
      call hipCheck(hipMalloc(self%wallprop_gpu,(nx+ng)-(1-ng)+1,(nz+ng)-(1-ng)+1,(4)-(2)+1))
      call hipCheck(hipMalloc(self%fhat_gpu,(nx+ng)-(1-ng)+1,(ny+ng)-(1-ng)+1,(nz+ng)-(1-ng)+1,(nv)-(1)+1))

      call hipCheck(hipMalloc(self%wrecyc_gpu,(ng)-(1)+1,(ny)-(1)+1,(nz)-(1)+1,(nv)-(1)+1))
      call hipCheck(hipMalloc(self%wrecycav_gpu,(ng)-(1)+1,(ny)-(1)+1,(nv)-(1)+1))
      call hipCheck(hipMalloc(self%wmean_gpu,(self%field%nx+self%grid%ng+1)-(1-self%grid%ng)+1,(self%field%ny)-(1)+1,(4)-(1)+1))
      call hipCheck(hipMalloc(self%redn_3d_gpu,(nx)-(1)+1,(ny)-(1)+1,(nz)-(1)+1))

      call hipCheck(hipMalloc(self%yplus_inflow_gpu,(ny+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%eta_inflow_gpu,(ny+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%yplus_recyc_gpu,(ny+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%eta_recyc_gpu,(ny+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%eta_recyc_blend_gpu,(ny+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%map_j_inn_gpu,(ny)-(1)+1))
      call hipCheck(hipMalloc(self%map_j_out_gpu,(ny)-(1)+1))
      call hipCheck(hipMalloc(self%map_j_out_blend_gpu,(ny)-(1)+1))
      call hipCheck(hipMalloc(self%weta_inflow_gpu,(ny)-(1)+1))
      call hipCheck(hipMalloc(self%inflow_random_plane_gpu,(ny)-(1)+1,(nz)-(1)+1,(3)-(1)+1))

      call hipCheck(hipMalloc(self%cv_coeff_gpu,(indx_cp_r+1)-(indx_cp_l)+1))
      call hipCheck(hipMalloc(self%cp_coeff_gpu,(indx_cp_r+1)-(indx_cp_l)+1))

      if (self%equation_base%num_probe>0) then
        call hipCheck(hipMalloc(self%w_aux_probe_gpu,(6)-(1)+1,(self%equation_base%num_probe)-(1)+1))
        call hipCheck(hipMalloc(self%ijk_probe_gpu,(3)-(1)+1,(self%equation_base%num_probe)-(1)+1))
        call hipCheck(hipMalloc(self%probe_coeff_gpu,(2)-(1)+1,(2)-(1)+1,(2)-(1)+1,(self%equation_base%num_probe)-(1)+1))
      endif

      call hipCheck(hipMalloc(self%lmax_tag_gpu,(nx+ng)-(1-ng)+1))
      call hipCheck(hipMalloc(self%vis_tag_gpu,(nx+ng)-(1-ng)+1))

      if (self%equation_base%enable_tspec > 0) then
        call hipCheck(hipMalloc(self%w_psd_tspec_gpu,(nx)-(1)+1,(ndft)-(1)+1))
        call hipCheck(hipMalloc(self%w_tspec_gpu,(nx)-(1)+1,(nz)-(1)+1,(ndft)-(1)+1,(2)-(1)+1,(2)-(1)+1))
      endif

      if (self%equation_base%ifilter > 0) call hipCheck(hipMalloc(self%coeff_filter_gpu,(4)-(0)+1))
      if (self%equation_base%enable_sponge > 0) call hipCheck(hipMalloc(self%f_sponge_gpu,(ny)-(1)+1))
  ...
  subroutine compute_residual(self)
    ...
    & tmin => self%equation_base%tmin, tmax => self%equation_base%tmax)
      ...
      call compute_residual_kernel(nx,ny,nz,ng,nv,fln_gpu,dt,residual_rhou,fluid_mask_gpu,self%redn_3d_gpu)
      ...
      call compute_rho_t_p_minmax_kernel(nx,ny,nz,ng,rgas0,w_aux_gpu,rhomin,rhomax,tmin,tmax,pmin,&
      &pmax,fluid_mask_gpu,self%redn_3d_gpu)
  ...
  subroutine count_weno(self)
    ...
    & wall_tag_gpu => self%base_amd%wall_tag_gpu, count_weno_x => self%equation_base%count_weno_x,&
    & count_weno_y => self%equation_base%count_weno_y, count_weno_z => self%equation_base%count_weno_z)

      if (grid_dim==1) then
        call count_weno_kernel(nv,nv_aux,nx,ny,nz,ng,eul_imin,eul_imax,eul_jmin,eul_jmax,eul_kmin,&
        &eul_kmax,weno_scheme,sensor_threshold,w_aux_gpu,ep_ord_change_gpu,count_weno_x,count_weno_y,&
        &count_weno_z,self%redn_3d_gpu)
      else
        lmax = ep_order/2
        call count_weno_c2_kernel(nv,nv_aux,nx,ny,nz,ng,eul_imin,eul_imax,eul_jmin,eul_jmax,&
        &eul_kmin,eul_kmax,lmax,weno_scheme,sensor_threshold,w_aux_gpu,ep_ord_change_gpu,lmax_tag_gpu,&
        &wall_tag_gpu,count_weno_x,count_weno_y,count_weno_z,self%redn_3d_gpu)
      endif
  ...
  subroutine compute_airfoil_forces_runtime(self)
    ...
    & wall_tag_gpu => self%base_amd%wall_tag_gpu, meta_gpu => self%base_amd%meta_gpu, csimod_gpu => self%&
    &base_amd%csimod_gpu, dxdcsic2_gpu => self%base_amd%dxdcsic2_gpu, dydcsic2_gpu => self%base_amd%dydcs&
    ...
    & pres_af => self%equation_base%pres_af, fric_af => self%equation_base%fric_af)
      ...
      call compute_airfoil_forces_runtime_kernel(nx,ny,nz,ng,nv,p0,u0,rgas0,w_aux_gpu,meta_gpu,&
      &csimod_gpu,wall_tag_gpu,dxdcsic2_gpu,dydcsic2_gpu,n,a,pn,pa,tn,ta,self%redn_3d_gpu)
  ...
  subroutine compute_dt(self)
    ...
    & sutherland_s => self%sutherland_s, w_gpu => self%base_amd%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    & dcsidx_gpu => self%base_amd%dcsidx_gpu, detady_gpu => self%base_amd%detady_gpu, dzitdz_gpu => self%&
    &base_amd%dzitdz_gpu, dcsidxs_gpu => self%base_amd%dcsidxs_gpu, detadys_gpu => self%base_amd%detadys_&
    &gpu, dzitdzs_gpu => self%base_amd%dzitdzs_gpu, dcsidxnc2_gpu => self%base_amd%dcsidxnc2_gpu,&
    & dcsidync2_gpu => self%base_amd%dcsidync2_gpu, detadxnc2_gpu => self%base_amd%detadxnc2_gpu,&
    & detadync2_gpu => self%base_amd%detadync2_gpu, mcsi_gpu => self%base_amd%mcsi_gpu,&
    & meta_gpu => self%base_amd%meta_gpu, calorically_perfect => self%equation_base%calorically_perfect,&
    & indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r,&
    ...
    &se%rgas0, t0 => self%equation_base%t0, ndim => self%grid%ndim)
      if (cfl < 0) then
        ...
        if(self%grid%grid_dim == 1) then
          call compute_dt_kernel(nx,ny,nz,ng,rgas0,prandtl,dcsidx_gpu,detady_gpu,dzitdz_gpu,&
          ...
          &calorically_perfect,t0,self%redn_3d_gpu)
        elseif(self%grid%grid_dim == 2) then
          call compute_dt_c2_kernel(nx,ny,nz,ng,rgas0,prandtl,dcsidxnc2_gpu,dcsidync2_gpu,&
          ...
          &cp_coeff_gpu,fluid_mask_gpu,calorically_perfect,t0,self%redn_3d_gpu)
        endif
        if(ndim == 2) then
  ...
  subroutine run(self, filename)
    ...
    &ion_base%time_is_freezed)
      ...
      call zero_flux_kernel(self%nx,self%ny,self%nz,self%nv,self%fl_gpu)
      ...
      if (self%equation_base%restart_type==0) then
        call hipCheck(hipMemcpy(self%equation_base%w_aux , self%w_aux_gpu,hipMemcpyDeviceToHost))
        if (self%equation_base%enable_plot3d>0) then
        ...
        if ( time_is_freezed ) then
          ...
          if (mod(icyc-icyc0, self%equation_base%print_control)==0) then
            ...
            if (ieee_is_nan(self%equation_base%residual_rhou)) then
              ...
              call self%base_amd%copy_gpu_cpu()
              if (self%equation_base%enable_plot3d>0) then
      ...
      if (self%equation_base%io_type_w==1) then
        ...
        if (self%equation_base%enable_tspec>0) then
          call hipCheck(hipMemcpy(self%equation_base%w_tspec , self%w_tspec_gpu,hipMemcpyDeviceToHost))
  ...
  subroutine insitu_coprocess(self)
    ...
    integer :: l,i,j,k,ll
    call self%update_ghost()
    call self%base_amd%bcswap_edges_corners()
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, visc_model => self%visc_mo&
    ...
    if (self%equation_base%npsi > 0) then
      ...
      do l=1,self%equation_base%npsi
        call self%base_amd%bcswap_var(self%psi_gpu(:,:,:,l:l ))
      enddo
      do l=1,self%equation_base%npsi
        call self%base_amd%bcswap_edges_corners_var(self%psi_gpu(:,:,:,l:l ))
      enddo
    endif
    associate( nxsl_ins => self%equation_base%nxsl_ins, nxel_ins => self%equation_base%nxel_ins,&
    & nysl_ins => self%equation_base%nysl_ins, nyel_ins => self%equation_base%nyel_ins,&
    ...
    &e, time_insitu => self%equation_base%time_insitu)
      ...
      call hipCheck(hipMemcpy(self%equation_base%w_aux , w_aux_gpu,hipMemcpyDeviceToHost))
      call hipCheck(hipMemcpy(self%equation_base%psi , psi_gpu,hipMemcpyDeviceToHost))
  ...
  subroutine insitu_compute_psi(self)
    ...
    & w_aux_trans_gpu => self%w_aux_trans_gpu, dcsidx_gpu => self%base_amd%dcsidx_gpu,&
    & detady_gpu => self%base_amd%detady_gpu, dzitdz_gpu => self%base_amd%dzitdz_gpu, dcsidxs_gpu => self&
    &%base_amd%dcsidxs_gpu, detadys_gpu => self%base_amd%detadys_gpu, dzitdzs_gpu => self%base_amd%dzitdz&
    &s_gpu, dcsidx2_gpu => self%base_amd%dcsidx2_gpu, detady2_gpu => self%base_amd%detady2_gpu,&
    & dzitdz2_gpu => self%base_amd%dzitdz2_gpu, dcsidxc2_gpu => self%base_amd%dcsidxc2_gpu,&
    & detadyc2_gpu => self%base_amd%detadyc2_gpu, detadxc2_gpu => self%base_amd%detadxc2_gpu,&
    & dcsidyc2_gpu => self%base_amd%dcsidyc2_gpu, vis_tag_gpu => self%vis_tag_gpu, wall_tag_gpu => self%b&
    &ase_amd%wall_tag_gpu, eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_base%eul_im&
    ...
    & x_gpu => self%base_amd%x_gpu )
      do l=1,npsi
        if (add_list(l) == 1) then
          call insitu_div_kernel(nx,ny,nz,ng,visc_order,npsi,l,w_aux_gpu,coeff_deriv1_gpu,dcsidx_gpu,detady_gpu,dzitdz_gpu,psi_gpu)
        endif
        if (add_list(l) == 2) then
          call insitu_omega_kernel(nx,ny,nz,ng,visc_order,npsi,l,w_aux_gpu,coeff_deriv1_gpu,&
          &dcsidx_gpu,detady_gpu,dzitdz_gpu,psi_gpu)
        endif
        if (add_list(l) == 3) then
          call insitu_ducros_kernel(nx,ny,nz,ng,visc_order,npsi,l,u0,l0,w_aux_gpu,coeff_deriv1_gpu,&
          &dcsidx_gpu,detady_gpu,dzitdz_gpu,psi_gpu)
        endif
        if (add_list(l) == 4) then
          ...
          grid = dim3(ceiling(real(nx)/tblock%x),ceiling(real(nz)/tblock%y),1)
          if(self%grid%grid_dim == 1) then
            call insitu_swirling_kernel_wrapper(c_null_ptr,nv,nx,ny,nz,visc_order,ng,npsi,l,u0,&
            &c_loc(dcsidx_gpu),c_loc(detady_gpu),c_loc(dzitdz_gpu),c_loc(psi_gpu),c_loc(w_aux_gpu),&
            &c_loc(coeff_deriv1_gpu),c_loc(x_gpu))
            call hipCheck(hipDeviceSynchronize())
          elseif(self%grid%grid_dim == 2) then
            call insitu_swirling_c2_kernel_wrapper(c_null_ptr,nv,nx,ny,nz,visc_order,ng,npsi,l,u0,&
            &c_loc(vis_tag_gpu),c_loc(wall_tag_gpu),c_loc(dzitdz_gpu),c_loc(dcsidxc2_gpu),c_loc(detadyc2_gpu),&
            &c_loc(detadxc2_gpu),c_loc(dcsidyc2_gpu),c_loc(psi_gpu),c_loc(w_aux_gpu),c_loc(coeff_deriv1_gpu),&
            &c_loc(x_gpu))
            call hipCheck(hipDeviceSynchronize())
          endif
        endif
        if (add_list(l) == 5) then
          ...
          grid = dim3(ceiling(real(nx)/tblock%x),ceiling(real(nz)/tblock%y),1)
          if(self%grid%grid_dim == 1) then
            call insitu_schlieren_kernel_wrapper(c_null_ptr,nv,nx,ny,nz,visc_order,ng,npsi,l,u0,&
            &c_loc(dcsidx_gpu),c_loc(detady_gpu),c_loc(dzitdz_gpu),c_loc(psi_gpu),c_loc(w_aux_gpu),&
            &c_loc(coeff_deriv1_gpu),c_loc(x_gpu))
            call hipCheck(hipDeviceSynchronize())
          elseif(self%grid%grid_dim == 2) then
            call insitu_schlieren_c2_kernel_wrapper(c_null_ptr,nv,nx,ny,nz,visc_order,ng,npsi,l,u0,&
            &c_loc(vis_tag_gpu),c_loc(wall_tag_gpu),c_loc(dzitdz_gpu),c_loc(dcsidxc2_gpu),c_loc(detadyc2_gpu),&
            &c_loc(detadxc2_gpu),c_loc(dcsidyc2_gpu),c_loc(psi_gpu),c_loc(w_aux_gpu),c_loc(coeff_deriv1_gpu),&
            &c_loc(x_gpu))
            call hipCheck(hipDeviceSynchronize())
  ...
  subroutine manage_output(self)
    ...
    &ic2_gpu )
      ...
      if (time_from_last_stat >= self%equation_base%dtstat) then
        ...
        call hipCheck(hipMemcpy(self%equation_base%w_aux(:,:,:,6) , self%w_aux_gpu(:,:,:,(6)-(1)+1),hipMemcpyDeviceToHost))
        call hipCheck(hipMemcpy(self%equation_base%w_aux(:,:,:,7) , self%w_aux_gpu(:,:,:,(7)-(1)+1),hipMemcpyDeviceToHost))
      ...
      if (time_from_last_slice >= self%equation_base%dtslice) then
        ...
        if (allocated(self%equation_base%islice)) then
          ...
          do i=1,size(self%equation_base%islice)
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              call hipCheck(hipMemcpy(sliceyz_aux(i,:,:,m) , self%w_aux_gpu((ii)-(1-ng)+1,:,:,(mm)-(1)+1),hipMemcpyDeviceToHost))
            enddo
          enddo
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              if(jj == 1 .and. mm >= 2 .and. mm <= 4) then
                if (grid_dim == 2 .and. mm == 2) then
                  call compute_wallprop_c2_kernel(nx,ny,nz,ng,w_aux_gpu,wallprop_gpu,dxdcsic2_gpu,&
                  &dydcsic2_gpu,csimod_gpu,meta_gpu,wall_tag_gpu)
                endif
                call hipCheck(hipMemcpy(slicexz_aux(:,j,:,m) , wallprop_gpu(:,:,(mm)-(2)+1),hipMemcpyDeviceToHost))
              else
                call hipCheck(hipMemcpy(slicexz_aux(:,j,:,m) , self%w_aux_gpu(:,(jj)-(1-ng)+1,:,(mm)-(1)+1),hipMemcpyDeviceToHost))
              endif
            enddo
          enddo
        ...
        if (allocated(self%equation_base%kslice)) then
          ...
          do k=1,size(self%equation_base%kslice)
            ...
            do m=1,self%equation_base%num_aux_slice
              mm = self%equation_base%list_aux_slice(m)
              call hipCheck(hipMemcpy(slicexy_aux(:,:,k,m) , self%w_aux_gpu(:,:,(kk)-(1-ng)+1,(mm)-(1)+1),hipMemcpyDeviceToHost))
            enddo
          enddo
      ...
      if (time_from_last_probe >= self%equation_base%dtprobe) then
        ...
        if (self%equation_base%num_probe>0) then
          call probe_interpolation_kernel(self%equation_base%num_probe,nx,ny,nz,ng,nv_aux,&
          &ijk_probe_gpu,w_aux_probe_gpu,w_aux_gpu,probe_coeff_gpu)
          call hipCheck(hipMemcpy(w_aux_probe , w_aux_probe_gpu,hipMemcpyDeviceToHost))
        ...
        if(self%equation_base%enable_slice_plot3d > 0) then
          ...
          if (allocated(self%equation_base%islice_p3d)) then
            do i=1,size(self%equation_base%islice_p3d)
              ...
              do m=1,self%equation_base%num_aux_slice
                mm = self%equation_base%list_aux_slice(m)
                call hipCheck(hipMemcpy(self%equation_base%w_aux(i,:,:,mm) , self%w_aux_gpu((ii)-(1-&
                &ng)+1,:,:,(mm)-(1)+1),hipMemcpyDeviceToHost))
              enddo
            enddo
          endif
          if (allocated(self%equation_base%jslice_p3d)) then
            do j=1,size(self%equation_base%jslice_p3d)
              ...
              do m=1,self%equation_base%num_aux_slice
                mm = self%equation_base%list_aux_slice(m)
                call hipCheck(hipMemcpy(self%equation_base%w_aux(:,j,:,mm) , self%w_aux_gpu(:,(jj)-&
                &(1-ng)+1,:,(mm)-(1)+1),hipMemcpyDeviceToHost))
              enddo
              if(jj == 1) then
                if (grid_dim == 2) then
                  call compute_wallprop_c2_kernel(nx,ny,nz,ng,w_aux_gpu,wallprop_gpu,dxdcsic2_gpu,&
                ...
                call hipCheck(hipMemcpy(wallprop(:,:,:) , wallprop_gpu(:,:,:),hipMemcpyDeviceToHost))
              endif
            enddo
          endif
          if (allocated(self%equation_base%kslice_p3d)) then
            do k=1,size(self%equation_base%kslice_p3d)
              ...
              do m=1,self%equation_base%num_aux_slice
                mm = self%equation_base%list_aux_slice(m)
                call hipCheck(hipMemcpy(self%equation_base%w_aux(:,:,k,mm) , self%w_aux_gpu(:,:,&
                &(kk)-(1-ng)+1,(mm)-(1)+1),hipMemcpyDeviceToHost))
              enddo
            enddo
          endif
          call self%equation_base%write_slice_p3d(plot3dgrid=.false.,plot3dfield=.true.)
      ...
      if (time_from_last_tspec >= self%equation_base%dt_tspec) then
        if(self%equation_base%enable_tspec > 0) then
          ...
          call compute_tspec_kernel(nx,ny,nz,ng,ndft,jg_tspec,1,it_win1_tspec,w_aux_gpu,w_tspec_gpu)
          if(self%equation_base%it_win2_tspec > 0) then
            call compute_tspec_kernel(nx,ny,nz,ng,ndft,jg_tspec,2,it_win2_tspec,w_aux_gpu,w_tspec_gpu)
          endif
          if(it_win1_tspec == ndft .or. it_win2_tspec == ndft) then
            if(it_win1_tspec == ndft) then
              call compute_psd_tspec_kernel(nx,ny,nz,ndft,1,dt_tspec,w_tspec_gpu,w_psd_tspec_gpu)
            endif
            if(it_win2_tspec == ndft) then
              call compute_psd_tspec_kernel(nx,ny,nz,ndft,2,dt_tspec,w_tspec_gpu,w_psd_tspec_gpu)
            endif

            call hipCheck(hipMemcpy(w_psd_tspec , w_psd_tspec_gpu,hipMemcpyDeviceToHost))
...
endmodule streams_equation_singleideal_amd_object
