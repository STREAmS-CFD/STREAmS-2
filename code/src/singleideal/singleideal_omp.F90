module streams_equation_singleideal_omp_object

  use streams_base_omp_object
  use streams_field_object
  use streams_grid_object
  use streams_kernels_omp
  ...
  implicit none
  private
  public :: equation_singleideal_omp_object
  ...
  type :: equation_singleideal_omp_object
    type(base_omp_object) :: base_omp
  ...
  endtype equation_singleideal_omp_object

contains


  subroutine point_to_field(self, field)
    class(equation_singleideal_omp_object), intent(inout) :: self
  ...
  subroutine insitu_alloc_gpu(self)
    ...
    &x_list, n_add_list => self%equation_base%n_add_list )
      if (npsi > 0) call omp_target_alloc_f(fptr_dev=self%psi_gpu,ubounds=[nx+ng,ny+ng,nz+ng,npsi],&
      &lbounds=[1-ng,1-ng,1-ng,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      if (n_aux_list > 0) then
        call omp_target_alloc_f(fptr_dev=self%aux_list_gpu,ubounds=[n_aux_list],lbounds=[1],&
        &omp_dev=self%base_omp%mydev,ierr=self%ierr)
        self%ierr = omp_target_memcpy_f(self%aux_list_gpu , self%equation_base%aux_list,0,0,&
        &self%base_omp%mydev,self%base_omp%myhost)
      endif
      if (n_add_list > 0) then
        call omp_target_alloc_f(fptr_dev=self%add_list_gpu,ubounds=[n_add_list],lbounds=[1],&
        &omp_dev=self%base_omp%mydev,ierr=self%ierr)
        self%ierr = omp_target_memcpy_f(self%add_list_gpu , self%equation_base%add_list,0,0,&
        &self%base_omp%mydev,self%base_omp%myhost)
      endif
    endassociate
  ...
  subroutine visflx(self, mode)
    ...
    & w_aux_gpu => self%w_aux_gpu, w_aux_trans_gpu => self%w_aux_trans_gpu, dcsidx_gpu => self%base_omp%d&
    &csidx_gpu, detady_gpu => self%base_omp%detady_gpu, dzitdz_gpu => self%base_omp%dzitdz_gpu,&
    & dcsidxs_gpu => self%base_omp%dcsidxs_gpu, detadys_gpu => self%base_omp%detadys_gpu,&
    & dzitdzs_gpu => self%base_omp%dzitdzs_gpu, dcsidx2_gpu => self%base_omp%dcsidx2_gpu,&
    & detady2_gpu => self%base_omp%detady2_gpu, dzitdz2_gpu => self%base_omp%dzitdz2_gpu,&
    & x_gpu => self%base_omp%x_gpu, y_gpu => self%base_omp%y_gpu, z_gpu => self%base_omp%z_gpu,&
    ...
    & grid_dim => self%grid%grid_dim, dcsidxc2_gpu => self%base_omp%dcsidxc2_gpu, dcsidyc2_gpu => self%ba&
    &se_omp%dcsidyc2_gpu, detadxc2_gpu => self%base_omp%detadxc2_gpu, detadyc2_gpu => self%base_omp%detad&
    &yc2_gpu, g1_gpu => self%base_omp%g1_gpu, g2_gpu => self%base_omp%g2_gpu, g12_gpu => self%base_omp%g1&
    &2_gpu, jac_gpu => self%base_omp%jac_gpu, vis_tag_gpu => self%vis_tag_gpu, wall_tag_gpu => self%base_&
    &omp%wall_tag_gpu, iblock => self%field%ncoords(1), ite_rank_x => self%field%ite_rank_x,&
    ...
    &weno, theta_ij_gpu => self%base_omp%theta_ij_gpu, theta_threshold => self%equation_base%theta_thresh&
    &old)

      if (mode == 0) then
        if (grid_dim == 1) then
          call visflx_kernel(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r,&
          & cp_coeff_gpu, calorically_perfect, u0, l0, self%base_omp%w_gpu, self%w_aux_gpu, self%fl_gpu,&
        ...
        elseif (grid_dim == 2) then
          call visflx_c2_kernel(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r,&
          & cp_coeff_gpu, calorically_perfect, u0, l0, self%base_omp%w_gpu, self%w_aux_gpu, self%fl_gpu,&
      ...
      elseif (mode == 1) then
        if (grid_dim == 1) then
          call visflx_div_kernel(nx, ny, nz, ng, visc_order, self%w_aux_gpu, self%fl_gpu,&
      ...
      elseif (mode == 2) then
        call visflx_reduced_ord2_kernel(nx, ny, nz, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_gpu, calorically_perfect, u0, l0, self%base_omp%w_gpu, self%w_aux_gpu, self%fl_gpu, x_gpu,&
      ...
      elseif (mode == 4) then
        if (grid_dim == 1) then
          ...
          & dzitdzs_gpu, dcsidx2_gpu, detady2_gpu, dzitdz2_gpu, self%wallprop_gpu)
        elseif (grid_dim == 2) then
          call visflx_nosensor_c2_kernel(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l,&
          & indx_cp_r, cp_coeff_gpu, calorically_perfect, iblock, ite_rank_x, itu_rank_x, ite_l, itu_l, u0, l0,&
          & self%base_omp%w_gpu, self%w_aux_gpu, self%fl_gpu, coeff_deriv1_gpu, coeff_deriv2_gpu, dcsidxc2_gpu,&
      ...
      elseif (mode == 5) then
        ...
        & calorically_perfect, self%base_omp%x_gpu, w_aux_gpu, self%fl_gpu, self%fhat_gpu)
        call visflx_y_kernel(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gpu,&
        & calorically_perfect, self%base_omp%y_gpu, self%w_aux_gpu, self%fl_gpu)
        call visflx_z_kernel(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gpu,&
        & calorically_perfect, self%base_omp%z_gpu, self%w_aux_gpu, self%fl_gpu)
      elseif (mode == 6) then
        call visflx_reduced_ord2_kernel(nx, ny, nz, ng, prandtl, t0, indx_cp_l, indx_cp_r,&
        & cp_coeff_gpu, calorically_perfect, u0, l0, self%base_omp%w_gpu, self%w_aux_gpu, self%fl_gpu, x_gpu,&
  ...
  subroutine compute_aux(self, central, ghost)
    ...
    & rgas0 => self%equation_base%rgas0)
      if(central_ == 1 .and. ghost_ == 1) then
        call eval_aux_kernel(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, ny+ng, 1-ng, nz+ng,&
        & self%base_omp%w_gpu, self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp,&
        & visc_power, visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0,&
        & calorically_perfect, tol_iter_nr,stream1)
      elseif(central_ == 1 .and. ghost_ == 0) then
        call eval_aux_kernel(nx, ny, nz, ng, 1, nx, 1, ny, 1, nz, self%base_omp%w_gpu,&
      ...
      elseif(central_ == 0 .and. ghost_ == 1) then
        call eval_aux_kernel(nx, ny, nz, ng, 1-ng, 0, 1-ng, ny+ng, 1-ng, nz+ng, self%base_omp%w_gpu,&
        & self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power,&
        & visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect,&
        & tol_iter_nr,stream1)
        call eval_aux_kernel(nx, ny, nz, ng, nx+1, nx+ng, 1-ng, ny+ng, 1-ng, nz+ng,&
        & self%base_omp%w_gpu, self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp,&
        & visc_power, visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0,&
        & calorically_perfect, tol_iter_nr,stream1)
        call eval_aux_kernel(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, 0, 1-ng, nz+ng, self%base_omp%w_gpu,&
        & self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power,&
        & visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect,&
        & tol_iter_nr,stream1)
        call eval_aux_kernel(nx, ny, nz, ng, 1-ng, nx+ng, ny+1, ny+ng, 1-ng, nz+ng,&
        & self%base_omp%w_gpu, self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp,&
        & visc_power, visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0,&
        & calorically_perfect, tol_iter_nr,stream1)
        call eval_aux_kernel(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, ny+ng, 1-ng, 0, self%base_omp%w_gpu,&
        & self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power,&
        & visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect,&
        & tol_iter_nr,stream1)
        call eval_aux_kernel(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, ny+ng, nz+1, nz+ng,&
        & self%base_omp%w_gpu, self%w_aux_gpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp,&
        & visc_power, visc_sutherland, visc_no, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0,&
        & calorically_perfect, tol_iter_nr,stream1)
      endif
  ...
  subroutine rk_sync(self)
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        if(flow_init == 5) then
          call self%base_omp%bcswap_wake_var(self%w_aux_gpu(:,:,:,10:10))
          call self%base_omp%bcswap_var(self%w_aux_gpu(:,:,:,10:10))
          call self%bcextr_var(self%w_aux_gpu(:,:,:,10:10), mode=2)
          call self%base_omp%bcte_var(self%w_aux_gpu(:,:,:,10:10))
        else
        ...
        if (istep == 3) then
          ...
          call self%base_omp%bcswap_var(self%w_aux_gpu(:,:,:,8:8))
          if(flow_init == 5) then
            call self%base_omp%bcswap_wake_var(self%w_aux_gpu(:,:,:,8:8))
          endif
        endif
        if (conservative_viscous==1) then
  ...
  subroutine bcextr_var(self, w_swap, mode)
    ...
    & irightz => self%field%irightz)
      ...
      elseif(mode == 2) then
        call bcextr_airfoil_var_kernel(nx, ny, nz, ng, ndim, self%base_omp%wall_tag_gpu, w_swap, ileftx, irightx, ileftz, irightz)
      endif
    endassociate
  ...
  subroutine rk_async(self)
    ...
    & eul_jmin => self%equation_base%eul_jmin, eul_jmax => self%equation_base%eul_jmax,&
    & eul_kmin => self%equation_base%eul_kmin, eul_kmax => self%equation_base%eul_kmax,&
    & channel_case => self%equation_base%channel_case, enable_sponge => self%equation_base%enable_sponge,&
    & nv_aux => self%nv_aux, enable_limiter => self%equation_base%enable_limiter, ndim => self%grid%ndim,&
    & grid_dim => self%grid%grid_dim)!
      if (channel_case) then
        self%equation_base%dpdx = 0._rkind
        self%equation_base%dpth = 0._rkind
      ...
      do istep=1,self%equation_base%nrk
        ...
        if(flow_init == 5) then
          call self%base_omp%bcswap_wake_var(self%w_aux_gpu(:,:,:,10:10)) ! div/3
          ...
          !$omp end parallel sections
          call self%bcextr_var(self%w_aux_gpu(:,:,:,10:10), mode=2) ! div
          call self%base_omp%bcte_var(self%w_aux_gpu(:,:,:,10:10)) ! div/3
        else
          call self%bcextr_var(self%w_aux_gpu(:,:,:,10:10), mode=1) ! div
          call self%base_omp%bcswap_var(self%w_aux_gpu(:,:,:,10:10), steps=[.true.,.false.,.false.]) ! div/3
          !$omp parallel sections
          !$omp section
          call self%euler_y(eul_jmin,eul_jmax)
        ...
        if (istep == 3) then
          ...
          if(flow_init == 5) then
            call self%base_omp%bcswap_var(self%w_aux_gpu(:,:,:,8:8))
            call self%base_omp%bcswap_wake_var(self%w_aux_gpu(:,:,:,8:8), steps=[.true.,.false.,.false.])
            !$omp parallel sections
            !$omp section
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            ...
            !$omp section
            call self%base_omp%bcswap_wake_var(self%w_aux_gpu(:,:,:,8:8), steps=[.false.,.true.,.true.])
            !$omp end parallel sections
          else
            call self%base_omp%bcswap_var(self%w_aux_gpu(:,:,:,8:8), steps=[.true.,.false.,.false.])
            !$omp parallel sections
            !$omp section
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            ...
            !$omp section
            call self%base_omp%bcswap_var(self%w_aux_gpu(:,:,:,8:8), steps=[.false.,.true.,.true.])
            !$omp end parallel sections
          endif
        else
          if (conservative_viscous==1) then
        ...
        if (flow_init == 5 .and. a_tr>0._rkind) call self%tripping()
        !debug airfoil if (flow_init == 5) call self%debug_airfoil()
  ...
  subroutine sponge(self)
    class(equation_singleideal_omp_object), intent(inout) :: self
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_gpu=>self%base_omp%w_gpu&
  ...
  subroutine limiter(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,w_gpu=>self%base_omp%w_gpu,&
    ...
    &m_rescale)
      ...
      &cv_coeff_gpu,calorically_perfect,t0, tol_iter_nr, rho_lim, tem_lim, rho_lim_rescale,&
      & tem_lim_rescale)
    endassociate
  ...
  subroutine filter(self)
    ...
    &se%jfilter, wall_tag_gpu => self%base_omp%wall_tag_gpu)
  ...
  subroutine tripping(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_gpu=>self%base_omp%w_gpu&
    &,fl_gpu=>self%fl_gpu, xc2_gpu => self%base_omp%xc2_gpu, yc2_gpu => self%base_omp%yc2_gpu,&
    & z_gpu => self%base_omp%z_gpu, ncoords=>self%field%ncoords, rlz => self%grid%domain_size(3),&
    & wall_tag_gpu => self%base_omp%wall_tag_gpu, dxdetanc2_gpu => self%base_omp%dxdetanc2_gpu,&
    & dydetanc2_gpu => self%base_omp%dydetanc2_gpu, a_tr => self%equation_base%a_tr, u0 => self%equation_&
  ...
  subroutine euler_x(self, istart, iend, istart_face, iend_face, do_update)
    ...
    &se%force_zero_flux, coeff_deriv1_gpu => self%coeff_deriv1_gpu, dcsidx_gpu => self%base_omp%dcsidx_gp&
    &u, fhat_gpu => self%fhat_gpu, w_gpu => self%base_omp%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    ...
    & dcsidxc2_gpu => self%base_omp%dcsidxc2_gpu, dcsidxnc2_gpu => self%base_omp%dcsidxnc2_gpu,&
    & dcsidyc2_gpu => self%base_omp%dcsidyc2_gpu, dcsidync2_gpu => self%base_omp%dcsidync2_gpu,&
    & jac_gpu => self%base_omp%jac_gpu, mcsijac1_gpu => self%base_omp%mcsijac1_gpu, lmax_tag_gpu => self%&
      ...
      if(iend - istart >= 0) then
        ...
        if (self%grid%grid_dim == 1) then
          if (flux_splitting==1) then
            ...
            & iend_face_, lmax, nkeep, rgas0, w_aux_gpu, coeff_deriv1_gpu, dcsidx_gpu, fhat_gpu,&
            ...
            & lmax, nkeep, rgas0, w_aux_gpu, coeff_deriv1_gpu, dcsidx_gpu, fhat_gpu, force_zero_flux_min,&
            & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_gpu,&
            & indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
        ...
        elseif (self%grid%grid_dim == 2) then
          ...
          & dcsidync2_gpu, jac_gpu, mcsijac1_gpu, lmax_tag_gpu, force_zero_flux_min, force_zero_flux_max,&
  ...
  subroutine euler_y(self, eul_jmin, eul_jmax)
    ...
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & nv_aux => self%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_ba&
    &se%force_zero_flux, coeff_deriv1_gpu => self%coeff_deriv1_gpu, detady_gpu => self%base_omp%detady_gp&
    ...
    & nkeep => self%equation_base%nkeep, wall_tag_gpu => self%base_omp%wall_tag_gpu, flux_splitting => se&
    &lf%equation_base%flux_splitting, detadxc2_gpu => self%base_omp%detadxc2_gpu, detadxnc2_gpu => self%b&
    &ase_omp%detadxnc2_gpu, detadyc2_gpu => self%base_omp%detadyc2_gpu, detadync2_gpu => self%base_omp%de&
    &tadync2_gpu, jac_gpu => self%base_omp%jac_gpu, metajac1_gpu => self%base_omp%metajac1_gpu,&
    & rgas0 => self%equation_base%rgas0)
      weno_size = 2*weno_scheme
      lmax = ep_order/2
      ...
      if (self%grid%grid_dim == 1) then
        if (flux_splitting==1) then
          ...
          & nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, detady_gpu, fhat_gpu, force_zero_flux_min,&
          & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_gpu,&
          & indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
          &self%equation_base%u0, self%equation_base%t0)
        else
          call euler_y_hybrid_kernel(nv, nv_aux, nx, ny, nz, ng, eul_jmin, eul_jmax, lmax, nkeep,&
          & rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, detady_gpu, fhat_gpu, force_zero_flux_min,&
          & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_gpu,&
          & indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
          &self%equation_base%u0, self%equation_base%t0)
        endif
      ...
      elseif (self%grid%grid_dim == 2) then
        ...
        & rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, fhat_gpu, detadxc2_gpu, detadxnc2_gpu, detadyc2_gpu,&
        & detadync2_gpu, jac_gpu, metajac1_gpu, wall_tag_gpu, force_zero_flux_min, force_zero_flux_max,&
        & weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_gpu, indx_cp_l, indx_cp_r,&
        & ep_ord_change_gpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u0,&
        & self%equation_base%t0)
  ...
  subroutine euler_z(self, eul_kmin, eul_kmax)
    ...
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv,&
    & nv_aux => self%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_ba&
    &se%force_zero_flux, coeff_deriv1_gpu => self%coeff_deriv1_gpu, dzitdz_gpu => self%base_omp%dzitdz_gp&
    &u, fhat_gpu => self%fhat_gpu, w_aux_gpu => self%w_aux_gpu, fl_gpu => self%fl_gpu,&
    & sensor_threshold => self%equation_base%sensor_threshold, weno_scheme => self%equation_base%weno_sch&
    ...
    & inflow_random_plane_gpu => self%inflow_random_plane_gpu)
      ...
      if (flux_splitting==1) then
        ...
        & nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, dzitdz_gpu, fhat_gpu, force_zero_flux_min,&
        & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_gpu,&
        & indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0, self%equation_base%t0)
      else
        call euler_z_hybrid_kernel(nv, nv_aux, nx, ny, nz, ng, eul_kmin, eul_kmax, lmax, nkeep,&
        & rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, dzitdz_gpu, fhat_gpu, force_zero_flux_min,&
        & force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_gpu,&
        & indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,&
        &self%equation_base%u0, self%equation_base%t0)
      endif
      call euler_z_update_kernel(nx, ny, nz, ng, nv, eul_kmin, eul_kmax, fhat_gpu, fl_gpu, dzitdz_gpu, 0)
  ...
  subroutine force_rhs(self)
    ...
    & yn_gpu => self%base_omp%yn_gpu, w_gpu => self%base_omp%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    & fln_gpu => self%fln_gpu, volchan => self%equation_base%volchan, fluid_mask_gpu => self%fluid_mask_g&
    &pu, r_curv => self%grid%r_curv, volume => self%field%volume, dxdcsinc2_gpu => self%base_omp%dxdcsinc&
    &2_gpu, dydcsinc2_gpu => self%base_omp%dydcsinc2_gpu, dcsidxnc2_gpu => self%base_omp%dcsidxnc2_gpu,&
    & dcsidync2_gpu => self%base_omp%dcsidync2_gpu, jac_gpu => self%base_omp%jac_gpu)

      if(self%grid%grid_dim == 1) then
        call force_rhs_1_kernel(nx, ny, nz, ng, yn_gpu, fln_gpu, w_gpu, w_aux_gpu, bulk5, fluid_mask_gpu)
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_1_c2_kernel(nx, ny, nz, ng, yn_gpu, fln_gpu, w_gpu, w_aux_gpu, bulk5,&
      ...
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_2_c2_kernel(nx, ny, nz, ng, fln_gpu, w_aux_gpu, bulk5g, fluid_mask_gpu,&
        & r_curv, yn_gpu, dxdcsinc2_gpu, dydcsinc2_gpu)
      endif
    endassociate
  ...
  subroutine force_var(self)
    ...
    & jac_gpu => self%base_omp%jac_gpu, dcsidxnc2_gpu => self%base_omp%dcsidxnc2_gpu, dcsidync2_gpu => se&
    &lf%base_omp%dcsidync2_gpu, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_bas&
    &e%indx_cp_r)

      if (self%equation_base%theta_wall>=-1._rkind) then
        if(self%grid%grid_dim == 1) then
          call force_var_1_kernel(nx, ny, nz, ng, yn_gpu, fln_gpu, w_gpu, w_aux_gpu, bulkt,&
          ...
          call force_var_2_kernel(nx, ny, nz, ng, w_gpu, w_aux_gpu, tbdiff, fluid_mask_gpu,&
        ...
        elseif(self%grid%grid_dim == 2) then
          ...
          call force_var_2_c2_kernel(nx, ny, nz, ng, w_gpu, w_aux_gpu, tbdiff, fluid_mask_gpu,&
  ...
  subroutine recyc_exchange(self)
    ...
    & nblocks => self%equation_base%field%nblocks, iermpi => self%mpi_err, w_gpu => self%base_omp%w_gpu,&
    & nv => self%equation_base%nv, wbuf1s_gpu => self%base_omp%wbuf1s_gpu, wbuf2r_gpu => self%base_omp%wb&
    &uf2r_gpu, wbuf1r_gpu => self%base_omp%wbuf1r_gpu, wrecyc_gpu => self%wrecyc_gpu, ibrecyc => self%equ&
  ...
  subroutine bc_nr(self)
    ...
    & nz => self%nz, nv => self%nv, ng => self%ng, w_aux_gpu => self%w_aux_gpu, w_gpu => self%base_omp%w_&
    &gpu, fl_gpu => self%fl_gpu, dcsidx_gpu => self%base_omp%dcsidx_gpu, detady_gpu => self%base_omp%deta&
    &dy_gpu, jac_gpu => self%base_omp%jac_gpu , dcsidxnc2_gpu => self%base_omp%dcsidxnc2_gpu ,&
    & dcsidync2_gpu => self%base_omp%dcsidync2_gpu , dcsidxc2_gpu => self%base_omp%dcsidxc2_gpu ,&
    & dcsidyc2_gpu => self%base_omp%dcsidyc2_gpu , detadxnc2_gpu => self%base_omp%detadxnc2_gpu ,&
    & detadync2_gpu => self%base_omp%detadync2_gpu , detadxc2_gpu => self%base_omp%detadxc2_gpu ,&
    & detadyc2_gpu => self%base_omp%detadyc2_gpu , wall_tag_gpu => self%base_omp%wall_tag_gpu ,&
    & dzitdz_gpu => self%base_omp%dzitdz_gpu, wfar_gpu => self%wfar_gpu, mcsi_gpu => self%base_omp%mcsi_g&
    &pu, meta_gpu => self%base_omp%meta_gpu, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_pe&
      ...
      do ilat=1,2*self%grid%ndim
        if(bctags_nr(ilat) > 0) then
          ...
          if(dir == 1) then
            ...
            if(self%grid%grid_dim == 1) then
              call bc_nr_lat_x_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_gpu,&
              & w_gpu, fl_gpu, dcsidx_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, calorically_perfect,rgas0,&
              &t0)
            elseif(self%grid%grid_dim == 2) then
          ...
          if(dir == 2) then
            ...
            if(self%grid%grid_dim == 1) then
              call bc_nr_lat_y_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_gpu,&
              & w_gpu, fl_gpu, detady_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, calorically_perfect,rgas0,&
              &t0)
            elseif(self%grid%grid_dim == 2) then
          ...
          if(dir == 3) then
            ...
            call bc_nr_lat_z_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_gpu,&
            & w_gpu, fl_gpu, dzitdz_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, calorically_perfect,rgas0,&
            &t0)
          endif
        endif
      enddo
  ...
  subroutine update_ghost(self, do_swap)
    ...
    do ilat=1,2*self%grid%ndim
      ...
      case(0)
      case(1)
        call bcfree_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%winf_gpu, self%base_omp%w_gpu)
      case(3)
        call bcfree_sub_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%base_omp%w_gpu,&
        ...
        &perfect, tol_iter_nr)
      case(2)
        call bcextr_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_omp%w_gpu)
      case(4)
        call bcextr_sub_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%p0,&
        & self%equation_base%rgas0, self%base_omp%w_gpu, self%equation_base%indx_cp_l, self%equation_base%ind&
        &x_cp_r, self%cv_coeff_gpu, self%equation_base%t0, self%equation_base%calorically_perfect)
      case(5)
        if(self%grid%grid_dim == 1) then
          call bcsym_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%base_omp%w_gpu)
        elseif(self%grid%grid_dim == 2) then
          call bcsym_c2_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%base_omp%w_gpu,&
          & self%base_omp%dxdcsic2_gpu, self%base_omp%dydcsic2_gpu)
        endif
      case(6)
        if (self%grid%is_y_staggered) then
          call bcwall_staggered_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%&
          &t_wall, self%base_omp%w_gpu, self%w_aux_gpu, self%equation_base%indx_cp_l, self%equation_base%indx_c&
          &p_r, self%cv_coeff_gpu, self%equation_base%t0, self%equation_base%rgas0, self%equation_base%calorica&
          &lly_perfect,tol_iter_nr)
        else
          call bcwall_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%t_wall,&
          & self%base_omp%w_gpu, self%w_aux_gpu, self%equation_base%indx_cp_l, self%equation_base%indx_cp_r,&
          & self%cv_coeff_gpu, self%equation_base%t0, self%equation_base%rgas0, self%equation_base%calorically_&
          &perfect,tol_iter_nr)
        endif
      case(7)
        call bcshock_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_omp%w_gpu,&
        & self%winf_gpu, self%winf_past_shock_gpu, self%equation_base%xshock_imp, self%equation_base%shock_an&
        &gle, self%base_omp%x_gpu, self%base_omp%y_gpu, self%equation_base%tanhfacs)
      case(8)
      case(9)
        call bclam_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_omp%w_gpu,&
      ...
      case(11)
        call self%base_omp%bcswap_wake()
        call bcwall_airfoil_kernel(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%t_wa&
        &ll, self%base_omp%w_gpu, self%w_aux_gpu, self%base_omp%wall_tag_gpu, self%equation_base%indx_cp_l,&
        & self%equation_base%indx_cp_r, self%cv_coeff_gpu, self%equation_base%t0, self%equation_base%rgas0,&
        & self%equation_base%calorically_perfect,tol_iter_nr, self%base_omp%xc2_gpu, self%base_omp%yc2_gpu,&
        ...
        &e, self%base_omp%dxdetanc2_gpu, self%base_omp%dydetanc2_gpu, self%equation_base%u0)
        call self%base_omp%bcte()
      endselect
    enddo

    if (do_swap_ == 1) call self%base_omp%bcswap()
  ...
  subroutine bcrecyc(self, ilat)
    ...
    if (ilat == 1) then
      ...
      & w_gpu => self%base_omp%w_gpu, wmean_gpu => self%wmean_gpu, weta_inflow_gpu => self%weta_inflow_gpu,&
      ...
      &quation_base%inflow_random_plane, inflow_random_plane_gpu => self%inflow_random_plane_gpu)
        ...
        self%ierr = omp_target_memcpy_f(inflow_random_plane_gpu , inflow_random_plane,0,0,self%base_omp%mydev,self%base_omp%myhost)
        ...
        call bcrecyc_kernel_3(nx, ny, nz, ng, p0, u0, rgas0, w_gpu, wmean_gpu, wrecyc_gpu,&
  ...
  subroutine initialize(self, filename)
    ...
    call self%base_omp%initialize(self%equation_base%field, self%equation_base%gpu_bind)

    if(self%equation_base%debug_memory>0) then
      call self%field%check_cpu_mem(description="--initialize-start--")
      call self%base_omp%check_gpu_mem(description="--initialize-start--")
    endif

    call self%base_omp%copy_cpu_gpu()

    if(self%equation_base%debug_memory>0) then
      call self%field%check_cpu_mem(description="--initialize-first-gpu-usage--")
      call self%base_omp%check_gpu_mem(description="--initialize-first-gpu-usage--")
    endif

    call self%alloc()

    self%ierr = omp_target_memcpy_f(self%w_aux_gpu , self%equation_base%w_aux,0,0,self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%fluid_mask_gpu , self%equation_base%fluid_mask,0,0,&
    &self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%ep_ord_change_gpu , self%equation_base%ep_ord_change,0,0,&
    &self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%winf_gpu , self%equation_base%winf,0,0,self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%winf_past_shock_gpu , self%equation_base%winf_past_shock,0,&
    &0,self%base_omp%mydev,self%base_omp%myhost)
    if (self%grid%grid_dim == 2) self%ierr = omp_target_memcpy_f(self%wfar_gpu , self%equation_base%&
    &wfar,0,0,self%base_omp%mydev,self%base_omp%myhost)

    self%ierr = omp_target_memcpy_f(self%lmax_tag_gpu , self%equation_base%lmax_tag,0,0,self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%vis_tag_gpu , self%equation_base%vis_tag,0,0,self%base_omp%mydev,self%base_omp%myhost)

    self%ierr = omp_target_memcpy_f(self%cp_coeff_gpu , self%equation_base%cp_coeff,0,0,self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%cv_coeff_gpu , self%equation_base%cv_coeff,0,0,self%base_omp%mydev,self%base_omp%myhost)

    if (self%equation_base%num_probe>0) then
      self%ierr = omp_target_memcpy_f(self%probe_coeff_gpu , self%equation_base%probe_coeff,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%ijk_probe_gpu , self%equation_base%ijk_probe,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
    endif

    if(self%equation_base%flow_init /= 5) then
      self%ierr = omp_target_memcpy_f(self%wmean_gpu , self%equation_base%wmean,0,0,self%base_omp%mydev,self%base_omp%myhost)
    endif

    if(self%equation_base%enable_tspec > 0) then
      self%ierr = omp_target_memcpy_f(self%w_tspec_gpu , self%equation_base%w_tspec,0,0,self%base_omp%mydev,self%base_omp%myhost)
    endif

    call omp_target_alloc_f(fptr_dev=self%coeff_deriv1_gpu,ubounds=[4,4],lbounds=[1,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
    call omp_target_alloc_f(fptr_dev=self%coeff_deriv2_gpu,ubounds=[4,4],lbounds=[0,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
    self%ierr = omp_target_memcpy_f(self%coeff_deriv1_gpu , self%equation_base%coeff_deriv1,0,0,&
    &self%base_omp%mydev,self%base_omp%myhost)
    self%ierr = omp_target_memcpy_f(self%coeff_deriv2_gpu , self%equation_base%coeff_deriv2,0,0,&
    &self%base_omp%mydev,self%base_omp%myhost)

    if (self%equation_base%ifilter > 0) then
      self%ierr = omp_target_memcpy_f(self%coeff_filter_gpu , self%equation_base%coeff_filter,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
    endif

    if (self%equation_base%enable_sponge > 0) then
      self%ierr = omp_target_memcpy_f(self%f_sponge_gpu , self%equation_base%f_sponge,0,0,self%base_omp%mydev,self%base_omp%myhost)
    endif

    if (self%equation_base%recyc) then
      self%ierr = omp_target_memcpy_f(self%yplus_inflow_gpu , self%equation_base%yplus_inflow,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%eta_inflow_gpu , self%equation_base%eta_inflow,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%yplus_recyc_gpu , self%equation_base%yplus_recyc,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%eta_recyc_gpu , self%equation_base%eta_recyc,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%eta_recyc_blend_gpu , self%equation_base%eta_recyc_blend,&
      &0,0,self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%map_j_inn_gpu , self%equation_base%map_j_inn,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%map_j_out_gpu , self%equation_base%map_j_out,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%map_j_out_blend_gpu , self%equation_base%map_j_out_blend,&
      &0,0,self%base_omp%mydev,self%base_omp%myhost)
      self%ierr = omp_target_memcpy_f(self%weta_inflow_gpu , self%equation_base%weta_inflow,0,0,&
      &self%base_omp%mydev,self%base_omp%myhost)
    endif
    ...
    if (self%equation_base%debug_memory>0) then
      ...
      call self%base_omp%check_gpu_mem(description="initialize-completed")
    endif
  ...
  subroutine alloc(self)
    ...
    &on_base%indx_cp_r, ndft => self%equation_base%ndft_tspec, grid_dim => self%grid%grid_dim)
      ...
      call omp_target_alloc_f(fptr_dev=self%w_aux_gpu,ubounds=[nx+ng,ny+ng,nz+ng,nv_aux],lbounds=[1-&
      &ng,1-ng,1-ng,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%fluid_mask_gpu,ubounds=[nx+ng,ny+ng,nz+ng],lbounds=[1-&
      &ng,1-ng,1-ng],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%ep_ord_change_gpu,ubounds=[nx,ny,nz,3],lbounds=[0,0,0,&
      &1],omp_dev=self%base_omp%mydev,ierr=self%ierr)

      self%ierr = omp_target_memcpy_f(self%w_aux_gpu , self%equation_base%w_aux,0,0,self%base_omp%mydev,self%base_omp%myhost)
      call omp_target_alloc_f(fptr_dev=self%winf_gpu,ubounds=[nv],lbounds=[1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%winf_past_shock_gpu,ubounds=[nv],lbounds=[1],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      if (grid_dim == 2) call omp_target_alloc_f(fptr_dev=self%wfar_gpu,ubounds=[nx,nv],lbounds=[1,&
      &1],omp_dev=self%base_omp%mydev,ierr=self%ierr)


      call omp_target_alloc_f(fptr_dev=self%fl_gpu,ubounds=[nx,ny,nz,nv],lbounds=[1,1,1,1],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%fln_gpu,ubounds=[nx,ny,nz,nv],lbounds=[1,1,1,1],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%wallprop_gpu,ubounds=[nx+ng,nz+ng,4],lbounds=[1-ng,1-ng,&
      &2],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%fhat_gpu,ubounds=[nx+ng,ny+ng,nz+ng,nv],lbounds=[1-ng,1-&
      &ng,1-ng,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)

      call omp_target_alloc_f(fptr_dev=self%wrecyc_gpu,ubounds=[ng,ny,nz,nv],lbounds=[1,1,1,1],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%wrecycav_gpu,ubounds=[ng,ny,nv],lbounds=[1,1,1],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%wmean_gpu,ubounds=[self%field%nx+self%grid%ng+1,&
      &self%field%ny,4],lbounds=[1-self%grid%ng,1,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)

      call omp_target_alloc_f(fptr_dev=self%yplus_inflow_gpu,ubounds=[ny+ng],lbounds=[1-ng],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%eta_inflow_gpu,ubounds=[ny+ng],lbounds=[1-ng],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%yplus_recyc_gpu,ubounds=[ny+ng],lbounds=[1-ng],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%eta_recyc_gpu,ubounds=[ny+ng],lbounds=[1-ng],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%eta_recyc_blend_gpu,ubounds=[ny+ng],lbounds=[1-ng],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%map_j_inn_gpu,ubounds=[ny],lbounds=[1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%map_j_out_gpu,ubounds=[ny],lbounds=[1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%map_j_out_blend_gpu,ubounds=[ny],lbounds=[1],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%weta_inflow_gpu,ubounds=[ny],lbounds=[1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%inflow_random_plane_gpu,ubounds=[ny,nz,3],lbounds=[1,1,&
      &1],omp_dev=self%base_omp%mydev,ierr=self%ierr)

      call omp_target_alloc_f(fptr_dev=self%cv_coeff_gpu,ubounds=[indx_cp_r+1],lbounds=[indx_cp_l],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%cp_coeff_gpu,ubounds=[indx_cp_r+1],lbounds=[indx_cp_l],&
      &omp_dev=self%base_omp%mydev,ierr=self%ierr)

      if (self%equation_base%num_probe>0) then
        call omp_target_alloc_f(fptr_dev=self%w_aux_probe_gpu,ubounds=[6,self%equation_base%num_prob&
        &e],lbounds=[1,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
        call omp_target_alloc_f(fptr_dev=self%ijk_probe_gpu,ubounds=[3,self%equation_base%num_probe]&
        &,lbounds=[1,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
        call omp_target_alloc_f(fptr_dev=self%probe_coeff_gpu,ubounds=[2,2,2,self%equation_base%num_&
        &probe],lbounds=[1,1,1,1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      endif

      call omp_target_alloc_f(fptr_dev=self%lmax_tag_gpu,ubounds=[nx+ng],lbounds=[1-ng],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      call omp_target_alloc_f(fptr_dev=self%vis_tag_gpu,ubounds=[nx+ng],lbounds=[1-ng],omp_dev=self%base_omp%mydev,ierr=self%ierr)

      if (self%equation_base%enable_tspec > 0) then
        call omp_target_alloc_f(fptr_dev=self%w_psd_tspec_gpu,ubounds=[nx,ndft],lbounds=[1,1],&
        &omp_dev=self%base_omp%mydev,ierr=self%ierr)
        call omp_target_alloc_f(fptr_dev=self%w_tspec_gpu,ubounds=[nx,nz,ndft,2,2],lbounds=[1,1,1,1,&
        &1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      endif

      if (self%equation_base%ifilter > 0) call omp_target_alloc_f(fptr_dev=self%coeff_filter_gpu,&
      &ubounds=[4],lbounds=[0],omp_dev=self%base_omp%mydev,ierr=self%ierr)
      if (self%equation_base%enable_sponge > 0) call omp_target_alloc_f(fptr_dev=self%f_sponge_gpu,&
      &ubounds=[ny],lbounds=[1],omp_dev=self%base_omp%mydev,ierr=self%ierr)
  ...
  subroutine compute_residual(self)
    ...
    & tmin => self%equation_base%tmin, tmax => self%equation_base%tmax)
      ...
      call compute_residual_kernel(nx, ny, nz, ng, nv, fln_gpu, dt, residual_rhou, fluid_mask_gpu)
      ...
      call compute_rho_t_p_minmax_kernel(nx, ny, nz, ng, rgas0, w_aux_gpu,rhomin,rhomax, tmin, tmax, pmin, pmax, fluid_mask_gpu)
  ...
  subroutine count_weno(self)
    ...
    & wall_tag_gpu => self%base_omp%wall_tag_gpu, count_weno_x => self%equation_base%count_weno_x,&
    & count_weno_y => self%equation_base%count_weno_y, count_weno_z => self%equation_base%count_weno_z)

      if (grid_dim==1) then
        call count_weno_kernel(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin, eul_jmax,&
        & eul_kmin, eul_kmax, weno_scheme, sensor_threshold, w_aux_gpu, ep_ord_change_gpu, count_weno_x,&
        & count_weno_y, count_weno_z)
      else
        lmax = ep_order/2
        call count_weno_c2_kernel(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin,&
        & eul_jmax, eul_kmin, eul_kmax, lmax, weno_scheme, sensor_threshold, w_aux_gpu, ep_ord_change_gpu ,&
  ...
  subroutine compute_airfoil_forces_runtime(self)
    ...
    & wall_tag_gpu => self%base_omp%wall_tag_gpu, meta_gpu => self%base_omp%meta_gpu, csimod_gpu => self%&
    &base_omp%csimod_gpu, dxdcsic2_gpu => self%base_omp%dxdcsic2_gpu, dydcsic2_gpu => self%base_omp%dydcs&
    ...
    & pres_af => self%equation_base%pres_af, fric_af => self%equation_base%fric_af)
      ...
      call compute_airfoil_forces_runtime_kernel(nx, ny, nz, ng, nv, p0, u0, rgas0, w_aux_gpu,&
  ...
  subroutine compute_dt(self)
    ...
    & sutherland_s => self%sutherland_s, w_gpu => self%base_omp%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    & dcsidx_gpu => self%base_omp%dcsidx_gpu, detady_gpu => self%base_omp%detady_gpu, dzitdz_gpu => self%&
    &base_omp%dzitdz_gpu, dcsidxs_gpu => self%base_omp%dcsidxs_gpu, detadys_gpu => self%base_omp%detadys_&
    &gpu, dzitdzs_gpu => self%base_omp%dzitdzs_gpu, dcsidxnc2_gpu => self%base_omp%dcsidxnc2_gpu,&
    & dcsidync2_gpu => self%base_omp%dcsidync2_gpu, detadxnc2_gpu => self%base_omp%detadxnc2_gpu,&
    & detadync2_gpu => self%base_omp%detadync2_gpu, mcsi_gpu => self%base_omp%mcsi_gpu,&
    & meta_gpu => self%base_omp%meta_gpu, calorically_perfect => self%equation_base%calorically_perfect,&
    & indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r,&
  ...
  subroutine run(self, filename)
    ...
    &ion_base%time_is_freezed)
      ...
      if (self%equation_base%restart_type==0) then
        self%ierr = omp_target_memcpy_f(self%equation_base%w_aux , self%w_aux_gpu,0,0,self%base_omp%myhost,self%base_omp%mydev)
        if (self%equation_base%enable_plot3d>0) then
        ...
        if ( time_is_freezed ) then
          ...
          if (mod(icyc-icyc0, self%equation_base%print_control)==0) then
            ...
            if (ieee_is_nan(self%equation_base%residual_rhou)) then
              ...
              call self%base_omp%copy_gpu_cpu()
              if (self%equation_base%enable_plot3d>0) then
      ...
      if (self%equation_base%io_type_w==1) then
        ...
        if (self%equation_base%enable_tspec>0) then
          self%ierr = omp_target_memcpy_f(self%equation_base%w_tspec , self%w_tspec_gpu,0,0,&
          &self%base_omp%myhost,self%base_omp%mydev)
  ...
  subroutine insitu_coprocess(self)
    ...
    integer :: l,i,j,k,ll
    call self%update_ghost()
    call self%base_omp%bcswap_edges_corners()
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, visc_model => self%visc_mo&
    ...
    if (self%equation_base%npsi > 0) then
      ...
      do l=1,self%equation_base%npsi
        call self%base_omp%bcswap_var(self%psi_gpu(:,:,:,l:l ))
      enddo
      do l=1,self%equation_base%npsi
        call self%base_omp%bcswap_edges_corners_var(self%psi_gpu(:,:,:,l:l ))
      enddo
    endif
    associate( nxsl_ins => self%equation_base%nxsl_ins, nxel_ins => self%equation_base%nxel_ins,&
    & nysl_ins => self%equation_base%nysl_ins, nyel_ins => self%equation_base%nyel_ins,&
    ...
    &e, time_insitu => self%equation_base%time_insitu)
      ...
      self%ierr = omp_target_memcpy_f(self%equation_base%w_aux , w_aux_gpu,0,0,self%base_omp%myhost,self%base_omp%mydev)
      self%ierr = omp_target_memcpy_f(self%equation_base%psi , psi_gpu,0,0,self%base_omp%myhost,self%base_omp%mydev)
  ...
  subroutine insitu_compute_psi(self)
    ...
    & w_aux_trans_gpu => self%w_aux_trans_gpu, dcsidx_gpu => self%base_omp%dcsidx_gpu,&
    & detady_gpu => self%base_omp%detady_gpu, dzitdz_gpu => self%base_omp%dzitdz_gpu, dcsidxs_gpu => self&
    &%base_omp%dcsidxs_gpu, detadys_gpu => self%base_omp%detadys_gpu, dzitdzs_gpu => self%base_omp%dzitdz&
    &s_gpu, dcsidx2_gpu => self%base_omp%dcsidx2_gpu, detady2_gpu => self%base_omp%detady2_gpu,&
    & dzitdz2_gpu => self%base_omp%dzitdz2_gpu, dcsidxc2_gpu => self%base_omp%dcsidxc2_gpu,&
    & detadyc2_gpu => self%base_omp%detadyc2_gpu, detadxc2_gpu => self%base_omp%detadxc2_gpu,&
    & dcsidyc2_gpu => self%base_omp%dcsidyc2_gpu, vis_tag_gpu => self%vis_tag_gpu, wall_tag_gpu => self%b&
    &ase_omp%wall_tag_gpu, eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_base%eul_im&
    ...
    & x_gpu => self%base_omp%x_gpu )
      do l=1,npsi
        if (add_list(l) == 1) then
          call insitu_div_kernel(nx, ny, nz, ng, visc_order, npsi,l, w_aux_gpu, coeff_deriv1_gpu,&
        ...
        if (add_list(l) == 3) then
          call insitu_ducros_kernel(nx, ny, nz, ng, visc_order, npsi,l, u0,l0,w_aux_gpu,&
        ...
        if (add_list(l) == 4) then
          ...
          if(self%grid%grid_dim == 1) then
            call insitu_swirling_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dcsidx_gpu,&
            & detady_gpu, dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, psi_gpu, u0, x_gpu )
          elseif(self%grid%grid_dim == 2) then
            call insitu_swirling_c2_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dzitdz_gpu,&
            & w_aux_gpu, coeff_deriv1_gpu, dcsidxc2_gpu, detadyc2_gpu, detadxc2_gpu, dcsidyc2_gpu, psi_gpu, u0,&
            & x_gpu, vis_tag_gpu, wall_tag_gpu)
          endif
        endif
        if (add_list(l) == 5) then
          ...
          if(self%grid%grid_dim == 1) then
            call insitu_schlieren_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dcsidx_gpu,&
            & detady_gpu, dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, psi_gpu, u0, x_gpu )
          elseif(self%grid%grid_dim == 2) then
            call insitu_schlieren_c2_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dzitdz_gpu,&
            & w_aux_gpu, coeff_deriv1_gpu, dcsidxc2_gpu, detadyc2_gpu, detadxc2_gpu, dcsidyc2_gpu, psi_gpu, u0,&
            & x_gpu, vis_tag_gpu, wall_tag_gpu)
          endif
        endif
  ...
  subroutine manage_output(self)
    ...
    &ic2_gpu )
      ...
      if (time_from_last_stat >= self%equation_base%dtstat) then
        ...
        self%ierr = omp_target_memcpy_f(self%equation_base%w_aux(:,:,:,6) , self%w_aux_gpu(:,:,:,&
        &(6)-(1)+1),0,0,self%base_omp%myhost,self%base_omp%mydev)
        self%ierr = omp_target_memcpy_f(self%equation_base%w_aux(:,:,:,7) , self%w_aux_gpu(:,:,:,&
        &(7)-(1)+1),0,0,self%base_omp%myhost,self%base_omp%mydev)
      ...
      if (time_from_last_slice >= self%equation_base%dtslice) then
        ...
        if (allocated(self%equation_base%islice)) then
          ...
          do i=1,size(self%equation_base%islice)
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              self%ierr = omp_target_memcpy_f(sliceyz_aux(i,:,:,m) , self%w_aux_gpu((ii)-(1-ng)+1,:,&
              &:,(mm)-(1)+1),0,0,self%base_omp%myhost,self%base_omp%mydev)
            enddo
          enddo
          ...
          do j=1,size(self%equation_base%jslice)
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              if(jj == 1 .and. mm >= 2 .and. mm <= 4) then
                ...
                self%ierr = omp_target_memcpy_f(slicexz_aux(:,j,:,m) , wallprop_gpu(:,:,(mm)-(2)+1),&
                &0,0,self%base_omp%myhost,self%base_omp%mydev)
              else
                self%ierr = omp_target_memcpy_f(slicexz_aux(:,j,:,m) , self%w_aux_gpu(:,(jj)-(1-ng)+&
                &1,:,(mm)-(1)+1),0,0,self%base_omp%myhost,self%base_omp%mydev)
              endif
            enddo
          enddo
        ...
        if (allocated(self%equation_base%kslice)) then
          ...
          do k=1,size(self%equation_base%kslice)
            ...
            do m=1,self%equation_base%num_aux_slice
              mm = self%equation_base%list_aux_slice(m)
              self%ierr = omp_target_memcpy_f(slicexy_aux(:,:,k,m) , self%w_aux_gpu(:,:,(kk)-(1-ng)+&
              &1,(mm)-(1)+1),0,0,self%base_omp%myhost,self%base_omp%mydev)
            enddo
          enddo
      ...
      if (time_from_last_probe >= self%equation_base%dtprobe) then
        ...
        if (self%equation_base%num_probe>0) then
          ...
          self%ierr = omp_target_memcpy_f(w_aux_probe , w_aux_probe_gpu,0,0,self%base_omp%myhost,self%base_omp%mydev)
          ...
          if (allocated(self%equation_base%jslice_p3d)) then
            do j=1,size(self%equation_base%jslice_p3d)
              ...
              if(jj == 1) then
                ...
                self%ierr = omp_target_memcpy_f(wallprop(:,:,:) , wallprop_gpu(:,:,:),0,0,self%base_omp%myhost,self%base_omp%mydev)
              endif
            enddo
          endif
      ...
      if (time_from_last_tspec >= self%equation_base%dt_tspec) then
        if(self%equation_base%enable_tspec > 0) then
          ...
          if(it_win1_tspec == ndft .or. it_win2_tspec == ndft) then
            ...
            self%ierr = omp_target_memcpy_f(w_psd_tspec , w_psd_tspec_gpu,0,0,self%base_omp%myhost,self%base_omp%mydev)
...
endmodule streams_equation_singleideal_omp_object
