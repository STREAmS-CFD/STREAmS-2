...
contains
  ...
  subroutine visflx(self, mode)
    ...
    & dzitdz2_gpu => self%base_gpu%dzitdz2_gpu, x_gpu => self%base_gpu%x_gpu, y_gpu => self%base_gpu%y_gp&
    &u, z_gpu => self%base_gpu%z_gpu, eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_&
    ...
    &alorically_perfect, t0 => self%equation_base%t0, channel_case => self%equation_base%channel_case)
      ...
      if (mode == 0) then
        ...
        & calorically_perfect, u0, l0, self%base_gpu%w_gpu, self%w_aux_gpu, self%fl_gpu, coeff_deriv1_gpu,&
  ...
  subroutine force_var(self)
    ...
    & yn_gpu => self%base_gpu%yn_gpu, w_gpu => self%base_gpu%w_gpu, w_aux_gpu => self%w_aux_gpu,&
    & fln_gpu => self%fln_gpu, volchan => self%equation_base%volchan, fluid_mask_gpu => self%fluid_mask_g&
    &pu, cv_coeff_gpu => self%cv_coeff_gpu, t0 => self%equation_base%t0, calorically_perfect => self%equa&
  ...
  subroutine bc_nr(self)
    ...
    &dy_gpu, dzitdz_gpu => self%base_gpu%dzitdz_gpu, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_pe&
  ...
  subroutine update_ghost(self, do_swap)
    ...
    do ilat=1,6
      ...
      case(5)
        call bcsym_cuf(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%t_wall,&
        & self%base_gpu%w_gpu, self%w_aux_gpu, self%equation_base%indx_cp_l, self%equation_base%indx_cp_r,&
        & self%cv_coeff_gpu, self%equation_base%calorically_perfect)
      case(6)
  ...
  subroutine bcrecyc(self, ilat)
    ...
    if (ilat == 1) then
      ...
      & cv_coeff_gpu => self%cv_coeff_gpu, inflow_random_plane => self%equation_base%inflow_random_plane,&
      & inflow_random_plane_gpu => self%inflow_random_plane_gpu)
  ...
  subroutine compute_dt(self)
    ...
    &gpu, dzitdzs_gpu => self%base_gpu%dzitdzs_gpu, calorically_perfect => self%equation_base%calorically&
  ...
  subroutine manage_output(self)
    ...
    & time => self%equation_base%time, w_aux_gpu => self%w_aux_gpu, ijk_probe_gpu => self%ijk_probe_gpu,&
    & w_aux_probe_gpu => self%w_aux_probe_gpu, probe_coeff_gpu => self%probe_coeff_gpu,&
