...
!               NUOVO
                if(nkeep == 0) then
                  ...
                  drho = 2._rkind*(rhoip-rhoi)/rhom
                  dee = 2._rkind*(eeip - eei)/eem
!                 RIMETTEREt_sumdenrho = (0.25_rkind*drho*drho)
!                 RIMETTEREt_sumdenee = (0.25_rkind*dee *dee )
...
!         call wenorec_x(i,j,nv,nx,ny,gplus_x_gpu,gminus_x_gpu,fi,weno_scheme,wenorec_ord,weno_version,rho0,u0)
...
!
    ...
    &, nx, ny, nz, ng, eul_imin, eul_imax, lmax_base, nkeep, rgas0, w_aux_gpu, coeff_deriv1_gpu, dcsidx_g&
    &pu, fhat_gpu, force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold,&
...
!
    attributes(global) launch_bounds(256) subroutine euler_z_hybrid_kernel(nv, nv_aux, nx, ny, nz, n&
    &g, eul_kmin, eul_kmax, lmax_base, nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, dzitdz_gpu, fha&
    &t_gpu, force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_s&
    &ize, cp_coeff_gpu, indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,rho0,u0&
    &,t0)
      implicit none
!     Passed arguments
      integer, value :: nv, nx, ny, nz, ng, nv_aux
...
!
    attributes(global) launch_bounds(256) subroutine euler_z_hybrid_rusanov_kernel(nv, nv_aux, nx, n&
    &y, nz, ng, eul_kmin, eul_kmax, lmax_base, nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, dzitdz_&
    &gpu, fhat_gpu, force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold&
    &, weno_size, cp_coeff_gpu, indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr&
    &,rho0,u0,t0)
      implicit none
!     Passed arguments
...
!
    attributes(global) launch_bounds(256) subroutine euler_y_hybrid_kernel(nv, nv_aux, nx, ny, nz, n&
    &g, eul_jmin, eul_jmax, lmax_base, nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, detady_gpu, fha&
    &t_gpu, force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_s&
    &ize, cp_coeff_gpu, indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,rho0,u0&
    &,t0)
      implicit none
...
!
    attributes(global) launch_bounds(256) subroutine euler_y_hybrid_rusanov_kernel(nv, nv_aux, nx, n&
    &y, nz, ng, eul_jmin, eul_jmax, lmax_base, nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, detady_&
    &gpu, fhat_gpu, force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold&
    &, weno_size, cp_coeff_gpu, indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr&
    &,rho0,u0,t0)
      implicit none
...
!           
            ...
            enddo
!           
            vminus = betap(2)*(c0*vp(m,i )+c1*vp(m,i+1)+c2*vp(m,i+2)) + betap(1)*(c2*vp(m,i-1)+c1*vp&
            &(m,i )+c0*vp(m,i+1)) + betap(0)*(c0*vp(m,i-2)+c3*vp(m,i-1)+c4*vp(m,i ))
            vplus = betam(2)*(c0*vm(m,i+1)+c1*vm(m,i )+c2*vm(m,i-1)) + betam(1)*(c2*vm(m,i+2)+c1*vm(&
            &m,i+1)+c0*vm(m,i )) + betam(0)*(c0*vm(m,i+3)+c3*vm(m,i+2)+c4*vm(m,i+1))
!           
            vhat(m) = vminus+vplus
!           
          enddo ! end of m-loop
...
!       
        do m=1,nvar
!         
          betap(3)= d1*(-11*vp(m, i)+18*vp(m,i+1)- 9*vp(m,i+2)+ 2*vp(m,i+3))**2+d2*( 2*vp(m, i)- 5*v&
          &p(m,i+1)+ 4*vp(m,i+2)- vp(m,i+3))**2+ d3*( -vp(m, i)+ 3*vp(m,i+1)- 3*vp(m,i+2)+ vp(m,i+3))**2
          ...
          betap(0)= d1*(- 2*vp(m,i-3)+ 9*vp(m,i-2)-18*vp(m,i-1)+11*vp(m,i ))**2+d2*(- vp(m,i-3)+ 4*v&
          &p(m,i-2)- 5*vp(m,i-1)+ 2*vp(m,i ))**2+d3*( -vp(m,i-3)+ 3*vp(m,i-2)- 3*vp(m,i-1)+ vp(m,i ))**2
...
!         
          ...
          betam(1)= d1*( vm(m,i+3)- 6*vm(m,i+2)+ 3*vm(m,i+1)+ 2*vm(m,i ))**2+d2*( vm(m,i+2)- 2*vm(m,&
          &i+1)+ vm(m,i ))**2+d3*( -vm(m,i+3)+ 3*vm(m,i+2)- 3*vm(m,i+1)+ vm(m,i ))**2
...
!         
          ...
          &2)+14*vm(m,i+1)+14*vm(m,i )- 2*vm(m,i-1))+betam(1)*( 2*vm(m,i+3)-10*vm(m,i+2)+26*vm(m,i+1)+ 6*vm(m,i&
...
!
    ...
    &, rgas0, b1, b2, b3, c, ci, h, uu, vv, ww, cp_coeff_gpu, indx_cp_l, indx_cp_r,calorically_perfect,to&
    &l_iter_nr,t0)
...
!
    subroutine force_var_1_cuf(nx, ny, nz, ng, yn_gpu, fln_gpu, w_gpu, w_aux_gpu, bulkt, fluid_mask_&
    &gpu, cv_coeff_gpu, indx_cp_l, indx_cp_r, t0, calorically_perfect, tol_iter_nr)
        ...
        do j=1,ny
          do i=1,nx
            if (fluid_mask_gpu(i,j,k)==0) then
              ...
              qq = 0.5_rkind*(uu*uu+vv*vv+ww*ww)
              ee = rhoe/rho-qq
              tt = get_temperature_from_e_dev(ee,w_aux_gpu(i,j,k,6),t0,cv_coeff_gpu,indx_cp_l,indx_c&
...
!   
    subroutine force_var_2_cuf(nx, ny, nz, ng, w_gpu, w_aux_gpu, tbdiff, fluid_mask_gpu, cv_coeff_gp&
    &u, indx_cp_l, indx_cp_r, t0, calorically_perfect)
...
!
    subroutine visflx_div_cuf(nx, ny, nz, ng, visc_order, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, dcsid&
    &x_gpu, detady_gpu, dzitdz_gpu, stream_id)
!
...
!
    subroutine visflx_cuf(nx, ny, nz, ng, visc_order, Prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gp&
    &u, calorically_perfect, u0, l0, w_gpu, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, coeff_deriv2_gpu, dcsidx&
    &_gpu, detady_gpu, dzitdz_gpu, dcsidxs_gpu, detadys_gpu, dzitdzs_gpu, dcsidx2_gpu, detady2_gpu, dzitd&
    &z2_gpu, wallprop_gpu)
!
...
!
    ...
    &, cp_coeff_gpu, calorically_perfect, u0, l0, w_gpu, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, coeff_deriv&
    &2_gpu, dcsidx_gpu, detady_gpu, dzitdz_gpu, dcsidxs_gpu, detadys_gpu, dzitdzs_gpu, dcsidx2_gpu, detad&
    &y2_gpu, dzitdz2_gpu, wallprop_gpu)
!
      integer, intent(in) :: nx, ny, nz, ng, visc_order, calorically_perfect
      integer, intent(in) :: indx_cp_l, indx_cp_r
      real(rkind), intent(in) :: Prandtl, u0, l0, t0
...
!
    subroutine visflx_nosensor_cuf(nx, ny, nz, ng, visc_order, Prandtl, t0, indx_cp_l, indx_cp_r, cp&
    &_coeff_gpu, calorically_perfect, u0, l0, w_gpu, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, coeff_deriv2_gp&
    &u, dcsidx_gpu, detady_gpu, dzitdz_gpu, dcsidxs_gpu, detadys_gpu, dzitdzs_gpu, dcsidx2_gpu, detady2_g&
    &pu, dzitdz2_gpu, wallprop_gpu)
!
      integer, intent(in) :: nx, ny, nz, ng, visc_order, calorically_perfect
      integer, intent(in) :: indx_cp_l, indx_cp_r
      real(rkind), intent(in) :: Prandtl, u0, l0, t0
...
!
    subroutine visflx_reduced_ord2_cuf(nx, ny, nz, ng, Prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_g&
    &pu, calorically_perfect, u0, l0, w_gpu, w_aux_gpu, fl_gpu, x_gpu, y_gpu, z_gpu, wallprop_gpu, update&
    &_sensor)
!
...
!
    subroutine visflx_reduced_cuf(nx, ny, nz, ng, visc_order, Prandtl, t0, indx_cp_l, indx_cp_r, cp_&
    &coeff_gpu, calorically_perfect, u0, l0, w_gpu, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, coeff_deriv2_gpu&
    &, dcsidx_gpu, detady_gpu, dzitdz_gpu, dcsidxs_gpu, detadys_gpu, dzitdzs_gpu, dcsidx2_gpu, detady2_gp&
    &u, dzitdz2_gpu, wallprop_gpu, update_sensor)
!
      integer, intent(in) :: nx, ny, nz, ng, visc_order, calorically_perfect
      integer, intent(in) :: indx_cp_l, indx_cp_r
      real(rkind), intent(in) :: Prandtl, u0, l0, t0
...
!
    subroutine visflx_x_cuf(nx, ny, nz, nv, ng, Prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gpu, cal&
    &orically_perfect, x_gpu, w_aux_gpu, fl_gpu, fhat_gpu)
!
...
!
    subroutine visflx_y_cuf(nx, ny, nz, nv, ng, Prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gpu, cal&
    &orically_perfect, y_gpu, w_aux_gpu, fl_gpu)
!
      integer, intent(in) :: nx, ny, nz, nv, ng, calorically_perfect
      integer, intent(in) :: indx_cp_l, indx_cp_r
      real(rkind), intent(in) :: Prandtl, t0
...
!   
    subroutine visflx_z_cuf(nx, ny, nz, nv, ng, Prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gpu, cal&
    &orically_perfect, z_gpu, w_aux_gpu, fl_gpu)
!
      integer, intent(in) :: nx, ny, nz, nv, ng, calorically_perfect
      integer, intent(in) :: indx_cp_l, indx_cp_r
      real(rkind), intent(in) :: Prandtl, t0
...
!
!
    attributes(global) subroutine bc_nr_lat_x_kernel(start_or_end, nr_type, nx, ny, nz, ng, nv, w_au&
    &x_gpu, w_gpu, fl_gpu, dcsidx_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, calorically_perfect,&
    & rgas0,t0)
...
!
    attributes(global) subroutine bc_nr_lat_y_kernel(start_or_end, nr_type, nx, ny, nz, ng, nv, w_au&
    &x_gpu, w_gpu, fl_gpu, detady_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, calorically_perfect,&
    &rgas0,t0)
      integer, intent(in), value :: start_or_end, nr_type, nx, ny, nz, ng, nv, indx_cp_l, indx_cp_r, calorically_perfect
      real(rkind), intent(in), value :: rgas0,t0
      real(rkind), dimension(3) :: c_one
...
!
    attributes(global) subroutine bc_nr_lat_z_kernel(start_or_end, nr_type, nx, ny, nz, ng, nv, w_au&
    &x_gpu, w_gpu, fl_gpu, dzitdz_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, calorically_perfect,&
    &rgas0,t0)
      integer, intent(in), value :: start_or_end, nr_type, nx, ny, nz, ng, nv, indx_cp_l, indx_cp_r, calorically_perfect
      real(rkind), intent(in), value :: rgas0,t0
      real(rkind), dimension(3) :: c_one
...
!
    subroutine bcrecyc_cuf_3(nx, ny, nz, ng, p0, u0, rgas0, w_gpu, wmean_gpu, wrecyc_gpu, weta_inflo&
    &w_gpu, map_j_inn_gpu, map_j_out_gpu, yplus_inflow_gpu, eta_inflow_gpu, yplus_recyc_gpu, eta_recyc_gp&
    &u, betarecyc, inflow_random_plane_gpu, indx_cp_l, indx_cp_r, cv_coeff_gpu, t0, calorically_perfect, &
    &rand_type)
...
!
    subroutine bcshock_cuf(ilat, nx, ny, nz, ng, nv, w_gpu, winf_gpu, winf_past_shock_gpu, xshock_im&
    &p, shock_angle, x_gpu, y_gpu, tanhfacs)
...
!
    subroutine bcwall_cuf(ilat, nx, ny, nz, ng, twall, w_gpu, w_aux_gpu, indx_cp_l, indx_cp_r, cv_co&
    &eff_gpu, t0, rgas0, calorically_perfect, tol_iter_nr)
      integer :: nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect
      integer :: ilat
...
!           w_gpu(i,1,k,5) = w_gpu(i,1,k,1)*gm*twall
            ...
            do l=1,ng
              ...
              qq = 0.5_rkind*(uu*uu+vv*vv+ww*ww)
              ee = rhoe/rho-qq
              tt = get_temperature_from_e_dev(ee,w_aux_gpu(i,1+l,k,6),t0,cv_coeff_gpu,indx_cp_l,indx&
              &_cp_r,calorically_perfect,tol_iter_nr)
              pp = rho*tt*rgas0
...
!
    subroutine bcwall_staggered_cuf(ilat, nx, ny, nz, ng, twall, w_gpu, w_aux_gpu, indx_cp_l, indx_c&
    &p_r, cv_coeff_gpu, t0, rgas0, calorically_perfect, tol_iter_nr)
      integer :: nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect
      integer :: ilat
      real(rkind) :: twall, t0, rgas0, tol_iter_nr
      integer :: i,k,l, iercuda
        ...
        do k=1,nz
          do l=1,ng
            do i=1,nx
              ...
              qq = 0.5_rkind*(uu*uu+vv*vv+ww*ww)
              ee = rhoe/rho-qq
              tt = get_temperature_from_e_dev(ee,w_aux_gpu(i,l,k,6),t0,cv_coeff_gpu,indx_cp_l,indx_c&
              &p_r,calorically_perfect,tol_iter_nr)
              pp = rho*tt*rgas0
              tt = 2._rkind*twall-tt ! bc
              ee = get_e_from_temperature_dev(tt, t0, indx_cp_l, indx_cp_r, cv_coeff_gpu,calorically_perfect)
        ...
        do k=1,nz
          do l=1,ng
            do i=1,nx
              ...
              qq = 0.5_rkind*(uu*uu+vv*vv+ww*ww)
              ee = rhoe/rho-qq
              tt = get_temperature_from_e_dev(ee,w_aux_gpu(i,ny+1-l,k,6),t0,cv_coeff_gpu,indx_cp_l,i&
...
!             vmax = min(vmax,w_gpu(i,j,k,3)/w_gpu(i,j,k,1))
            endif
          enddo
        enddo
      enddo
...
!
!
    subroutine compute_dt_cuf(nx, ny, nz, ng, rgas0, Prandtl, dcsidx_gpu, detady_gpu, dzitdz_gpu, dc&
    &sidxs_gpu, detadys_gpu, dzitdzs_gpu, w_gpu, w_aux_gpu, dtxi_max, dtyi_max, dtzi_max, dtxv_max, dtyv_&
    &max, dtzv_max, dtxk_max, dtyk_max, dtzk_max, indx_cp_l, indx_cp_r, cp_coeff_gpu,fluid_mask_gpu,calor&
    &ically_perfect,t0)
      integer :: nx, ny, nz, ng, indx_cp_l, indx_cp_r, calorically_perfect
      real(rkind) :: rgas0, t0
      real(rkind) :: Prandtl
...
!
    subroutine eval_aux_cuf(nx, ny, nz, ng, istart, iend, jstart, jend, kstart, kend, w_gpu, w_aux_g&
    &pu, visc_model, mu0, t0, sutherland_S, T_ref_dim, powerlaw_vtexp, VISC_POWER, VISC_SUTHERLAND, VISC_&
    &NO, cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr, stream_id)
...
!           w_aux(:) : rho, u, v, w, h, T, viscosity, div, |omega|, ducros
            ...
            qq = 0.5_rkind*(uu*uu+vv*vv+ww*ww)
            ee = rhoe/rho-qq
            tt = get_temperature_from_e_dev(ee, w_aux_gpu(i,j,k,6), t0, cv_coeff_gpu, indx_cp_l, ind&
...
!
    subroutine ibm_interpolation_cuf(ibm_num_interface,nx,ny,nz,ng,indx_cp_l,indx_cp_r,ibm_ijk_refl_&
    &gpu,ibm_refl_type_gpu,w_gpu,w_aux_gpu,ibm_is_interface_node_gpu,ibm_coeff_d_gpu,ibm_coeff_n_gpu,ibm_&
    &bc_gpu,cv_coeff_gpu,ibm_w_refl_gpu,ibm_twall,ibm_nxyz_interface_gpu,calorically_perfect,rgas0)
!     
      integer, intent(in) :: ibm_num_interface,nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect
      real(rkind), intent(in) :: ibm_twall, rgas0
      real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(in), device :: w_gpu
      real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(in), device :: w_aux_gpu
...
!     
      integer :: l,i,j,k,ii,jj,kk,iercuda
      real(rkind) :: wi1,wi2,wi3,wi4,wi5,wi6
...
!           
            do kk=1,2
              do jj=1,2
                do ii=1,2
                  wi1 = wi1 + ibm_coeff_d_gpu(ii,jj,kk,l)*w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,1)
                  wi2 = wi2 + ibm_coeff_d_gpu(ii,jj,kk,l)*w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,2)
                  wi3 = wi3 + ibm_coeff_d_gpu(ii,jj,kk,l)*w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,3)
                  wi4 = wi4 + ibm_coeff_d_gpu(ii,jj,kk,l)*w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,4)
                  wi5 = wi5 + ibm_coeff_d_gpu(ii,jj,kk,l)*w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,6)
                enddo
              enddo
            enddo
!           
...
!             
              do kk=1,2
                do jj=1,2
                  do ii=1,2
!                   
                    rho = w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,1)
                    uu = w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,2)
                    vv = w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,3)
                    ww = w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,4)
                    tt = w_aux_gpu(i+ii-1,j+jj-1,k+kk-1,6)
                    pp = rho*tt*rgas0
...
!                   
                    if (ibm_is_interface_node_gpu(i+ii-1,j+jj-1,k+kk-1)==1) then
                      ...
                      wi5 = wi5 + ibm_coeff_d_gpu(ii,jj,kk,l)*ibm_twall
                    else
...
!             
              wi1 = wi6/wi5/rgas0 ! rho = p/tt
!             
            case(8) ! Viscous adiabatic wall (u,v,w => D, p,T => N)
!
              do kk=1,2
                do jj=1,2
                  do ii=1,2
...
!             
              wi1 = wi6/wi5/rgas0 ! rho = p/tt
!             
            case(9) ! Supersonic inflow (no interpolation needed because ibm_w_refl_gpu not used in forcing)
!             
            endselect type_bc
!           
          endif type_refl
!         
          ibm_w_refl_gpu(l,1) = wi1
          ibm_w_refl_gpu(l,2) = wi2
          ibm_w_refl_gpu(l,3) = wi3
          ibm_w_refl_gpu(l,4) = wi4
          ibm_w_refl_gpu(l,5) = wi5
!         
        enddo
        !@cuf iercuda=cudaDeviceSynchronize()
      endif
...
!   
    subroutine ibm_forcing_cuf(ibm_num_interface,nx,ny,nz,ng,indx_cp_l,indx_cp_r,ibm_ijk_interface_g&
    &pu,w_gpu,w_aux_gpu,ibm_bc_gpu, cv_coeff_gpu,ibm_w_refl_gpu, ibm_nxyz_interface_gpu,ibm_aero_rad,ibm_&
    &aero_pp,ibm_aero_tt, ibm_aero_modvel,x_gpu,y_gpu,z_gpu,ibm_twall, ibm_bc_relax_factor,t0,calorically&
    &_perfect,rgas0, ibm_parbc_gpu, ibm_num_bc)
!     
      integer, intent(in) :: ibm_num_interface,nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect, ibm_num_bc
      real(rkind), intent(in) :: ibm_aero_rad,ibm_aero_pp,ibm_aero_tt,ibm_aero_modvel,ibm_twall,ibm_bc_relax_factor,t0,rgas0
      integer, dimension(1:,1:), intent(in), device :: ibm_ijk_interface_gpu
      real(rkind), dimension(1:,1:), intent(in), device :: ibm_nxyz_interface_gpu
      integer, dimension(1:,1:), intent(in), device :: ibm_bc_gpu
      real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu
      real(rkind), dimension(1:,1:), intent(in), device :: ibm_w_refl_gpu
      real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(inout), device :: w_gpu
      real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(inout), device :: w_aux_gpu
      real(rkind), dimension(1-ng:), intent(in), device :: x_gpu,y_gpu,z_gpu
      real(rkind), dimension(1:,1:), intent(in), device :: ibm_parbc_gpu
!     
      integer :: l,i,j,k,iercuda
      integer :: indx_patch
      real(rkind) :: rho,uu,vv,ww,pp,tt
...
!         Quantities at reflected node
          ...
          uu = ibm_w_refl_gpu(l,2)
          vv = ibm_w_refl_gpu(l,3)
          ww = ibm_w_refl_gpu(l,4)
          tt = ibm_w_refl_gpu(l,5)
          pp = rho*tt*rgas0
!         
          type_bc: select case(ibm_bc_gpu(1,l))
!           
...
!           rad = sqrt(y_gpu(j)**2+z_gpu(k)**2)
            if (rad<ibm_aero_rad) then
              ...
              rhoi = w_aux_gpu(i,j,k,1)
              uui = w_aux_gpu(i,j,k,2)
              vvi = w_aux_gpu(i,j,k,3)
              wwi = w_aux_gpu(i,j,k,4)
              tti = w_aux_gpu(i,j,k,6)
              ppi = rhoi*tti*rgas0
            endif
!           
          end select type_bc
...
!         
          ...
          rhovi = rhoi*vvi
          rhowi = rhoi*wwi
          eei = get_e_from_temperature_dev(tti, t0, indx_cp_l, indx_cp_r, cv_coeff_gpu, calorically_perfect)
          w_gpu(i,j,k,1) = rhoi
          w_gpu(i,j,k,2) = rhoui
          w_gpu(i,j,k,3) = rhovi
          w_gpu(i,j,k,4) = rhowi
          w_gpu(i,j,k,5) = rhoi*eei+0.5_rkind*(rhoui*rhoui+rhovi*rhovi+rhowi*rhowi)/rhoi
!         
        enddo
        !@cuf iercuda=cudaDeviceSynchronize()
!       
...
!
    subroutine ibm_inside_moving_cuf(nx,ny,nz,ng,ibm_inside_moving_gpu, x_gpu,y_gpu,z_gpu,ibm_vega_n&
    &y, ibm_vega_dy,ibm_vega_y_gpu,ibm_vega_r_gpu, ep_ord_change_gpu,ibm_order_reduce)
      integer, intent(in) :: nx,ny,nz,ng,ibm_vega_ny,ibm_order_reduce
      real(rkind), intent(in) :: ibm_vega_dy
      integer, dimension(1:,1:,1:), intent(inout), device :: ibm_inside_moving_gpu
      real(rkind), dimension(1-ng:), intent(in), device :: x_gpu,y_gpu,z_gpu
      real(rkind), dimension(1:ibm_vega_ny+1), intent(in), device :: ibm_vega_y_gpu,ibm_vega_r_gpu
      integer, dimension(0:,0:,0:,1:), intent(inout), device :: ep_ord_change_gpu
      real(rkind) :: yvegamin,yvegamax,rad,rv,drv
      integer :: i,j,k,iercuda,jj
!     
      yvegamin = ibm_vega_y_gpu(1)
      yvegamax = ibm_vega_y_gpu(ibm_vega_ny+1)
      !$cuf kernel do(3) <<<*,*>>>
      do k=1,nz
        do j=1,ny
          do i=1,nx
            ibm_inside_moving_gpu(i,j,k) = 0
            if (y_gpu(j)<yvegamax.and.y_gpu(j)>yvegamin) then
              jj = 1+int((y_gpu(j)-yvegamin)/ibm_vega_dy)
              drv = ibm_vega_r_gpu(jj+1)-ibm_vega_r_gpu(jj)
              rv = ibm_vega_r_gpu(jj)+drv/ibm_vega_dy*(y_gpu(j)-ibm_vega_y_gpu(jj))
              rad = sqrt(x_gpu(i)**2+z_gpu(k)**2)
              if (rad<rv) then
                ibm_inside_moving_gpu(i,j,k) = 1
                ep_ord_change_gpu(i,j,k,1) = ibm_order_reduce
                ep_ord_change_gpu(i,j,k,2) = ibm_order_reduce
                ep_ord_change_gpu(i,j,k,3) = ibm_order_reduce
                if ((y_gpu(j)-yvegamin)<0.2715_rkind) ibm_inside_moving_gpu(i,j,k) = 9
              endif
            endif
          enddo
        enddo
      ...
      !@cuf iercuda=cudaDeviceSynchronize()
!
    end subroutine ibm_inside_moving_cuf
!   
    subroutine ibm_forcing1_cuf(nx,ny,nz,ng,indx_cp_l,indx_cp_r,w_gpu,ibm_inside_moving_gpu, cv_coef&
    &f_gpu,ibm_aero_rad,ibm_aero_pp,ibm_aero_tt, ibm_vega_vel, ibm_aero_modvel,x_gpu,y_gpu,z_gpu,ibm_bc_r&
    &elax_factor,t0,calorically_perfect,rgas0)
      integer :: nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect
      real(rkind), intent(in) :: ibm_aero_rad,ibm_aero_pp,ibm_aero_tt,ibm_aero_modvel,ibm_bc_relax_factor,t0,rgas0
...
!               ww = ibm_aero_modvel*ibm_nxyz_interface_gpu(3,l)
                ...
                w_gpu(i,j,k,3) = rhov
                w_gpu(i,j,k,4) = rhow
                w_gpu(i,j,k,5) = rho*ee+0.5_rkind*(rhou*rhou+rhov*rhov+rhow*rhow)/rho
              endif
!             
...
!             
              ...
              w_gpu(i,j,k,5) = rhoe-rho*qq+0.5_rkind*rho*vv*vv
!             
            endif
!           
          enddo
      ...
      !@cuf iercuda=cudaDeviceSynchronize()
!     
    end subroutine ibm_forcing1_cuf
!
    subroutine ibm_compute_force_cuf(ibm_num_interface,nx,ny,nz,ng,ibm_ijk_interface_gpu,fln_gpu,w_g&
    &pu,w_aux_gpu,ibm_bc_gpu,dcsidx_gpu,detady_gpu,dzitdz_gpu,ibm_force_x_s,ibm_force_y_s,ibm_force_z_s,f&
    &luid_mask_gpu)
!     
...
!         w4 = w1*w_aux_gpu(i,j,k,4)
!         ibm_force_x = ibm_force_x+(w_gpu(i,j,k,2)-w2)*dvol
!         ibm_force_y = ibm_force_y+(w_gpu(i,j,k,3)-w3)*dvol
!         ibm_force_z = ibm_force_z+(w_gpu(i,j,k,4)-w4)*dvol
!         !
!       enddo
!       !@cuf iercuda=cudaDeviceSynchronize()
!       
        ...
        do k=1,nz
          do j=1,ny
            do i=1,nx
              if (fluid_mask_gpu(i,j,k)/=0) then
                dvol = 1._rkind/(dcsidx_gpu(i)*detady_gpu(j)*dzitdz_gpu(k))
                ibm_force_x_s = ibm_force_x_s+(-fln_gpu(i,j,k,2)*dvol)
                ibm_force_y_s = ibm_force_y_s+(-fln_gpu(i,j,k,3)*dvol)
                ibm_force_z_s = ibm_force_z_s+(-fln_gpu(i,j,k,4)*dvol)
              endif
            enddo
          enddo
        enddo
...
!
    attributes(global) launch_bounds(256) subroutine insitu_swirling_kernel(nv, nx, ny, nz, visc_ord&
    &er, ng, npsi,mpsi,dcsidx_gpu, detady_gpu, dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, psi_gpu, u0, x_gp&
    &u )
!     
...
!
    subroutine insitu_div_cuf(nx, ny, nz, ng, visc_order, npsi,mpsi, w_aux_gpu, coeff_deriv1_gpu, dc&
    &sidx_gpu, detady_gpu, dzitdz_gpu, psi_gpu )
!
...
!
    subroutine insitu_ducros_cuf(nx, ny, nz, ng, visc_order, npsi,mpsi, u0,l0, w_aux_gpu, coeff_deri&
    &v1_gpu, dcsidx_gpu, detady_gpu, dzitdz_gpu, psi_gpu )
!
      integer, intent(in) :: nx, ny, nz, ng, visc_order, npsi, mpsi
      real(rkind), intent(in) :: u0, l0
      real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_aux_gpu
...
!
!   NOMANAGEDsubroutine copy_to_psi_pv_managed_cuf(nxsl_ins,nxel_ins,nysl_ins,nyel_ins,nzsl_ins,nzel
!_ins, !NOMANAGED ng,nx,ny,nz,npsi, npsi_pv, nv_aux, n_aux_list, !NOMANAGED psi_gpu,psi_pv_managed,w_
!aux_gpu,aux_list_gpu)
