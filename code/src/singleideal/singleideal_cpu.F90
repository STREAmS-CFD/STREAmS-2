...
!
  subroutine visflx(self, mode)
    ...
    &law_vtexp => self%powerlaw_vtexp, coeff_deriv1_cpu => self%coeff_deriv1_cpu, coeff_deriv2_cpu => sel&
    &f%coeff_deriv2_cpu, fhat_trans_cpu => self%fhat_trans_cpu, fl_trans_cpu => self%fl_trans_cpu, fl_cpu&
    & => self%fl_cpu, w_aux_cpu => self%w_aux_cpu, w_aux_trans_cpu => self%w_aux_trans_cpu, dcsidx_cpu =>&
    & self%base_cpu%dcsidx_cpu, detady_cpu => self%base_cpu%detady_cpu, dzitdz_cpu => self%base_cpu%dzitd&
    &z_cpu, dcsidxs_cpu => self%base_cpu%dcsidxs_cpu, detadys_cpu => self%base_cpu%detadys_cpu, dzitdzs_c&
    &pu => self%base_cpu%dzitdzs_cpu, dcsidx2_cpu => self%base_cpu%dcsidx2_cpu, detady2_cpu => self%base_&
    &cpu%detady2_cpu, dzitdz2_cpu => self%base_cpu%dzitdz2_cpu, x_cpu => self%base_cpu%x_cpu, y_cpu => se&
    &lf%base_cpu%y_cpu, z_cpu => self%base_cpu%z_cpu, eul_imin => self%equation_base%eul_imin, eul_imax =&
    ...
    &eff_cpu => self%cv_coeff_cpu, cp_coeff_cpu => self%cp_coeff_cpu, indx_cp_l => self%equation_base%ind&
...
!
      if (mode == 0) then
        call visflx_subroutine(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r, cp_coe&
        &ff_cpu, calorically_perfect, u0, l0, self%base_cpu%w_cpu, self%w_aux_cpu, self%fl_cpu, coeff_deriv1_&
        &cpu, coeff_deriv2_cpu, dcsidx_cpu, detady_cpu, dzitdz_cpu, dcsidxs_cpu, detadys_cpu, dzitdzs_cpu, dc&
        &sidx2_cpu, detady2_cpu, dzitdz2_cpu, self%wallprop_cpu)
      elseif (mode == 1) then
        call visflx_div_subroutine(nx, ny, nz, ng, visc_order, self%w_aux_cpu, self%fl_cpu, coeff_de&
        &riv1_cpu, dcsidx_cpu, detady_cpu, dzitdz_cpu, stream1)
      elseif (mode == 2) then
        call visflx_reduced_ord2_subroutine(nx, ny, nz, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_co&
        &eff_cpu, calorically_perfect, u0, l0, self%base_cpu%w_cpu, self%w_aux_cpu, self%fl_cpu, x_cpu, y_cpu&
        &, z_cpu, self%wallprop_cpu, 1)
      elseif (mode == 3) then
        call sensor_subroutine(nx, ny, nz, ng, u0, l0, self%w_aux_cpu)
      elseif (mode == 4) then
        call visflx_nosensor_subroutine(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_&
        &r, cp_coeff_cpu, calorically_perfect, u0, l0, self%base_cpu%w_cpu, self%w_aux_cpu, self%fl_cpu, coef&
        &f_deriv1_cpu, coeff_deriv2_cpu, dcsidx_cpu, detady_cpu, dzitdz_cpu, dcsidxs_cpu, detadys_cpu, dzitdz&
        &s_cpu, dcsidx2_cpu, detady2_cpu, dzitdz2_cpu, self%wallprop_cpu)
      elseif (mode == 5) then
        call visflx_x_subroutine(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_cpu&
        &, calorically_perfect, self%base_cpu%x_cpu, w_aux_cpu, self%fl_cpu, self%fhat_cpu)
        call visflx_y_subroutine(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_cpu&
        &, calorically_perfect, self%base_cpu%y_cpu, self%w_aux_cpu, self%fl_cpu)
        call visflx_z_subroutine(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_cpu&
        &, calorically_perfect, self%base_cpu%z_cpu, self%w_aux_cpu, self%fl_cpu)
      elseif (mode == 6) then
        call visflx_reduced_ord2_subroutine(nx, ny, nz, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_co&
        &eff_cpu, calorically_perfect, u0, l0, self%base_cpu%w_cpu, self%w_aux_cpu, self%fl_cpu, x_cpu, y_cpu&
        &, z_cpu, self%wallprop_cpu, 0)
      elseif (mode == 7) then
...
!
  subroutine compute_aux(self, central, ghost)
    ...
    &%sutherland_s, powerlaw_vtexp => self%powerlaw_vtexp, cv_coeff_cpu => self%cv_coeff_cpu, cp_coeff_cp&
    &u => self%cp_coeff_cpu, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%i&
    ...
    &%rgas0)
      if(central_ == 1 .and. ghost_ == 1) then
        call eval_aux_subroutine(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, ny+ng, 1-ng, nz+ng, self%base_cp&
        &u%w_cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, v&
        &isc_sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr&
        &,stream1)
      elseif(central_ == 1 .and. ghost_ == 0) then
        call eval_aux_subroutine(nx, ny, nz, ng, 1, nx, 1, ny, 1, nz, self%base_cpu%w_cpu, self%w_au&
        &x_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, visc_sutherland, vi&
        &sc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr,stream1)
      elseif(central_ == 0 .and. ghost_ == 1) then
        call eval_aux_subroutine(nx, ny, nz, ng, 1-ng, 0, 1-ng, ny+ng, 1-ng, nz+ng, self%base_cpu%w_&
        &cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, visc_&
        &sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr,str&
        &eam1)
        call eval_aux_subroutine(nx, ny, nz, ng, nx+1, nx+ng, 1-ng, ny+ng, 1-ng, nz+ng, self%base_cp&
        &u%w_cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, v&
        &isc_sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr&
        &,stream1)
        call eval_aux_subroutine(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, 0, 1-ng, nz+ng, self%base_cpu%w_&
        &cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, visc_&
        &sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr,str&
        &eam1)
        call eval_aux_subroutine(nx, ny, nz, ng, 1-ng, nx+ng, ny+1, ny+ng, 1-ng, nz+ng, self%base_cp&
        &u%w_cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, v&
        &isc_sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr&
        &,stream1)
        call eval_aux_subroutine(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, ny+ng, 1-ng, 0, self%base_cpu%w_&
        &cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, visc_&
        &sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr,str&
        &eam1)
        call eval_aux_subroutine(nx, ny, nz, ng, 1-ng, nx+ng, 1-ng, ny+ng, nz+1, nz+ng, self%base_cp&
        &u%w_cpu, self%w_aux_cpu, visc_model, mu0, t0, sutherland_s, t_ref_dim, powerlaw_vtexp, visc_power, v&
        &isc_sutherland, visc_no, cv_coeff_cpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr&
        &,stream1)
      endif
...
!
    ...
    &o_flux, coeff_deriv1_cpu => self%coeff_deriv1_cpu, dcsidx_cpu => self%base_cpu%dcsidx_cpu, fhat_cpu &
    &=> self%fhat_cpu, w_cpu => self%base_cpu%w_cpu, w_aux_cpu => self%w_aux_cpu, fl_cpu => self%fl_cpu, &
    &sensor_threshold => self%equation_base%sensor_threshold, weno_scheme => self%equation_base%weno_sche&
    &me, weno_version => self%equation_base%weno_version, cp_coeff_cpu => self%cp_coeff_cpu, indx_cp_l =>&
    & self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => sel&
    &f%equation_base%calorically_perfect, ep_ord_change_cpu => self%ep_ord_change_cpu, nkeep => self%equa&
...
!
        call euler_x_fluxes_hybrid_rusanov_kernel(nv, nv_aux, nx, ny, nz, ng, istart, iend, lmax, nk&
        &eep, rgas0, w_aux_cpu, coeff_deriv1_cpu, dcsidx_cpu, fhat_cpu, force_zero_flux_min, force_zero_flux_&
        &max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp_r, ep_&
        &ord_change_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u0, self&
        &%equation_base%t0)
      else
!
        call euler_x_fluxes_hybrid_kernel(nv, nv_aux, nx, ny, nz, ng, istart, iend, lmax, nkeep, rga&
        &s0, w_aux_cpu, coeff_deriv1_cpu, dcsidx_cpu, fhat_cpu, force_zero_flux_min, force_zero_flux_max, wen&
        &o_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp_r, ep_ord_chan&
        &ge_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u0, self%equatio&
        &n_base%t0)
      endif
!
      call euler_x_update_subroutine(nx, ny, nz, ng, nv, istart, iend, fhat_cpu, fl_cpu, dcsidx_cpu, stream1)
!
    endassociate
...
!
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv, nv_aux => s&
    &elf%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_base%force_zer&
    &o_flux, coeff_deriv1_cpu => self%coeff_deriv1_cpu, detady_cpu => self%base_cpu%detady_cpu, fhat_cpu &
    &=> self%fhat_cpu, w_aux_cpu => self%w_aux_cpu, fl_cpu => self%fl_cpu, sensor_threshold => self%equat&
    ...
    &dx_cp_r, calorically_perfect => self%equation_base%calorically_perfect, cp_coeff_cpu => self%cp_coef&
    &f_cpu, ep_ord_change_cpu => self%ep_ord_change_cpu, nkeep => self%equation_base%nkeep, flux_splittin&
...
!
      if (flux_splitting==1) then
        call euler_y_hybrid_rusanov_kernel(nv, nv_aux, nx, ny, nz, ng, eul_jmin, eul_jmax, lmax, nke&
        &ep, rgas0, w_aux_cpu, fl_cpu, coeff_deriv1_cpu, detady_cpu, fhat_cpu, force_zero_flux_min, force_zer&
        &o_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp&
        &_r, ep_ord_change_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u&
        &0, self%equation_base%t0)
      else
        call euler_y_hybrid_kernel(nv, nv_aux, nx, ny, nz, ng, eul_jmin, eul_jmax, lmax, nkeep, rgas&
        &0, w_aux_cpu, fl_cpu, coeff_deriv1_cpu, detady_cpu, fhat_cpu, force_zero_flux_min, force_zero_flux_m&
        &ax, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp_r, ep_o&
        &rd_change_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u0, self%&
        &equation_base%t0)
      endif
...
!
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv, nv_aux => s&
    &elf%nv_aux, ep_order => self%equation_base%ep_order, force_zero_flux => self%equation_base%force_zer&
    &o_flux, coeff_deriv1_cpu => self%coeff_deriv1_cpu, dzitdz_cpu => self%base_cpu%dzitdz_cpu, fhat_cpu &
    &=> self%fhat_cpu, w_aux_cpu => self%w_aux_cpu, fl_cpu => self%fl_cpu, sensor_threshold => self%equat&
    &ion_base%sensor_threshold, weno_scheme => self%equation_base%weno_scheme, weno_version => self%equat&
    &ion_base%weno_version, cp_coeff_cpu => self%cp_coeff_cpu, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_pe&
    &rfect, ep_ord_change_cpu => self%ep_ord_change_cpu, nkeep => self%equation_base%nkeep, flux_splittin&
    &g => self%equation_base%flux_splitting, rgas0 => self%equation_base%rgas0, inflow_random_plane => se&
    &lf%equation_base%inflow_random_plane, inflow_random_plane_cpu => self%inflow_random_plane_cpu)
      weno_size = 2*weno_scheme
      lmax = ep_order/2
...
!
      if (flux_splitting==1) then
        call euler_z_hybrid_rusanov_kernel(nv, nv_aux, nx, ny, nz, ng, eul_kmin, eul_kmax, lmax, nke&
        &ep, rgas0, w_aux_cpu, fl_cpu, coeff_deriv1_cpu, dzitdz_cpu, fhat_cpu, force_zero_flux_min, force_zer&
        &o_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp&
        &_r, ep_ord_change_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u&
        &0, self%equation_base%t0)
      else
        call euler_z_hybrid_kernel(nv, nv_aux, nx, ny, nz, ng, eul_kmin, eul_kmax, lmax, nkeep, rgas&
        &0, w_aux_cpu, fl_cpu, coeff_deriv1_cpu, dzitdz_cpu, fhat_cpu, force_zero_flux_min, force_zero_flux_m&
        &ax, weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp_r, ep_o&
        &rd_change_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u0, self%&
        &equation_base%t0)
      endif
...
!
    ...
    &e%grid%nxmax, nzmax => self%equation_base%grid%nzmax, yn => self%equation_base%field%yn, yn_cpu => s&
    &elf%base_cpu%yn_cpu, w_cpu => self%base_cpu%w_cpu, w_aux_cpu => self%w_aux_cpu, fln_cpu => self%fln_&
    &cpu, volchan => self%equation_base%volchan, fluid_mask_cpu => self%fluid_mask_cpu)
!
...
!
    ...
    &e%grid%nxmax, nzmax => self%equation_base%grid%nzmax, yn => self%equation_base%field%yn, yn_cpu => s&
    &elf%base_cpu%yn_cpu, w_cpu => self%base_cpu%w_cpu, w_aux_cpu => self%w_aux_cpu, fln_cpu => self%fln_&
    &cpu, volchan => self%equation_base%volchan, fluid_mask_cpu => self%fluid_mask_cpu, cv_coeff_cpu => s&
    &elf%cv_coeff_cpu, t0 => self%equation_base%t0, calorically_perfect => self%equation_base%calorically&
...
!
      if (self%equation_base%theta_wall>=-1._rkind) then
        call force_var_1_subroutine(nx, ny, nz, ng, yn_cpu, fln_cpu, w_cpu, w_aux_cpu, bulkt, fluid_&
        &mask_cpu, cv_coeff_cpu, indx_cp_l, indx_cp_r, t0, calorically_perfect, tol_iter_nr)
...
!
        ...
        call force_var_2_subroutine(nx, ny, nz, ng, w_cpu, w_aux_cpu, tbdiff, fluid_mask_cpu, cv_coe&
        &ff_cpu, indx_cp_l, indx_cp_r, t0, calorically_perfect)
      endif
    endassociate
...
!
    ...
    &quation_base%field%nblocks, iermpi => self%mpi_err, w_cpu => self%base_cpu%w_cpu, nv => self%equatio&
    &n_base%nv, wbuf1s_cpu => self%base_cpu%wbuf1s_cpu, wbuf2r_cpu => self%base_cpu%wbuf2r_cpu, wbuf1r_cp&
    &u => self%base_cpu%wbuf1r_cpu, wrecyc_cpu => self%wrecyc_cpu, ibrecyc => self%equation_base%ib_recyc&
...
!
    ...
    &, nv => self%nv, ng => self%ng, w_aux_cpu => self%w_aux_cpu, w_cpu => self%base_cpu%w_cpu, fl_cpu =>&
    & self%fl_cpu, dcsidx_cpu => self%base_cpu%dcsidx_cpu, detady_cpu => self%base_cpu%detady_cpu, dzitdz&
    &_cpu => self%base_cpu%dzitdz_cpu, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equat&
    &ion_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_perfect, rgas0 => self%equ&
    &ation_base%rgas0, t0 => self%equation_base%t0, cp_coeff_cpu => self%cp_coeff_cpu, winf_cpu => self%w&
    &inf_cpu)
!
      do ilat=1,6
...
!
!
            call bc_nr_lat_x_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_cpu, w_&
            &cpu, fl_cpu, dcsidx_cpu, indx_cp_l, indx_cp_r, cp_coeff_cpu, winf_cpu, calorically_perfect,rgas0,t0)
          endif
          if(dir == 2) then
!
!
            call bc_nr_lat_y_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_cpu, w_&
            &cpu, fl_cpu, detady_cpu, indx_cp_l, indx_cp_r, cp_coeff_cpu, winf_cpu, calorically_perfect,rgas0,t0)
          endif
          if(dir == 3) then
!
!
            call bc_nr_lat_z_kernel(start_or_end, bctags_nr(ilat), nx, ny, nz, ng, nv, w_aux_cpu, w_&
            &cpu, fl_cpu, dzitdz_cpu, indx_cp_l, indx_cp_r, cp_coeff_cpu, winf_cpu, calorically_perfect,rgas0,t0)
          endif
        endif
      enddo
    endassociate
...
!
    do ilat=1,6
      ...
      case(4)
        ...
        &self%equation_base%rgas0, self%base_cpu%w_cpu, self%equation_base%indx_cp_l, self%equation_base%indx&
        &_cp_r, self%cv_coeff_cpu, self%equation_base%t0, self%equation_base%calorically_perfect)
      case(5)
        call bcsym_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%t_wall, s&
        &elf%base_cpu%w_cpu, self%w_aux_cpu, self%equation_base%indx_cp_l, self%equation_base%indx_cp_r, self&
        &%cv_coeff_cpu, self%equation_base%calorically_perfect)
      case(6)
        if (self%equation_base%channel_case) then
          call bcwall_staggered_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_b&
          &ase%t_wall, self%base_cpu%w_cpu, self%w_aux_cpu, self%equation_base%indx_cp_l, self%equation_base%in&
          &dx_cp_r, self%cv_coeff_cpu, self%equation_base%t0, self%equation_base%rgas0, self%equation_base%calo&
          &rically_perfect,tol_iter_nr)
        else
          call bcwall_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%t_wall&
          &, self%base_cpu%w_cpu, self%w_aux_cpu, self%equation_base%indx_cp_l, self%equation_base%indx_cp_r, s&
          &elf%cv_coeff_cpu, self%equation_base%t0, self%equation_base%rgas0, self%equation_base%calorically_pe&
          &rfect,tol_iter_nr)
        endif
      case(7)
        call bcshock_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_cpu%w_c&
        &pu, self%winf_cpu, self%winf_past_shock_cpu, self%equation_base%xshock_imp, self%equation_base%shock&
        &_angle, self%base_cpu%x_cpu, self%base_cpu%y_cpu, self%equation_base%tanhfacs)
      case(8)
      case(9)
        call bclam_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%nv, self%base_cpu%w_cpu&
        &, self%wmean_cpu, self%equation_base%p0, self%equation_base%rgas0, self%equation_base%indx_cp_l, sel&
        &f%equation_base%indx_cp_r, self%cv_coeff_cpu, self%equation_base%t0, self%equation_base%calorically_&
        &perfect)
      case(10)
...
!
  subroutine bcrecyc(self, ilat)
    ...
    if (ilat == 1) then
      ...
      &, nv => self%nv, wrecycav_cpu => self%wrecycav_cpu, wrecyc_cpu => self%wrecyc_cpu, mp_cartz => self%&
      &field%mp_cartz, iermpi => self%mpi_err, p0 => self%equation_base%p0, w_cpu => self%base_cpu%w_cpu, w&
      &mean_cpu => self%wmean_cpu, weta_inflow_cpu => self%weta_inflow_cpu, map_j_inn_cpu => self%map_j_inn&
      &_cpu, map_j_out_cpu => self%map_j_out_cpu, yplus_inflow_cpu => self%yplus_inflow_cpu, eta_inflow_cpu&
      & => self%eta_inflow_cpu, yplus_recyc_cpu => self%yplus_recyc_cpu, eta_recyc_cpu => self%eta_recyc_cp&
      ...
      &v_coeff_cpu => self%cv_coeff_cpu, inflow_random_plane => self%equation_base%inflow_random_plane, inf&
      &low_random_plane_cpu => self%inflow_random_plane_cpu)
...
!
        call bcrecyc_subroutine_3(nx, ny, nz, ng, p0, u0, rgas0, w_cpu, wmean_cpu, wrecyc_cpu, weta_&
        &inflow_cpu, map_j_inn_cpu, map_j_out_cpu, yplus_inflow_cpu, eta_inflow_cpu, yplus_recyc_cpu, eta_rec&
        &yc_cpu, betarecyc, inflow_random_plane_cpu, indx_cp_l, indx_cp_r, cv_coeff_cpu, t0, calorically_perf&
        &ect,rand_type)
      endassociate
    endif
!
...
!
  subroutine compute_residual(self)
    class(equation_singleideal_cpu_object), intent(inout) :: self
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv => self%nv, dt => self%&
    &equation_base%dt, fln_cpu => self%fln_cpu, residual_rhou => self%equation_base%residual_rhou, fluid_&
    &mask_cpu => self%fluid_mask_cpu, vmax => self%equation_base%vmax, w_cpu => self%base_cpu%w_cpu)
...
!
  subroutine compute_dt(self)
    ...
    &s => self%sutherland_s, w_cpu => self%base_cpu%w_cpu, w_aux_cpu => self%w_aux_cpu, dcsidx_cpu => sel&
    &f%base_cpu%dcsidx_cpu, detady_cpu => self%base_cpu%detady_cpu, dzitdz_cpu => self%base_cpu%dzitdz_cp&
    &u, dcsidxs_cpu => self%base_cpu%dcsidxs_cpu, detadys_cpu => self%base_cpu%detadys_cpu, dzitdzs_cpu =&
    &> self%base_cpu%dzitdzs_cpu, calorically_perfect => self%equation_base%calorically_perfect, indx_cp_&
    &l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r, cp_coeff_cpu => self%c&
    &p_coeff_cpu, fluid_mask_cpu => self%fluid_mask_cpu, rgas0 => self%equation_base%rgas0, t0 => self%eq&
    &uation_base%t0)
      if (cfl < 0) then
        dt = -cfl
      else
        call compute_dt_subroutine(nx, ny, nz, ng, rgas0, prandtl, dcsidx_cpu, detady_cpu, dzitdz_cp&
        &u, dcsidxs_cpu, detadys_cpu, dzitdzs_cpu, w_cpu, w_aux_cpu, dtxi_max, dtyi_max, dtzi_max, dtxv_max, &
        &dtyv_max, dtzv_max, dtxk_max, dtyk_max, dtzk_max, indx_cp_l, indx_cp_r, cp_coeff_cpu,fluid_mask_cpu,&
...
!
    ...
    &im, sutherland_s => self%sutherland_s, powerlaw_vtexp => self%powerlaw_vtexp, cv_coeff_cpu => self%c&
    &v_coeff_cpu, calorically_perfect => self%equation_base%calorically_perfect, indx_cp_l => self%equati&
...
!
  subroutine manage_output(self)
    ...
    &0, icyc => self%equation_base%icyc, time => self%equation_base%time, w_aux_cpu => self%w_aux_cpu, ij&
    &k_probe_cpu => self%ijk_probe_cpu, w_aux_probe_cpu => self%w_aux_probe_cpu, probe_coeff_cpu => self%&
    &probe_coeff_cpu, nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, nv_aux => self%nv_aux, &
...
!
      ...
      if (time_from_last_slice >= self%equation_base%dtslice) then
        ...
        if (self%equation_base%num_probe>0) then
          call probe_interpolation_subroutine(self%equation_base%num_probe,nx,ny,nz,ng,nv_aux,ijk_pr&
          &obe_cpu,w_aux_probe_cpu,w_aux_cpu,probe_coeff_cpu)
