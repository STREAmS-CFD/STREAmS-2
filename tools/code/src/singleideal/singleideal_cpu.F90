module streams_equation_singleideal_cpu_object
  ...
  type :: equation_singleideal_cpu_object
    ...
    real(rkind), allocatable, dimension(:) :: coeff_filter_cpu
    ...
    real(rkind), pointer, contiguous, dimension(:,:,:,:) :: w_aux_cpu
    integer, pointer, contiguous, dimension(:,:,:) :: fluid_mask_cpu
    integer, pointer, contiguous, dimension(:,:,:,:) :: ep_ord_change_cpu
    ...
    integer, dimension(:), allocatable :: lmax_tag_cpu
    integer, dimension(:), allocatable :: vis_tag_cpu

    real(rkind), dimension(:,:,:,:,:), allocatable :: w_tspec_cpu
    real(rkind), dimension(:,:), allocatable :: w_psd_tspec_cpu
    real(rkind), dimension(:,:), allocatable :: wfar_cpu
    real(rkind), dimension(:), allocatable :: f_sponge_cpu


    !insitu_var_start
    real(rkind), dimension(:,:,:,:), allocatable :: psi_cpu
    integer, dimension(:), allocatable :: aux_list_cpu, add_list_cpu
    !insitu_var_end
  ...
  contains
    ...
    !insitu_proc_start
    procedure, pass(self) :: insitu_alloc_cpu
...
contains
  ...
  subroutine point_to_equation_base(self, equation_base)
    ...
    self%w_aux_cpu          => equation_base%w_aux
    self%ep_ord_change_cpu => equation_base%ep_ord_change
    self%fluid_mask_cpu    => equation_base%fluid_mask
  ...
  subroutine insitu_alloc_cpu(self)
    class(equation_singleideal_cpu_object), intent(inout) :: self
    ...
    &x_list, n_add_list => self%equation_base%n_add_list )
      if (npsi > 0) allocate(self%psi_cpu(1-ng:nx+ng,1-ng:ny+ng,1-ng:nz+ng,npsi))
      if (n_aux_list > 0) then
        allocate(self%aux_list_cpu(1:n_aux_list))
        self%aux_list_cpu = self%equation_base%aux_list
      endif
      if (n_add_list > 0) then
        allocate(self%add_list_cpu(1:n_add_list))
        self%add_list_cpu = self%equation_base%add_list
      endif
    endassociate
  endsubroutine insitu_alloc_cpu
  ...
  subroutine visflx(self, mode)
    ...
    &exp, coeff_deriv1_cpu => self%coeff_deriv1_cpu, coeff_deriv2_cpu => self%coeff_deriv2_cpu,&
    & fhat_trans_cpu => self%fhat_trans_cpu, fl_trans_cpu => self%fl_trans_cpu, fl_cpu => self%fl_cpu,&
    & w_aux_cpu => self%w_aux_cpu, w_aux_trans_cpu => self%w_aux_trans_cpu, dcsidx_cpu => self%base_cpu%d&
    &csidx_cpu, detady_cpu => self%base_cpu%detady_cpu, dzitdz_cpu => self%base_cpu%dzitdz_cpu,&
    & dcsidxs_cpu => self%base_cpu%dcsidxs_cpu, detadys_cpu => self%base_cpu%detadys_cpu,&
    & dzitdzs_cpu => self%base_cpu%dzitdzs_cpu, dcsidx2_cpu => self%base_cpu%dcsidx2_cpu,&
    & detady2_cpu => self%base_cpu%detady2_cpu, dzitdz2_cpu => self%base_cpu%dzitdz2_cpu,&
    & x_cpu => self%base_cpu%x_cpu, y_cpu => self%base_cpu%y_cpu, z_cpu => self%base_cpu%z_cpu,&
    ...
    & grid_dim => self%grid%grid_dim, dcsidxc2_cpu => self%base_cpu%dcsidxc2_cpu, dcsidyc2_cpu => self%ba&
    &se_cpu%dcsidyc2_cpu, detadxc2_cpu => self%base_cpu%detadxc2_cpu, detadyc2_cpu => self%base_cpu%detad&
    &yc2_cpu, g1_cpu => self%base_cpu%g1_cpu, g2_cpu => self%base_cpu%g2_cpu, g12_cpu => self%base_cpu%g1&
    &2_cpu, jac_cpu => self%base_cpu%jac_cpu, vis_tag_cpu => self%vis_tag_cpu, wall_tag_cpu => self%base_&
    &cpu%wall_tag_cpu, iblock => self%field%ncoords(1), ite_rank_x => self%field%ite_rank_x,&
    ...
    &weno, theta_ij_cpu => self%base_cpu%theta_ij_cpu, theta_threshold => self%equation_base%theta_thresh&
    &old)
      ...
      if (mode == 0) then
        ...
        elseif (grid_dim == 2) then
          ...
          & cp_coeff_cpu, calorically_perfect, u0, l0, self%base_cpu%w_cpu, self%w_aux_cpu, self%fl_cpu,&
          & coeff_deriv1_cpu, coeff_deriv2_cpu, dcsidxc2_cpu, detadyc2_cpu, detadxc2_cpu, dcsidyc2_cpu,&
          & dzitdz_cpu, dzitdzs_cpu, dzitdz2_cpu, g1_cpu, g2_cpu, g12_cpu, jac_cpu, self%wallprop_cpu,&
          & vis_tag_cpu, wall_tag_cpu, iblock, ite_rank_x, itu_rank_x, ite_l, itu_l, teshk )
        endif
      elseif (mode == 1) then
        if (grid_dim == 1) then
        ...
        elseif (grid_dim == 2) then
          call visflx_div_c2_subroutine(nx, ny, nz, ng, visc_order, self%w_aux_cpu, self%fl_cpu,&
          & coeff_deriv1_cpu, dcsidxc2_cpu, dcsidyc2_cpu, detadxc2_cpu, detadyc2_cpu, dzitdz_cpu, vis_tag_cpu,&
          & wall_tag_cpu, stream1)
        endif
      elseif (mode == 2) then
      ...
      elseif (mode == 3) then
        ...
        elseif (grid_dim == 2) then
          call sensor_c2_subroutine(nx, ny, nz, ng, u0, l0_ducros, self%w_aux_cpu, iblock,&
          & ite_rank_x, itu_rank_x, ite_l, itu_l, teshk, theta_ij_cpu, theta_threshold, jweno)
        endif
      elseif (mode == 4) then
        if (grid_dim == 1) then
        ...
        elseif (grid_dim == 2) then
          ...
          & indx_cp_r, cp_coeff_cpu, calorically_perfect, iblock, ite_rank_x, itu_rank_x, ite_l, itu_l, u0, l0,&
          & self%base_cpu%w_cpu, self%w_aux_cpu, self%fl_cpu, coeff_deriv1_cpu, coeff_deriv2_cpu, dcsidxc2_cpu,&
          & detadyc2_cpu, detadxc2_cpu, dcsidyc2_cpu, dzitdz_cpu, dzitdzs_cpu, dzitdz2_cpu, g1_cpu, g2_cpu,&
          & g12_cpu, jac_cpu, self%wallprop_cpu, vis_tag_cpu, wall_tag_cpu, ortho)
        endif
      elseif (mode == 5) then
  ...
  subroutine rk_sync(self)
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        if(flow_init == 5) then
          call self%base_cpu%bcswap_wake_var(self%w_aux_cpu(:,:,:,10:10))
          call self%base_cpu%bcswap_var(self%w_aux_cpu(:,:,:,10:10))
          call self%bcextr_var(self%w_aux_cpu(:,:,:,10:10), mode=2)
          call self%base_cpu%bcte_var(self%w_aux_cpu(:,:,:,10:10))
        else
          call self%bcextr_var(self%w_aux_cpu(:,:,:,10:10), mode=1)
          call self%base_cpu%bcswap_var(self%w_aux_cpu(:,:,:,10:10))
        endif
        ...
        if (istep == 3) then
          ...
          if(flow_init == 5) then
            call self%base_cpu%bcswap_wake_var(self%w_aux_cpu(:,:,:,8:8))
          endif
        endif
        if (conservative_viscous==1) then
  ...
  subroutine bcextr_var(self, w_swap, mode)
    ...
    & irightz => self%field%irightz)
      ...
      elseif(mode == 2) then
        call bcextr_airfoil_var_subroutine(nx, ny, nz, ng, ndim, self%base_cpu%wall_tag_cpu, w_swap,&
        & ileftx, irightx, ileftz, irightz)
      endif
    endassociate
    ...
    & grid_dim => self%grid%grid_dim)
      ...
      do istep=1,self%equation_base%nrk
        ...
        if (istep == 3) then
          ...
          if(flow_init == 5) then
            call self%base_cpu%bcswap_var(self%w_aux_cpu(:,:,:,8:8))
            call self%base_cpu%bcswap_wake_var(self%w_aux_cpu(:,:,:,8:8), steps=[.true.,.false.,.false.])
            if (conservative_viscous==1) then
              call self%visflx(mode=7)
            else
              call self%visflx(mode=1)
            endif
            call self%base_cpu%bcswap_wake_var(self%w_aux_cpu(:,:,:,8:8), steps=[.false.,.true.,.true.])
          else
  ...
  subroutine sponge(self)
    class(equation_singleideal_cpu_object), intent(inout) :: self
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_cpu=>self%base_cpu%w_cpu&
    &, wfar_cpu=>self%wfar_cpu,fln_cpu=>self%fln_cpu,f_sponge_cpu=>self%f_sponge_cpu, j_sponge=>self%equa&
    &tion_base%j_sponge)

      call sponge_subroutine(nx,ny,nz,ng,nv,w_cpu,wfar_cpu,fln_cpu,f_sponge_cpu,j_sponge)
    endassociate
  ...
  subroutine limiter(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,w_cpu=>self%base_cpu%w_cpu,&
    &w_aux_cpu=>self%w_aux_cpu, ncoords=>self%field%ncoords, indx_cp_l=>self%equation_base%indx_cp_l,&
    & indx_cp_r=>self%equation_base%indx_cp_r, cv_coeff_cpu => self%cv_coeff_cpu, calorically_perfect => &
    ...
    &m_rescale)
      ...
      call limiter_subroutine(nx,ny,nz,ng,w_cpu,w_aux_cpu,iblock,kblock,indx_cp_l,indx_cp_r,&
      &cv_coeff_cpu,calorically_perfect,t0, tol_iter_nr, rho_lim, tem_lim, rho_lim_rescale,&
      & tem_lim_rescale)
    endassociate
  ...
  subroutine filter(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,w_cpu=>self%base_cpu%w_cpu,&
    &w_aux_cpu=>self%w_aux_cpu,indx_cp_l=>self%equation_base%indx_cp_l,indx_cp_r=>self%equation_base%indx&
    &_cp_r,cv_coeff_cpu => self%cv_coeff_cpu,calorically_perfect => self%equation_base%calorically_perfec&
    &t,t0 => self%equation_base%t0, coeff_filter_cpu => self%coeff_filter_cpu,jfilter => self%equation_ba&
    &se%jfilter, wall_tag_cpu => self%base_cpu%wall_tag_cpu)


      call filter_subroutine(nx,ny,nz,ng,w_cpu,w_aux_cpu,indx_cp_l,indx_cp_r,cv_coeff_cpu,&
  ...
  subroutine tripping(self)
    ...
    associate(nx=>self%nx,ny=>self%ny,nz=>self%nz,ng=>self%ng,nv=>self%nv,w_cpu=>self%base_cpu%w_cpu&
    &,fl_cpu=>self%fl_cpu, xc2_cpu => self%base_cpu%xc2_cpu, yc2_cpu => self%base_cpu%yc2_cpu,&
    & z_cpu => self%base_cpu%z_cpu, ncoords=>self%field%ncoords, rlz => self%grid%domain_size(3),&
    & wall_tag_cpu => self%base_cpu%wall_tag_cpu, dxdetanc2_cpu => self%base_cpu%dxdetanc2_cpu,&
    & dydetanc2_cpu => self%base_cpu%dydetanc2_cpu, a_tr => self%equation_base%a_tr, u0 => self%equation_&
    ...
    &e%is_old)
      ...
      &lamx,lamy,lams,lams1,phis,phis1,asl,bt,xc2_cpu,yc2_cpu,z_cpu,w_cpu,fl_cpu,wall_tag_cpu,&
      & dxdetanc2_cpu, dydetanc2_cpu)
  ...
  subroutine euler_x(self, istart, iend, istart_face, iend_face, do_update)
    ...
    & dcsidxc2_cpu => self%base_cpu%dcsidxc2_cpu, dcsidxnc2_cpu => self%base_cpu%dcsidxnc2_cpu,&
    & dcsidyc2_cpu => self%base_cpu%dcsidyc2_cpu, dcsidync2_cpu => self%base_cpu%dcsidync2_cpu,&
    & jac_cpu => self%base_cpu%jac_cpu, mcsijac1_cpu => self%base_cpu%mcsijac1_cpu, lmax_tag_cpu => self%&
    &lmax_tag_cpu, eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_base%eul_imax,&
    & rgas0 => self%equation_base%rgas0)
      ...
      if(iend - istart >= 0) then
        ...
        elseif (self%grid%grid_dim == 2) then
          ...
          & dcsidync2_cpu, jac_cpu, mcsijac1_cpu, lmax_tag_cpu, force_zero_flux_min, force_zero_flux_max,&
          ...
          call euler_x_update_c2_subroutine(nx, ny, nz, ng, nv, istart, iend, fhat_cpu, fl_cpu, jac_cpu, stream1)
        endif
  ...
  subroutine euler_y(self, eul_jmin, eul_jmax)
    ...
    & nkeep => self%equation_base%nkeep, wall_tag_cpu => self%base_cpu%wall_tag_cpu, flux_splitting => se&
    &lf%equation_base%flux_splitting, detadxc2_cpu => self%base_cpu%detadxc2_cpu, detadxnc2_cpu => self%b&
    &ase_cpu%detadxnc2_cpu, detadyc2_cpu => self%base_cpu%detadyc2_cpu, detadync2_cpu => self%base_cpu%de&
    &tadync2_cpu, jac_cpu => self%base_cpu%jac_cpu, metajac1_cpu => self%base_cpu%metajac1_cpu,&
    & rgas0 => self%equation_base%rgas0)
      weno_size = 2*weno_scheme
      lmax = ep_order/2
      ...
      elseif (self%grid%grid_dim == 2) then
        ...
        & rgas0, w_aux_cpu, fl_cpu, coeff_deriv1_cpu, fhat_cpu, detadxc2_cpu, detadxnc2_cpu, detadyc2_cpu,&
        & detadync2_cpu, jac_cpu, metajac1_cpu, wall_tag_cpu, force_zero_flux_min, force_zero_flux_max,&
        & weno_scheme, weno_version, sensor_threshold, weno_size, cp_coeff_cpu, indx_cp_l, indx_cp_r,&
        & ep_ord_change_cpu, calorically_perfect, tol_iter_nr,self%equation_base%rho0,self%equation_base%u0,&
        & self%equation_base%t0)
        call euler_y_update_c2_subroutine(nx, ny, nz, ng, nv, eul_jmin, eul_jmax, fhat_cpu, fl_cpu, jac_cpu, wall_tag_cpu, stream1)
      endif
    endassociate
  ...
  subroutine force_rhs(self)
    ...
    &pu, r_curv => self%grid%r_curv, volume => self%field%volume, dxdcsinc2_cpu => self%base_cpu%dxdcsinc&
    &2_cpu, dydcsinc2_cpu => self%base_cpu%dydcsinc2_cpu, dcsidxnc2_cpu => self%base_cpu%dcsidxnc2_cpu,&
    & dcsidync2_cpu => self%base_cpu%dcsidync2_cpu, jac_cpu => self%base_cpu%jac_cpu)

      if(self%grid%grid_dim == 1) then
        call force_rhs_1_subroutine(nx, ny, nz, ng, yn_cpu, fln_cpu, w_cpu, w_aux_cpu, bulk5, fluid_mask_cpu)
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_1_c2_subroutine(nx, ny, nz, ng, yn_cpu, fln_cpu, w_cpu, w_aux_cpu, bulk5,&
      ...
      elseif(self%grid%grid_dim == 2) then
        call force_rhs_2_c2_subroutine(nx, ny, nz, ng, fln_cpu, w_aux_cpu, bulk5g, fluid_mask_cpu,&
        & r_curv, yn_cpu, dxdcsinc2_cpu, dydcsinc2_cpu)
      endif
    endassociate
  ...
  subroutine force_var(self)
    ...
    & yn_cpu => self%base_cpu%yn_cpu, w_cpu => self%base_cpu%w_cpu, w_aux_cpu => self%w_aux_cpu,&
    & fln_cpu => self%fln_cpu, volchan => self%equation_base%volchan, fluid_mask_cpu => self%fluid_mask_c&
    &pu, cv_coeff_cpu => self%cv_coeff_cpu, volume => self%field%volume, t0 => self%equation_base%t0,&
    & time => self%equation_base%time, calorically_perfect => self%equation_base%calorically_perfect,&
    & jac_cpu => self%base_cpu%jac_cpu, dcsidxnc2_cpu => self%base_cpu%dcsidxnc2_cpu, dcsidync2_cpu => se&
    &lf%base_cpu%dcsidync2_cpu, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_bas&
    &e%indx_cp_r)
      ...
      if (self%equation_base%theta_wall>=-1._rkind) then
        ...
        elseif(self%grid%grid_dim == 2) then
          call force_var_1_c2_subroutine(nx, ny, nz, ng, yn_cpu, fln_cpu, w_cpu, w_aux_cpu, bulkt,&
          ...
          call force_var_2_c2_subroutine(nx, ny, nz, ng, w_cpu, w_aux_cpu, tbdiff, fluid_mask_cpu,&
  ...
  subroutine bc_nr(self)
    ...
    &dy_cpu, jac_cpu => self%base_cpu%jac_cpu , dcsidxnc2_cpu => self%base_cpu%dcsidxnc2_cpu ,&
    & dcsidync2_cpu => self%base_cpu%dcsidync2_cpu , dcsidxc2_cpu => self%base_cpu%dcsidxc2_cpu ,&
    & dcsidyc2_cpu => self%base_cpu%dcsidyc2_cpu , detadxnc2_cpu => self%base_cpu%detadxnc2_cpu ,&
    & detadync2_cpu => self%base_cpu%detadync2_cpu , detadxc2_cpu => self%base_cpu%detadxc2_cpu ,&
    & detadyc2_cpu => self%base_cpu%detadyc2_cpu , wall_tag_cpu => self%base_cpu%wall_tag_cpu ,&
    & dzitdz_cpu => self%base_cpu%dzitdz_cpu, wfar_cpu => self%wfar_cpu, mcsi_cpu => self%base_cpu%mcsi_c&
    &pu, meta_cpu => self%base_cpu%meta_cpu, indx_cp_l => self%equation_base%indx_cp_l,&
    & indx_cp_r => self%equation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_pe&
        ...
        if(bctags_nr(ilat) > 0) then
          ...
          if(dir == 1) then
            ...
            elseif(self%grid%grid_dim == 2) then
              ...
              & w_aux_cpu, w_cpu, fl_cpu, dcsidx_cpu, indx_cp_l, indx_cp_r, cp_coeff_cpu, winf_cpu, jac_cpu,&
              & mcsi_cpu, dcsidxnc2_cpu, dcsidync2_cpu, dcsidxc2_cpu, dcsidyc2_cpu, calorically_perfect,rgas0,t0)
            endif
          endif
          if(dir == 2) then
            ...
            elseif(self%grid%grid_dim == 2) then
              ...
              & w_aux_cpu, w_cpu, fl_cpu, detady_cpu, indx_cp_l, indx_cp_r, cp_coeff_cpu, wfar_cpu, jac_cpu,&
              & meta_cpu, detadxnc2_cpu, detadync2_cpu, detadxc2_cpu, detadyc2_cpu, wall_tag_cpu,&
  ...
  subroutine update_ghost(self, do_swap)
    ...
    do ilat=1,2*self%grid%ndim
      ...
      case(3)
        call bcfree_sub_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%base_cpu%w_cpu,&
        & self%w_aux_cpu, self%equation_base%aoa, self%equation_base%t0, self%equation_base%ptot0,&
        & self%equation_base%ttot0, self%equation_base%rgas0, self%equation_base%indx_cp_l,&
        & self%equation_base%indx_cp_r, self%cv_coeff_cpu, self%cp_coeff_cpu, self%equation_base%calorically_&
        &perfect, tol_iter_nr)
      case(2)
      ...
      case(5)
        if(self%grid%grid_dim == 1) then
          call bcsym_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%base_cpu%w_cpu)
        elseif(self%grid%grid_dim == 2) then
          call bcsym_c2_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%base_cpu%w_cpu,&
          & self%base_cpu%dxdcsic2_cpu, self%base_cpu%dydcsic2_cpu)
        endif
      case(6)
      ...
      case(11)
        call self%base_cpu%bcswap_wake()
        call bcwall_airfoil_subroutine(ilat, self%nx, self%ny, self%nz, self%ng, self%equation_base%&
        &t_wall, self%base_cpu%w_cpu, self%w_aux_cpu, self%base_cpu%wall_tag_cpu, self%equation_base%indx_cp_&
        &l, self%equation_base%indx_cp_r, self%cv_coeff_cpu, self%equation_base%t0, self%equation_base%rgas0,&
        & self%equation_base%calorically_perfect,tol_iter_nr, self%base_cpu%xc2_cpu, self%base_cpu%yc2_cpu,&
        ...
        &e, self%base_cpu%dxdetanc2_cpu, self%base_cpu%dydetanc2_cpu, self%equation_base%u0)
        call self%base_cpu%bcte()
      endselect
    enddo
  ...
  subroutine bcrecyc(self, ilat)
    ...
    if (ilat == 1) then
      ...
      & cv_coeff_cpu => self%cv_coeff_cpu, cp_coeff_cpu => self%cp_coeff_cpu, inflow_random_plane => self%e&
      &quation_base%inflow_random_plane, inflow_random_plane_cpu => self%inflow_random_plane_cpu)
        ...
        & inflow_random_plane_cpu, indx_cp_l, indx_cp_r, cv_coeff_cpu, cp_coeff_cpu, t0, calorically_perfect,&
        &rand_type)
      endassociate
    endif
  ...
  subroutine initialize(self, filename)
    ...
    call self%base_cpu%initialize(self%equation_base%field, self%equation_base%gpu_bind)
    ...
    if (self%grid%grid_dim == 2) self%wfar_cpu = self%equation_base%wfar

    self%lmax_tag_cpu = self%equation_base%lmax_tag
    self%vis_tag_cpu = self%equation_base%vis_tag
    ...
    if(self%equation_base%enable_tspec > 0) then
      self%w_tspec_cpu = self%equation_base%w_tspec
    endif
    ...
    if (self%equation_base%ifilter > 0) then
      self%coeff_filter_cpu = self%equation_base%coeff_filter
    endif

    if (self%equation_base%enable_sponge > 0) then
      self%f_sponge_cpu = self%equation_base%f_sponge
    endif
  ...
  subroutine alloc(self)
    ...
    &on_base%indx_cp_r, ndft => self%equation_base%ndft_tspec, grid_dim => self%grid%grid_dim)
      ...
      if (grid_dim == 2) allocate (self%wfar_cpu(nx,nv))
      ...
      allocate(self%lmax_tag_cpu(1-ng:nx+ng))
      allocate(self%vis_tag_cpu(1-ng:nx+ng))

      if (self%equation_base%enable_tspec > 0) then
        allocate(self%w_psd_tspec_cpu(nx, ndft))
        allocate(self%w_tspec_cpu(nx, nz, ndft, 2, 2))
      endif

      if (self%equation_base%ifilter > 0) allocate(self%coeff_filter_cpu(0:4))
      if (self%equation_base%enable_sponge > 0) allocate(self%f_sponge_cpu(ny))
  ...
  subroutine count_weno(self)
    ...
    &ion_base%weno_scheme, sensor_threshold => self%equation_base%sensor_threshold, w_aux_cpu => self%w_a&
    &ux_cpu, ep_ord_change_cpu => self%ep_ord_change_cpu, lmax_tag_cpu => self%lmax_tag_cpu,&
    & wall_tag_cpu => self%base_cpu%wall_tag_cpu, count_weno_x => self%equation_base%count_weno_x,&
    & count_weno_y => self%equation_base%count_weno_y, count_weno_z => self%equation_base%count_weno_z)
      ...
      if (grid_dim==1) then
        ...
        & eul_jmax, eul_kmin, eul_kmax, weno_scheme, sensor_threshold, w_aux_cpu, ep_ord_change_cpu,&
        & count_weno_x, count_weno_y, count_weno_z)
      else
        lmax = ep_order/2
        call count_weno_c2_subroutine(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin,&
        & eul_jmax, eul_kmin, eul_kmax, lmax, weno_scheme, sensor_threshold, w_aux_cpu, ep_ord_change_cpu ,&
        & lmax_tag_cpu, wall_tag_cpu, count_weno_x, count_weno_y, count_weno_z)
      endif
  ...
  subroutine compute_airfoil_forces_runtime(self)
    ...
    &0, u0 => self%equation_base%u0, rgas0 => self%equation_base%rgas0, w_aux_cpu => self%w_aux_cpu,&
    & wall_tag_cpu => self%base_cpu%wall_tag_cpu, meta_cpu => self%base_cpu%meta_cpu, csimod_cpu => self%&
    &base_cpu%csimod_cpu, dxdcsic2_cpu => self%base_cpu%dxdcsic2_cpu, dydcsic2_cpu => self%base_cpu%dydcs&
    &ic2_cpu, lift_af => self%equation_base%lift_af, drag_af => self%equation_base%drag_af,&
    & pres_af => self%equation_base%pres_af, fric_af => self%equation_base%fric_af)

      call compute_airfoil_forces_runtime_subroutine(nx, ny, nz, ng, nv, p0, u0, rgas0, w_aux_cpu,&
  ...
  subroutine compute_dt(self)
    ...
    &cpu, dzitdzs_cpu => self%base_cpu%dzitdzs_cpu, dcsidxnc2_cpu => self%base_cpu%dcsidxnc2_cpu,&
    & dcsidync2_cpu => self%base_cpu%dcsidync2_cpu, detadxnc2_cpu => self%base_cpu%detadxnc2_cpu,&
    & detadync2_cpu => self%base_cpu%detadync2_cpu, mcsi_cpu => self%base_cpu%mcsi_cpu,&
    & meta_cpu => self%base_cpu%meta_cpu, calorically_perfect => self%equation_base%calorically_perfect,&
    & indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equation_base%indx_cp_r,&
    ...
    &se%rgas0, t0 => self%equation_base%t0, ndim => self%grid%ndim)
      if (cfl < 0) then
        ...
        elseif(self%grid%grid_dim == 2) then
          call compute_dt_c2_subroutine(nx, ny, nz, ng, rgas0, prandtl, dcsidxnc2_cpu,&
          & dcsidync2_cpu, detadxnc2_cpu, detadync2_cpu, mcsi_cpu, meta_cpu, dzitdz_cpu, dzitdzs_cpu, w_cpu,&
          & w_aux_cpu, dtxi_max, dtyi_max, dtzi_max, dtxv_max, dtyv_max, dtzv_max, dtxk_max, dtyk_max,&
          & dtzk_max, indx_cp_l, indx_cp_r, cp_coeff_cpu,fluid_mask_cpu,calorically_perfect,t0)
        endif
        if(ndim == 2) then
  ...
  subroutine run(self, filename)
    ...
    &ion_base%time_is_freezed)
      ...
      if (self%equation_base%io_type_w==1) then
        ...
        if (self%equation_base%enable_tspec>0) then
          self%equation_base%w_tspec = self%w_tspec_cpu
  ...
  subroutine insitu_coprocess(self)
    ...
    integer :: l,i,j,k,ll
    call self%update_ghost()
    call self%base_cpu%bcswap_edges_corners()
    associate(nx => self%nx, ny => self%ny, nz => self%nz, ng => self%ng, visc_model => self%visc_mo&
    &del, mu0 => self%mu0, t_ref_dim => self%t_ref_dim, sutherland_s => self%sutherland_s,&
    & powerlaw_vtexp => self%powerlaw_vtexp, w_aux_cpu => self%w_aux_cpu, cv_coeff_cpu => self%cv_coeff_c&
    ...
    if (self%equation_base%npsi > 0) then
      ...
      do l=1,self%equation_base%npsi
        call self%base_cpu%bcswap_var(self%psi_cpu(:,:,:,l:l ))
      enddo
      do l=1,self%equation_base%npsi
        call self%base_cpu%bcswap_edges_corners_var(self%psi_cpu(:,:,:,l:l ))
      enddo
    endif
    associate( nxsl_ins => self%equation_base%nxsl_ins, nxel_ins => self%equation_base%nxel_ins,&
    & nysl_ins => self%equation_base%nysl_ins, nyel_ins => self%equation_base%nyel_ins,&
    ...
    &e%nv_aux, n_aux_list => self%equation_base%n_aux_list, psi_cpu => self%psi_cpu, n_add_list => self%e&
    &quation_base%n_add_list, w_aux_cpu => self%w_aux_cpu, aux_list_cpu => self%aux_list_cpu,&
    & add_list_cpu => self%add_list_cpu, nx => self%field%nx, ny => self%field%ny, nz => self%field%nz,&
    ...
    &e, time_insitu => self%equation_base%time_insitu)
      ...
      self%equation_base%w_aux = w_aux_cpu
      self%equation_base%psi = psi_cpu
  ...
  subroutine insitu_compute_psi(self)
    ...
    & dzitdz2_cpu => self%base_cpu%dzitdz2_cpu, dcsidxc2_cpu => self%base_cpu%dcsidxc2_cpu,&
    & detadyc2_cpu => self%base_cpu%detadyc2_cpu, detadxc2_cpu => self%base_cpu%detadxc2_cpu,&
    & dcsidyc2_cpu => self%base_cpu%dcsidyc2_cpu, vis_tag_cpu => self%vis_tag_cpu, wall_tag_cpu => self%b&
    &ase_cpu%wall_tag_cpu, eul_imin => self%equation_base%eul_imin, eul_imax => self%equation_base%eul_im&
    &ax, eul_jmin => self%equation_base%eul_jmin, eul_jmax => self%equation_base%eul_jmax,&
    & eul_kmin => self%equation_base%eul_kmin, eul_kmax => self%equation_base%eul_kmax,&
    & cv_coeff_cpu => self%cv_coeff_cpu, indx_cp_l => self%equation_base%indx_cp_l, indx_cp_r => self%equ&
    &ation_base%indx_cp_r, calorically_perfect => self%equation_base%calorically_perfect,&
    & npsi => self%equation_base%npsi, psi_cpu => self%psi_cpu, add_list => self%equation_base%add_list,&
    & x_cpu => self%base_cpu%x_cpu )
      do l=1,npsi
        if (add_list(l) == 1) then
          call insitu_div_subroutine(nx, ny, nz, ng, visc_order, npsi,l, w_aux_cpu,&
          & coeff_deriv1_cpu, dcsidx_cpu, detady_cpu, dzitdz_cpu, psi_cpu )
        endif
        if (add_list(l) == 2) then
          call insitu_omega_subroutine(nx, ny, nz, ng, visc_order, npsi,l, w_aux_cpu,&
          & coeff_deriv1_cpu, dcsidx_cpu, detady_cpu, dzitdz_cpu, psi_cpu )
        endif
        if (add_list(l) == 3) then
          call insitu_ducros_subroutine(nx, ny, nz, ng, visc_order, npsi,l, u0,l0,w_aux_cpu,&
          & coeff_deriv1_cpu, dcsidx_cpu, detady_cpu, dzitdz_cpu, psi_cpu )
        endif
        if (add_list(l) == 4) then
          ...
          if(self%grid%grid_dim == 1) then
            call insitu_swirling_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dcsidx_cpu,&
            & detady_cpu, dzitdz_cpu, w_aux_cpu, coeff_deriv1_cpu, psi_cpu, u0, x_cpu )
          elseif(self%grid%grid_dim == 2) then
            call insitu_swirling_c2_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dzitdz_cpu,&
            & w_aux_cpu, coeff_deriv1_cpu, dcsidxc2_cpu, detadyc2_cpu, detadxc2_cpu, dcsidyc2_cpu, psi_cpu, u0,&
            & x_cpu, vis_tag_cpu, wall_tag_cpu)
          endif
        endif
        if (add_list(l) == 5) then
          ...
          if(self%grid%grid_dim == 1) then
            call insitu_schlieren_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dcsidx_cpu,&
            & detady_cpu, dzitdz_cpu, w_aux_cpu, coeff_deriv1_cpu, psi_cpu, u0, x_cpu )
          elseif(self%grid%grid_dim == 2) then
            call insitu_schlieren_c2_kernel(nv, nx, ny, nz, visc_order, ng, npsi,l,dzitdz_cpu,&
            & w_aux_cpu, coeff_deriv1_cpu, dcsidxc2_cpu, detadyc2_cpu, detadxc2_cpu, dcsidyc2_cpu, psi_cpu, u0,&
            & x_cpu, vis_tag_cpu, wall_tag_cpu)
          endif
        endif
  ...
  subroutine manage_output(self)
    ...
    &icyc, time => self%equation_base%time, w_aux_cpu => self%w_aux_cpu, grid_dim => self%grid%grid_dim,&
    & ijk_probe_cpu => self%ijk_probe_cpu, w_aux_probe_cpu => self%w_aux_probe_cpu, probe_coeff_cpu => se&
    &lf%probe_coeff_cpu, w_aux_probe => self%equation_base%w_aux_probe, wallprop => self%equation_base%wa&
    &llprop, wallprop_cpu => self%wallprop_cpu, nx => self%nx, ny => self%ny, nz => self%nz,&
    ...
    &c, it_nwin_tspec => self%equation_base%it_nwin_tspec, w_tspec_cpu => self%w_tspec_cpu,&
    & w_psd_tspec_cpu => self%w_psd_tspec_cpu, w_psd_tspec => self%equation_base%w_psd_tspec,&
    ...
    &ic2_cpu )
      ...
      if (time_from_last_slice >= self%equation_base%dtslice) then
        ...
        if (allocated(self%equation_base%islice)) then
          ...
          do i=1,size(self%equation_base%islice)
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              sliceyz_aux(i,:,:,m) = self%w_aux_cpu(ii,:,:,mm)
            enddo
          enddo
            ...
            do m=1,self%equation_base%num_aux_slice
              ...
              if(jj == 1 .and. mm >= 2 .and. mm <= 4) then
                if (grid_dim == 2 .and. mm == 2) then
                  call compute_wallprop_c2_subroutine(nx,ny,nz,ng,w_aux_cpu,wallprop_cpu,&
                  &dxdcsic2_cpu,dydcsic2_cpu,csimod_cpu,meta_cpu,wall_tag_cpu)
                endif
                slicexz_aux(:,j,:,m) = wallprop_cpu(:,:,mm)
              else
                slicexz_aux(:,j,:,m) = self%w_aux_cpu(:,jj,:,mm)
              endif
            enddo
          enddo
        ...
        if (allocated(self%equation_base%kslice)) then
          ...
          do k=1,size(self%equation_base%kslice)
            ...
            do m=1,self%equation_base%num_aux_slice
              mm = self%equation_base%list_aux_slice(m)
              slicexy_aux(:,:,k,m) = self%w_aux_cpu(:,:,kk,mm)
            enddo
          enddo
      ...
      if (time_from_last_tspec >= self%equation_base%dt_tspec) then
        if(self%equation_base%enable_tspec > 0) then
          ...
          call compute_tspec_subroutine(nx, ny, nz, ng, ndft, jg_tspec, 1, it_win1_tspec, w_aux_cpu, w_tspec_cpu)
          if(self%equation_base%it_win2_tspec > 0) then
            call compute_tspec_subroutine(nx, ny, nz, ng, ndft, jg_tspec, 2, it_win2_tspec, w_aux_cpu, w_tspec_cpu)
          endif
          if(it_win1_tspec == ndft .or. it_win2_tspec == ndft) then
            if(it_win1_tspec == ndft) then
              call compute_psd_tspec_subroutine(nx, ny, nz, ndft, 1, dt_tspec, w_tspec_cpu, w_psd_tspec_cpu)
            endif
            if(it_win2_tspec == ndft) then
              call compute_psd_tspec_subroutine(nx, ny, nz, ndft, 2, dt_tspec, w_tspec_cpu, w_psd_tspec_cpu)
            endif

            w_psd_tspec = w_psd_tspec_cpu
