module crandom_f_mod
   ...
   implicit none
     ...
     integer, parameter, private :: c_rkind = C_FLOAT
#else
     integer, parameter, private :: c_rkind = C_DOUBLE
#endif
   ...
   subroutine init_crandom_f(num, reproducible)
       ...
       if(reproducible_) then
           ...
           start_seed = int(real(clock,c_rkind)+num,C_INT) !+ (x+1) * (/ (i - 1, i = 1, n) /)
       endif
